...
For content-unit image/video generation, do not start from a raw generated resource and then manually write a candidate. Edit the content unit `edit_prompt` with semantic refs, call `domain_build_content_unit_backend_prompt` or `generation_prepare` to verify that refs compile to selected resources without blockers, call `generation_submit` with `scope: "content_unit"`, then poll `generation_job_get`. Successful terminal polls automatically create or refresh backend content candidates. For video generation, treat `generation_submit` as a paid spend gate: stop after prompt compilation/readiness checks and ask for explicit user confirmation before submitting the video job. Image generation does not require this video-specific confirmation.
...
- Generate candidate: edit the content unit prompt, compile it with `domain_build_content_unit_backend_prompt` or `generation_prepare`, call `generation_submit` with `scope: "content_unit"`, then poll `generation_job_get`. The monitor creates/refreshed candidates automatically on success and does not auto-select. If the candidate is video, call `generation_submit` only after explicit user confirmation of the paid video generation job.
...
1. For normal content-unit image generation, edit/compile the content-unit prompt, submit the content-unit generation job, and poll the matching content-unit job tool until terminal. On success, the backend monitor creates or refreshes the candidate automatically. For normal content-unit video generation, edit/compile and report readiness first, then submit and poll only after explicit user confirmation.
