...
- Editing tools: `editing_project_*`, `editing_timeline_*`, `editing_runtime_capabilities_get`, `editing_task_*`, and `editing_export_*`. Pure project/timeline operations can run in the headless host. Render/HLS/transcode/reframe work is routed toward daemon-owned `movscript.media.pipeline`; Desktop may provide enhanced Electron `mediaPipeline` preview or bridge capabilities, but it is not the business sidecar owner.
