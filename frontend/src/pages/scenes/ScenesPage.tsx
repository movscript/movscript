    ...
    <div className="flex h-full overflow-hidden bg-background">
      ...
      <div className={cn('flex flex-col border-r border-border bg-card overflow-hidden', detailOpen ? 'w-72 shrink-0' : 'flex-1')}>
        <div className="flex items-center justify-between px-3 py-2.5 border-b border-border bg-background shrink-0">
          <span className="text-sm font-medium text-foreground px-1">分场列表</span>
            ...
            scenes.map((s) => (
              <button key={s.ID} onClick={() => setSelectedId(s.ID)}
                ...
                <div className="flex items-center gap-2">
                  <span className="text-xs text-muted-foreground font-mono shrink-0">场{s.number}</span>
              ...
              {scenes.map((s) => (
                <button key={s.ID} onClick={() => setSelectedId(s.ID)}
                  ...
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-sm font-mono text-muted-foreground">场{String(s.number).padStart(2, '0')}</span>
                    <ReviewStatusBadge status={s.review_status} />
                  </div>
      ...
      <CreateDialog open={showCreate} onClose={() => setShowCreate(false)} title="新建分场">
