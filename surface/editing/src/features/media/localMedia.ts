...
export function localMediaUrl(asset: ElectronMediaPipelineAssetDescriptor) {
  ...
  if (/^[a-zA-Z]:[\\/]/.test(localPath) || localPath.startsWith('/')) {
    if (usesLocalSurfaceMediaProxy()) return `${EDITING_LOCAL_SURFACE_MEDIA_URL}?path=${encodeURIComponent(localPath)}`
    return `${EDITING_LOCAL_FILE_MEDIA_URL}?path=${encodeURIComponent(localPath)}`
  }
  return localPath
}
...
function usesLocalSurfaceMediaProxy(): boolean {
