...
import type { Canvas, CanvasNodeData, CanvasType, PublicModel } from '@movscript/shared'
import { ensureFinalOutputNode, normalizeWorkflowIoNodeOrders } from '../domain/graph'
import { canvasNodeAbsolutePosition, normalizedCanvasNodeStyle } from '../domain/layout'
import { canvasGraphSignature, serializableCanvasNodeData } from '../domain/serialization'
import {
...
async function defaultModelsForCanvasSave(nodes: Node[]) {
  ...
  await Promise.all(Array.from(capabilityRequests.values()).map(async ({ capability }) => {
    const models = await canvasApi.get(canvasServicePaths.runtimeModels, { params: { capability } }).then((response) => response.data as PublicModel[])
