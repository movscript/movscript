...
import { canvasDocumentChangedResult, invalidateCanvasMutationResult } from '../application/canvasMutationInvalidation'
import { canvasApi, canvasServicePaths } from '../application/canvasServiceApi'
import type { Canvas, CanvasNodeData, CanvasType } from '@movscript/shared'
import { toast } from '@movscript/ui/toast'
import { canvasGraphSignature } from '../domain/serialization'
...
export function useCanvasDocument({
  ...
  const persistCanvasGraph = useCallback(async (nextNodes: Node[], nextEdges: Edge[]) => {
    ...
    await canvasApi.put(canvasServicePaths.canvas(canvasId), payload)
