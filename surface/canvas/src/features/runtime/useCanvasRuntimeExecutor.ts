...
import { nodeAcceptsTextResult } from '../domain/graph'
import { hydrateCanvasDocument } from '../editor/canvasDocument'
import {
  workflowInputValuesForReferenceNode,
  workflowReferenceOutputsForNode,
} from '../integrations/workflowReferences'
import {
...
} from './canvasRuntimeGeneration'
import type { Canvas, CanvasNodeData, CanvasPortValue, RawResource } from '@movscript/shared'
import {
...
export function useCanvasRuntimeExecutor({
  ...
  const executeCanvasRuntimeNode = useCallback(async (
    ...
    if (node.type === 'canvas') {
      ...
      const referencedCanvas = await canvasApi.get(canvasServicePaths.canvas(data.referencedCanvasId)).then((response) => response.data as Canvas)
