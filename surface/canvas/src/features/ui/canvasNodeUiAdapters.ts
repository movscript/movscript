...
export function canvasDisplayLabel(label: unknown, fallback: string, t: CanvasTranslator) {
  const fallbackLabel = TRANSLATABLE_LABEL_KEY.test(fallback) ? t(fallback) : fallback
  const value = typeof label === 'string' ? label.trim() : ''
  if (!value) return fallbackLabel
  return TRANSLATABLE_LABEL_KEY.test(value) ? t(value, { defaultValue: fallbackLabel }) : value
}
