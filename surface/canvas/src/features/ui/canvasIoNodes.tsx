...
import type { CanvasParamType } from '@movscript/shared'
import {
  CanvasIOActionCard,
...
export function InputNode({ data, selected }: NodeProps & { data: NodeDataWithHandlers }) {
  ...
  const title = data.paramName || canvasDisplayLabel(data.label, order ? `${t('canvas.nodeLabels.input')} ${order}` : 'canvas.nodeLabels.input', t)
  return (
    <CanvasIOActionCard
      tone="sky"
      icon={LogIn}
...
export function OutputNode({ data, selected }: NodeProps & { data: NodeDataWithHandlers }) {
  ...
  const title = data.paramName || canvasDisplayLabel(data.label, order ? `${t('canvas.nodeLabels.output')} ${order}` : 'canvas.nodeLabels.output', t)
  return (
    <CanvasIOActionCard
      tone="emerald"
      icon={LogOut}
...
export function ResourceSinkNode({ data, selected }: NodeProps & { data: NodeDataWithHandlers }) {
  ...
  return (
    <CanvasIOActionCard
      tone="amber"
      icon={HardDrive}
      title={canvasDisplayLabel(data.label, 'canvas.nodeLabels.resource_sink', t)}
  ...
  return (
    <CanvasNodeCard selected={selected}>
      <CanvasNodeCardHeader
        ...
        label={canvasDisplayLabel(data.label, 'canvas.nodeLabels.approval', t)}
        tone="warning"
