...
import { canvasTextNodeEditState } from '../editor/nodeFactory'
import { canvasResourceKeys } from '@movscript/resource-surface/data'
import { loadResourceTextUrl, resolveResourceUrl } from '@movscript/resource-surface/resource-media'
import { MediaViewer } from '@movscript/resource-surface/resource-media-viewer'
import { ResourceImage } from '@movscript/resource-surface/resource-media-components'
...
export function TextNode({ data, selected }: NodeProps & { data: NodeDataWithHandlers }) {
  ...
  return (
    <CanvasTextNodeView
      ...
      label={canvasDisplayLabel(data.label, 'canvas.nodeLabels.text', t)}
      status={status}
...
export function ImageNode({ data, selected }: NodeProps & { data: NodeDataWithHandlers }) {
  ...
  return (
    <CanvasImageNodeView
      selected={selected}
      icon={<Image size={12} />}
      label={canvasDisplayLabel(data.label, 'canvas.nodeLabels.image', t)}
      status={status}
      statusIcons={canvasNodeStatusIcons}
...
export function VideoNode({ data, selected }: NodeProps & { data: NodeDataWithHandlers }) {
  ...
  return (
    <CanvasVideoNodeView
      selected={selected}
      icon={<Video size={12} />}
      label={canvasDisplayLabel(data.label, 'canvas.nodeLabels.video', t)}
      status={status}
      statusIcons={canvasNodeStatusIcons}
      runIcon={<Play size={12} />}
      onRun={data.onRun}
