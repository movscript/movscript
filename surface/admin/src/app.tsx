...
import { useUserStore, type AuthSession } from '@admin/store/userStore'
...
export type AdminLaunchContext = (AuthSession & {
...
const baseNavItems: { to: string; labelKey: string; icon: LucideIcon; end?: boolean }[] = [
  { to: '/overview', labelKey: 'admin.nav.overview', icon: Database, end: true },
...
export type AdminWindowChrome = 'auto' | 'window' | 'none'
...
function useAdminWindowController() {
  ...
  const hasWindowControl = typeof electronApi?.windowControl === 'function'
  ...
  return { hasWindowControl, isMacOS, windowControl, windowState }
}
...
function AdminWindowHeader({ windowChrome }: { windowChrome: AdminWindowChrome }) {
  const { t } = useTranslation()
  const { hasWindowControl, isMacOS, windowControl, windowState } = useAdminWindowController()
  if (windowChrome === 'none') return null
  if (windowChrome === 'auto' && !hasWindowControl) return null

  return (
    <AppWindowHeader
      isMacOS={isMacOS}
      windowControls={isMacOS && hasWindowControl ? (
      ...
      controls={!isMacOS && hasWindowControl ? (
        <AppWindowControls>
...
function AdminShell({ children, windowChrome }: { children: React.ReactNode; windowChrome: AdminWindowChrome }) {
  const { t } = useTranslation()
  ...
  return (
    <div className="flex h-screen flex-col bg-background text-foreground">
      <AdminWindowHeader windowChrome={windowChrome} />
...
export interface AdminSurfaceAppProps {
  basename?: string
  launchContext?: AdminLaunchContext
  windowChrome?: AdminWindowChrome
}

function useAdminSurfaceBootstrap(launchContext: AdminLaunchContext | undefined) {
  const initializedRef = React.useRef(false)
  if (initializedRef.current) return
  const session = launchContext === undefined ? readAdminLaunchContextFromHash() : launchContext
  applyAdminLaunchContext(session)
  initTheme()
  bootstrapElectronAdminSession(session)
  initializedRef.current = true
}

export function AdminSurfaceApp({ basename = adminBasename, launchContext, windowChrome = 'auto' }: AdminSurfaceAppProps = {}) {
  useAdminSurfaceBootstrap(launchContext)
  return (
    <BrowserRouter basename={basename}>
      ...
      <Routes>
        ...
        {runtimeBaseRoutes
          ...
          .map((route) => (
            <Route key={route.path} path={route.path} element={<AdminShell windowChrome={windowChrome}><LazyAdminPage>{route.element}</LazyAdminPage></AdminShell>} />
          ))}
        {runtimeRoutes.map((route) => (
          <Route key={route.path} path={route.path} element={<AdminShell windowChrome={windowChrome}>{route.element}</AdminShell>} />
        ))}
