...
import { downloadAdminCSV } from '@admin/lib/adminExport'
import {
  emptyUsageFilters,
  ...
  usageSearchParams,
  type UsageFilters,
} from '@admin/lib/adminLogQueryParams'
import { translateAPIRequestError } from '@admin/lib/apiError'
import { readListPayload, readNumberPayload, readPaginatedPayload, readRecordPayload } from '@admin/lib/listPayload'
import { cn } from '@admin/lib/utils'
import type { AICredential, PaginatedResponse, UsageLog } from '@admin/types'
...
function readUsageTotals(raw: unknown): UsageTotals {
  const record = readRecordPayload(raw)
  return {
    records: readNumberPayload(record.records),
    cost: readNumberPayload(record.cost),
    input_tokens: readNumberPayload(record.input_tokens),
    output_tokens: readNumberPayload(record.output_tokens),
    cached_input_tokens: readNumberPayload(record.cached_input_tokens),
    reasoning_tokens: readNumberPayload(record.reasoning_tokens),
    duration_sec: readNumberPayload(record.duration_sec),
    image_count: readNumberPayload(record.image_count),
  }
}
...
function readUsageSummary(raw: unknown): UsageSummary {
  const record = readRecordPayload(raw)
  const operations = readListPayload<Record<string, unknown>>(record.operations).map((row) => ({
    ...readUsageTotals(row),
    operation_type: String(readRecordPayload(row).operation_type ?? ''),
  }))
  const topModels = readListPayload<Record<string, unknown>>(record.top_models).map((row) => ({
    ...readUsageTotals(row),
    ai_model_catalog_entry: readRecordPayload(row).ai_model_catalog_entry as UsageLog['ai_model_catalog_entry'],
  }))
  const topUsers = readListPayload<Record<string, unknown>>(record.top_users).map((row) => ({
    ...readUsageTotals(row),
    user_id: readNumberPayload(readRecordPayload(row).user_id),
    user: readRecordPayload(row).user as UsageLog['user'],
  }))

  return {
    totals: readUsageTotals(record.totals),
    operations,
...
export function UsageLogsPage() {
  ...
  const logsQuery = useQuery<PaginatedResponse<UsageLog>>({
    ...
    queryFn: () => api.get('/admin/usage-logs', { params }).then((r) => readPaginatedPayload<UsageLog>(r.data, { defaultPageSize: PAGE_SIZE })),
  })
  const summaryQuery = useQuery<UsageSummary>({
    queryKey: ['admin', 'usage-logs', 'summary', summaryParams],
    queryFn: () => api.get('/admin/usage-logs/summary', { params: summaryParams }).then((r) => readUsageSummary(r.data)),
  })

  const items = logsQuery.data?.items ?? []
  const total = logsQuery.data?.total ?? 0
  const responsePageSize = logsQuery.data?.page_size ?? PAGE_SIZE
  const summary = summaryQuery.data ?? emptyUsageSummary()
