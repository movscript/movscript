...
function readNumberMap(raw: unknown): Record<string, number> {
  ...
  return Object.fromEntries(Object.entries(record).map(([key, value]) => [key, readNumberPayload(value)]))
}

function emptyVectorStats(): VectorStats {
  return {
    documents: 0,
    embedded_documents: 0,
    references: 0,
    source_references: 0,
    unindexed_references: 0,
    orphan_references: 0,
    index_coverage: 0,
    by_kind: {},
    by_locale: {},
...
function readVectorStats(raw: unknown): VectorStats {
  const record = readRecordPayload(raw)
  return {
    documents: readNumberPayload(record.documents),
    embedded_documents: readNumberPayload(record.embedded_documents),
    references: readNumberPayload(record.references),
    source_references: readNumberPayload(record.source_references),
    unindexed_references: readNumberPayload(record.unindexed_references),
    orphan_references: readNumberPayload(record.orphan_references),
    index_coverage: readNumberPayload(record.index_coverage),
    by_kind: readNumberMap(record.by_kind),
    by_locale: readNumberMap(record.by_locale),
    by_embedding_model: readNumberMap(record.by_embedding_model),
    last_updated_at: typeof record.last_updated_at === 'string' ? record.last_updated_at : undefined,
  }
}

function emptyVectorMetricsResponse(): VectorMetricsResponse {
  return {
    generated_at: '',
    summary: {
      operations: 0,
...
function readVectorSearchResponse(raw: unknown): VectorSearchResponse {
  return {
    items: readListPayload<VectorSearchResult>(raw),
  }
}

function readVectorMetricsResponse(raw: unknown): VectorMetricsResponse {
  const record = readRecordPayload(raw)
  const summary = readRecordPayload(record.summary)
  return {
    generated_at: typeof record.generated_at === 'string' ? record.generated_at : '',
    summary: {
      operations: readNumberPayload(summary.operations),
      errors: readNumberPayload(summary.errors),
      documents: readNumberPayload(summary.documents),
    },
    operations: readListPayload<VectorMetricsResponse['operations'][number]>(record.operations),
  }
}
...
export function ShotVectorPage() {
  ...
  const statsQuery = useQuery<VectorStats>({
    ...
    queryFn: () => api.get('/admin/shot-vectors/stats').then((r) => readVectorStats(r.data)),
  })
  const searchQuery = useQuery<VectorSearchResponse>({
    queryKey: ['admin', 'shot-vectors', 'search', activeQuery, locale],
    queryFn: () => api.get('/admin/shot-vectors/search', { params: { q: activeQuery, locale, top_k: 20 } }).then((r) => readVectorSearchResponse(r.data)),
    enabled: activeQuery.trim().length > 0,
  })
  const metricsQuery = useQuery<VectorMetricsResponse>({
    queryKey: ['admin', 'shot-vectors', 'metrics'],
    queryFn: () => api.get('/admin/shot-vectors/metrics').then((r) => readVectorMetricsResponse(r.data)),
  ...
  const stats = statsQuery.data ?? emptyVectorStats()
  const metrics = metricsQuery.data ?? emptyVectorMetricsResponse()
  ...
  const operationRows = metrics.operations
  ...
  return (
    <div className="mx-auto flex w-full max-w-7xl flex-col gap-5">
      ...
      <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-6">
        <MetricTile label={t('admin.shotVectors.documents')} value={stats.documents} />
        <MetricTile label={t('admin.shotVectors.embeddedDocuments')} value={stats.embedded_documents} />
        <MetricTile label={t('admin.shotVectors.sourceReferences')} value={stats.source_references} />
        <MetricTile label={t('admin.shotVectors.references')} value={stats.references} />
      ...
      <section className="grid gap-3 sm:grid-cols-3 xl:grid-cols-6">
        <MetricTile label={t('admin.shotVectors.unindexedReferences')} value={stats.unindexed_references} />
        <MetricTile label={t('admin.shotVectors.orphanReferences')} value={stats.orphan_references} />
        ...
        <MetricTile label={t('admin.shotVectors.operations')} value={metrics.summary.operations} />
        <MetricTile label={t('admin.shotVectors.errors')} value={metrics.summary.errors} />
        <MetricTile label={t('admin.shotVectors.processedDocuments')} value={metrics.summary.documents} />
      </section>
