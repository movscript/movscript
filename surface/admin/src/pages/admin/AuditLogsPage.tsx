...
import { readListPayload, readPaginatedPayload, readRecordPayload } from '@admin/lib/listPayload'
import type { AuditLog, PaginatedResponse } from '@admin/types'
...
function emptyAuditLogSummary(): AuditLogSummary {
  return {
    totals: { records: 0, unique_actors: 0 },
    top_actions: [],
    top_targets: [],
    top_actors: [],
    generated_at: '',
...
function readAuditLogSummary(raw: unknown): AuditLogSummary {
  const record = readRecordPayload(raw)
  const totals = readRecordPayload(record.totals)
  return {
    totals: {
      records: Number(totals.records) || 0,
      unique_actors: Number(totals.unique_actors) || 0,
    },
    top_actions: readListPayload<{ action: string; count: number }>(raw, ['top_actions']),
    top_targets: readListPayload<{ target_type: string; count: number }>(raw, ['top_targets']),
    top_actors: readListPayload<{ actor_id: number; count: number }>(raw, ['top_actors']),
...
export function AuditLogsPage() {
  ...
  const logsQuery = useQuery<PaginatedResponse<AuditLog>>({
    ...
    queryFn: () => api.get('/admin/audit-logs', { params }).then((r) => readPaginatedPayload<AuditLog>(r.data, { defaultPageSize: PAGE_SIZE })),
  })
  const summaryQuery = useQuery<AuditLogSummary>({
    queryKey: ['admin', 'audit-logs', 'summary', summaryParams],
    queryFn: () => api.get('/admin/audit-logs/summary', { params: summaryParams }).then((r) => readAuditLogSummary(r.data)),
  })

  const items = logsQuery.data?.items ?? []
  const total = logsQuery.data?.total ?? 0
  const responsePageSize = logsQuery.data?.page_size ?? PAGE_SIZE
  const summary = summaryQuery.data ?? emptyAuditLogSummary()
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
        <SummaryCard label={t('admin.auditLogs.summary.records')} value={summary.totals.records.toLocaleString()} detail={t('admin.auditLogs.summary.filtered')} />
        <SummaryCard label={t('admin.auditLogs.summary.uniqueActors')} value={summary.totals.unique_actors.toLocaleString()} detail={t('admin.auditLogs.actor')} />
