...
import { AppFeedbackText, AppIconFrame, AppInlineError, AppStateMessage } from '@movscript/ui/business/app'
...
import {
  ...
  updateAuthOrg,
  updateAuthOrgMember,
} from '@admin/lib/authManagementApi'
...
import {
  ...
  orgSearchParams,
  type OrgListFilters,
} from '@admin/lib/adminOrgQueryParams'
import { userListHref } from '@admin/lib/adminUserQueryParams'
import { cn } from '@admin/lib/utils'
import type { OrgInvitation, Organization, OrganizationMember, PaginatedResponse } from '@admin/types'
...
export function OrgManagementPage() {
  ...
  const createOrg = useMutation({
    mutationFn: ({ name, slug, ownerUserId }: { name: string; slug: string; ownerUserId: number }) =>
      createAuthOrg({ name, slug: slug || undefined, owner_user_id: ownerUserId }),
    onSuccess: () => {
      setError('')
  ...
  const updateMember = useMutation({
    mutationFn: ({ orgId, userId, role }: { orgId: number; userId: number; role: string }) =>
      updateAuthOrgMember(orgId, userId, { role }),
    onSuccess: (_result, variables) => {
      setError('')
  ...
  const addMember = useMutation({
    mutationFn: ({ orgId, userId, role }: { orgId: number; userId: number; role: string }) =>
      addAuthOrgMember(orgId, { user_id: userId, role }),
    onSuccess: (_result, variables) => {
      setError('')
      setAddMemberUserId('')
      qc.invalidateQueries({ queryKey: ['admin', 'orgs'] })
  ...
  const removeMember = useMutation({
    mutationFn: ({ orgId, userId }: { orgId: number; userId: number }) =>
      removeAuthOrgMember(orgId, userId),
    onSuccess: (_result, variables) => {
      setError('')
      qc.invalidateQueries({ queryKey: ['admin', 'orgs'] })
      qc.invalidateQueries({ queryKey: ['admin', 'orgs', variables.orgId, 'members'] })
