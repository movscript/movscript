...
test('projectFiltersFromSearchParams parses project list filters', () => {
  const filters = projectFiltersFromSearchParams(new URLSearchParams('q=promo&project_id=11&owner_id=7&org_id=3'))
...
test('projectSearchParams trims values and omits empty defaults', () => {
  ...
  assert.equal(params.toString(), 'q=show&project_id=11&owner_id=7&page=2')
})
...
test('projectListHref creates shareable admin project links', () => {
  ...
  assert.equal(projectListHref({ orgId: 9 }), '/projects?org_id=9')
