...
import {
  listSurfaceSemanticEntities,
  semanticEntityConfig,
  type SemanticEntityRecord,
} from '@movscript/shared/semantic-entities'
import {
  createSurfaceWorkspaceAssetSlotCandidate,
  createSurfaceWorkspaceKeyframeCandidate,
} from '@movscript/shared/workspace-candidates'
import {
...
import { assetCandidateSelectedResult, invalidateResourceMutationResult } from './features/application/resourceMutationInvalidation.js'
import { resourceCandidateKeys } from './resourceQueryKeys.js'
...
export interface ResourceCandidateAttachPanelProps {
...
export function ResourceCandidateAttachPanel({
  ...
  const { data: targetRecords = [], isFetching: loadingTargets } = useQuery({
    ...
    queryFn: () => listSurfaceSemanticEntities(projectId!, semanticEntityConfig(targetConfig.entityKind)),
...
async function attachResourceCandidate(
  ...
  if (targetType === 'keyframe') {
    return createSurfaceWorkspaceKeyframeCandidate(projectId, resourceKeyframeCandidatePayload(targetRecord, resource))
  }
  return createSurfaceWorkspaceAssetSlotCandidate(projectId, resourceAssetCandidatePayload(targetId, resource), targetRecord)
}
