import {
  ...
  type ResourceDragPayloadResource,
...
export interface ResourceDropInteractionDataTransfer extends Pick<ResourceDragDataTransfer, 'getData'> {
...
export function resourceViewportBoundaryFromWindow(viewport: { innerWidth: number; innerHeight: number }): ResourceViewportBoundary {
  return {
...
export function startResourceDragSource<TResource extends ResourceDragPayloadResource>({
  dataTransfer,
  resource,
  target,
  preventDefault,
...
export function resourceDropAcceptsPayload(dataTransfer: Pick<ResourceDropInteractionDataTransfer, 'types'>): boolean {
  return coreResourceDropAcceptsPayload(dataTransfer)
}

export function acceptResourceDropDragOver(dataTransfer: ResourceDropInteractionDataTransfer): boolean {
...
export function resolveResourceDropResource<TResource extends ResourceDragPayloadResource>({
  ...
  dataTransfer: Pick<ResourceDropInteractionDataTransfer, 'getData'>
  resources: TResource[]
}): TResource | null {
