...
import type { ExternalResourceItem, ExternalResourceSearchResult, ExternalResourceSource, RawResource } from '@movscript/shared'
...
import {
  ...
  externalResourceKey,
} from './externalResourceSearchModel'
import { externalResourceKeys } from '../../resourceQueryKeys.js'
import { invalidateResourceMutationResult, resourceLibraryChangedResult } from './resourceMutationInvalidation'
import { uploadExternalResourceItem } from './externalResourceImport'
import { useSurfaceHostState } from '../../surfaceHostStateHooks.js'

export function useExternalResourceSearchController() {
  const qc = useQueryClient()
  const onboardingCompleted = useSurfaceHostState(state => state.appSettings?.onboardingCompleted ?? false)
