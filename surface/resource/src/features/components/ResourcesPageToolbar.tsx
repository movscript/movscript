...
export function ResourcesPageToolbar({
  ...
  onScopeChange: (scope: ResourceLibraryScopeFilter) => void
  onFilterChange: (filter: ResourceLibraryTypeFilter) => void
  ...
  return (
    <>
      ...
      <ResourcePageFilterBar>
        ...
        <ResourcePageActionGroup>
          {RESOURCE_LIBRARY_SCOPE_TABS.map(tabItem => {
            if (tabItem.value === 'project' && !projectScopeEnabled) return null
        ...
        <ResourcePageActionGroup>
          {RESOURCE_LIBRARY_TYPE_TABS.map(tabItem => (
            <ResourcePageActionButton
              key={tabItem.value}
              size="xs"
