...
export async function listScriptVersions(projectId: number, params: { scriptId?: number } = {}) {
  const versions = await listSurfaceSemanticEntities(projectId, semanticEntityConfig('scriptVersions')) as unknown as ScriptVersion[]
...
export async function createScriptVersion(projectId: number, payload: CreateScriptVersionPayload) {
  ...
  return await createSurfaceSemanticEntity(projectId, semanticEntityConfig('scriptVersions'), {
    ...payload,
