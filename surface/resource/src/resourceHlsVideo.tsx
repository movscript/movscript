...
import { getResourceMediaAPIBaseURL, getResourceMediaAuthHeaders } from './resourceMediaBrowser.js'
...
function hlsRequestHeaders(url: string): HeadersInit | undefined {
  return requiresMediaStreamAuth(url) ? getResourceMediaAuthHeaders() : undefined
}
...
function normalizeHlsSource(src: string): string {
  ...
  if (trimmed.startsWith('/api/v1/')) return `${getResourceMediaAPIBaseURL()}${trimmed}`
