...
import { readSurfaceHostApi, type SurfaceHostApi } from '@movscript/shared'
...
>>>(key: K): Required<Pick<SurfaceHostApi, K>>[K] {
  const fn = readSurfaceHostApi()?.[key]
  if (!fn) throw new Error(`MovScript engine API is unavailable: ${String(key)}`)
  return fn as Required<Pick<SurfaceHostApi, K>>[K]
}
