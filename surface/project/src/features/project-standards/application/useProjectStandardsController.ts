...
import { useProjectEntrySessionStore, hasExplicitProjectEntrySearchParam } from '../../project/application/projectEntrySessionStore'
import { toast } from '@movscript/ui/toast'
import { surfaceRoutePath, type RawResource } from '@movscript/shared'
import {
...
} from './projectStandardsControllerCommands'
import { projectStandardsKeys } from './projectStandardsQueryKeys'
import {
...
} from '../presentation/projectStandardsBoardModel'
...
export function useProjectStandardsController() {
  ...
  const pageKey = useMemo(() => {
    ...
    return projectStandardsPageKey(projectId)
  }, [projectId])
  ...
  useEffect(() => {
    ...
    upsertProjectEntrySessionSnapshot({
      ...
      route: surfaceRoutePath('project.standards', { projectId }),
...
function projectStandardsPageKey(projectId: number): string {
  return [
    'page',
    surfaceRoutePath('project.standards', { projectId }),
    'project',
    projectId,
  ].join('|')
}
