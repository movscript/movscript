...
import { createScriptVersion, listScriptVersions, type ScriptVersion } from '@movscript/resource-surface/data'
...
} from '../application/scriptMutationInvalidation'
import {
  normalizeComparableScriptText,
  scriptVersionSourceText,
  scriptWorkspaceSourceText,
} from '../presentation/scriptDisplayModel'
import { surfaceRoutePath, surfaceWorkspaceOwnerContext } from '@movscript/shared'
...
function ScriptsSection({ projectId }: { projectId: number }) {
  ...
  useEffect(() => {
    ...
    upsertProjectEntrySessionSnapshot({
      ...
      route: surfaceRoutePath('project.scripts', { projectId }),
              ...
              <ScriptWorkspaceEmptySelection
                ...
                action={(
                  <Button asChild variant="outline" size="sm">
                    <Link to={surfaceRoutePath('project.home', { projectId })}>返回项目 Home</Link>
                  </Button>
                )}
              />
            ) : (
...
export default function ScriptsPage() {
  const projectId = useSurfaceHostState((state) => state.currentProject?.ID)
