import type { SemanticEntityRecord, WorkspaceArtifact } from '../types.js'
import {
  CORE_STANDARD_DEFS,
  coreStandardText,
...
} from './projectStandardsPromptRules.js'

export * from './projectStandardsPromptRules.js'
...
export function projectStandardsWorkspaceDataFromRecords(input: Partial<WorkspaceData>): WorkspaceData {
  return {
    project: input.project ?? null,
    productions: input.productions ?? [],
    settings: input.settings ?? [],
    creativeRelationships: input.creativeRelationships ?? [],
    settingUsages: input.settingUsages ?? [],
    assetSlots: input.assetSlots ?? [],
    assetSlotCandidates: input.assetSlotCandidates ?? [],
    segments: input.segments ?? [],
    sceneMoments: input.sceneMoments ?? [],
