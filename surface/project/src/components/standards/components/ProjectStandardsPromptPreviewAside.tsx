...
} from './ProjectStandardsUi.js'
import type { ProjectPromptRule } from '../application/projectStandardsModel.js'
import type { RawResource } from '../types.js'
            ...
            {styleReferenceIds.map((id) => {
              ...
              return (
                <ProjectStandardsImageCard key={id}>
                  <ProjectStandardsImageFrame>
                    <ProjectStandardsResourceImage resource={uploaded} resourceId={id} />
                    <ProjectStandardsIconButton
                      size="icon-sm"
                      variant="ghost"
                      tone="danger"
...
function ProjectStandardsResourceImage({ resource, resourceId }: { resource?: RawResource; resourceId: number }) {
  const src = typeof resource?.url === 'string'
    ? resource.url
    : typeof resource?.file_url === 'string'
      ? resource.file_url
      : typeof resource?.thumbnail_url === 'string'
        ? resource.thumbnail_url
        : undefined
  const alt = resource?.name ?? resource?.title ?? `resource#${resourceId}`
  if (!src) {
    return (
      <div className="flex h-full w-full items-center justify-center bg-muted text-muted-foreground type-tiny">
        #{resourceId}
      </div>
    )
  }
