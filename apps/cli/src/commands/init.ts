...
export async function cmdInit(name: string | undefined, options: InitOptions) {
  ...
  writeFileSync(join(dir, 'src', 'index.ts'), [
    ...
    ``,
    `export async function run(host: MovPluginHost, args: Record<string, unknown>): Promise<PluginRunResult> {`,
  ...
  if (options.webview) {
    writeFileSync(join(dir, 'src', 'ui.tsx'), [
      ...
      `declare const window: Window & { mov: import('@movscript/plugin-sdk').MovPluginHost }`,
      ``,
      `async function main() {`,
