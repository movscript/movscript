...
func New(db *gorm.DB, cfg *config.Config, store storage.Storage) *gin.Engine {
	...
	v1 := r.Group("/api/v1")
	{
		...
		protected := v1.Group("", middleware.RequireAuth())
		{
			...
			protected.GET("/projects/:id/v2/sceneMoments", v2Semantics.ListSceneMoments)
			protected.POST("/projects/:id/v2/sceneMoments", v2Semantics.CreateSceneMoment)
			protected.PATCH("/projects/:id/v2/sceneMoments/:sceneMomentId", v2Semantics.PatchSceneMoment)
			protected.DELETE("/projects/:id/v2/sceneMoments/:sceneMomentId", func(c *gin.Context) { v2Semantics.DeleteV2Item(c, &model.SceneMoment{}, c.Param("sceneMomentId")) })
