...
func New(db *gorm.DB, cfg *config.Config, store storage.Storage) *gin.Engine {
	...
	previewH := handler.NewPreviewHandler(db)
	...
	v1 := r.Group("/api/v1")
	{
		...
		protected := v1.Group("", middleware.RequireAuth())
		{
			...
			protected.DELETE("/projects/:id/entities/work-items/:workItemId", semanticEntities.DeleteWorkItem)
			...
			protected.DELETE("/projects/:id/entities/work-reviews/:reviewId", semanticEntities.DeleteWorkReview)
			...
			protected.DELETE("/projects/:id/entities/work-dependencies/:dependencyId", semanticEntities.DeleteWorkDependency)
