...
func New(db *gorm.DB, cfg *config.Config, store storage.Storage) *gin.Engine {
	...
	v1 := r.Group("/api/v1")
	{
		...
		protected := v1.Group("", middleware.RequireAuth())
		{
			...
			protected.PATCH("/projects/:id/v2/asset-requirements/:requirementId", v2Semantics.PatchAssetRequirement)
			protected.DELETE("/projects/:id/v2/asset-requirements/:requirementId", func(c *gin.Context) { v2Semantics.DeleteV2Item(c, &model.AssetRequirement{}, c.Param("requirementId")) })
