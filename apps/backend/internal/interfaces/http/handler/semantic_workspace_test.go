...
func TestPreviewProductionWorkspaceRejectsLegacyActionPayload(t *testing.T) {
	gin.SetMode(gin.TestMode)
	handler := NewSemanticEntityHandler(newSemanticWorkspaceHandlerTestDB(t))
	router := gin.New()
	router.POST("/projects/:id/entities/production-workspaces/apply-preview", handler.PreviewProductionWorkspaceApply)

	req := httptest.NewRequest(http.MethodPost, "/projects/1/entities/production-workspaces/apply-preview", strings.NewReader(`{
		"mode":"snapshot",
		"production_id":1,
		"workspace":{"segments":[{"action":"create","title":"Opening","scene_moments":[]}]}
	}`))
...
func TestPreviewProductionWorkspaceReturnsDebugForMissingCreativeReference(t *testing.T) {
	gin.SetMode(gin.TestMode)
	db := newSemanticWorkspaceHandlerTestDB(t)
	...
	if debug["path"] != "/workspace/segments/0/scene_moments/0/creative_references/0/id" {
...
func newSemanticWorkspaceHandlerTestDB(t *testing.T) *gorm.DB {
	t.Helper()
	return testutil.OpenSQLiteWithConfig(t, "semantic_workspace_handler.db", &gorm.Config{
