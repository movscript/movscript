...
func (h *WorkflowMarketHandler) Publish(c *gin.Context) {
	...
	cv, err := h.service.Publish(c.Request.Context(), parseID(c.Param("id")), user.ID, workflowmarket.PublishInput{
...
func (h *WorkflowMarketHandler) Unpublish(c *gin.Context) {
	...
	cv, err := h.service.Unpublish(c.Request.Context(), parseID(c.Param("id")), user.ID)
	if err != nil {
		h.writeWorkflowMarketError(c, err)
		return
	}
...
func (h *WorkflowMarketHandler) Clone(c *gin.Context) {
	...
	cv, err := h.service.Clone(c.Request.Context(), parseID(c.Param("id")), user.ID, workflowmarket.CloneInput{
		Name:      req.Name,
		ProjectID: req.ProjectID,
		Stage:     req.Stage,
	})
