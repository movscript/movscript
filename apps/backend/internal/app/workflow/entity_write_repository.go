...
func buildEntityWriteAudits(
	...
	bindingIDsByPort map[string][]uint,
	meta EntityWriteMeta,
) []canvasdomain.EntityWriteAudit {
	audits := make([]canvasdomain.EntityWriteAudit, 0, len(values))
	for portID, value := range values {
		...
		audits = append(audits, canvasdomain.NewEntityWriteAudit(canvasdomain.EntityWriteAuditSpec{
