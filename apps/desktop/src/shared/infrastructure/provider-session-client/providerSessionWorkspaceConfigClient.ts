...
export async function saveProviderSessionConfigFile(
  ...
  const agentCatalog = nextAgentCatalogConfig(config, {
    configFiles: upsertConfigFile(providerCatalogConfigFiles(config.agentCatalog?.configFiles), input.configFile),
  ...
  return runtimeResult ?? runtimeConfigFileSaveResult(
    input.configFile,
    providerCatalogConfigFiles(savedConfig.agentCatalog?.configFiles ?? agentCatalog.configFiles),
    savedConfig.agentCatalog?.activeConfigFileId,
  )
}
...
export async function deleteProviderSessionConfigFile(
  ...
  const config = await context.getWorkspaceConfig()
  const configFiles = providerCatalogConfigFiles(config.agentCatalog?.configFiles).filter((configFile) => configFile.id !== input.configFileId)
  ...
  return runtimeResult ?? runtimeConfigFileDeleteResult(
    providerCatalogConfigFiles(savedConfig.agentCatalog?.configFiles ?? agentCatalog.configFiles),
    savedConfig.agentCatalog?.activeConfigFileId,
  )
}
...
function mergeProviderCatalogInspectWithWorkspaceConfig(
  ...
  for (const configFile of providerCatalogConfigFiles(agentCatalog.configFiles)) configFilesById.set(configFile.id, configFile)
...
function providerCatalogConfigFiles(configFiles: unknown): ProviderCatalogConfigFile[] {
  if (!Array.isArray(configFiles)) return []
  return configFiles.filter(isProviderCatalogConfigFile)
}
...
function isProviderCatalogConfigFile(value: unknown): value is ProviderCatalogConfigFile {
  ...
  if (!stringField(value.id) || !stringField(value.version) || !stringField(value.name)) return false
  if (!stringArray(value.enabledPackIds) || !stringArray(value.skillIds)) return false
  if (!Array.isArray(value.toolGrants) || !value.toolGrants.every(isProviderToolGrant)) return false
  return true
}

function isProviderToolGrant(value: unknown): value is ProviderCatalogConfigFile['toolGrants'][number] {
  if (!isRecord(value)) return false
  const approval = value.approval
  return stringField(value.name) !== undefined
    && (value.mode === 'allow' || value.mode === 'deny')
    && (approval === undefined || approval === 'never' || approval === 'always' || approval === 'on_write')
}
