...
import { removeProjectCaches } from '@movscript/project-surface/data'
    ...
    <Dialog open onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="max-w-sm">
        ...
        <div className="space-y-3 py-2">
          ...
          {error && <OrganizationStatusMessage tone="danger">{error}</OrganizationStatusMessage>}
        </div>
        <DialogFooter>
  ...
  return (
    <div className="w-full max-w-3xl">
        <div className="mb-7">
          ...
          <h1 className="type-title font-bold text-foreground mb-1">{t('org.selectTitle')}</h1>
          <p className="type-body text-muted-foreground">{t('org.selectSubtitle')}</p>
        </div>

        {currentMembership && (
          <OrganizationSelectCurrentCard
            icon={currentMembership.is_personal ? (
              <CircleUserRound size={17} className="text-muted-foreground" />
            ) : (
              <Building2 size={17} className="text-muted-foreground" />
            )}
            eyebrow={t('org.currentWorkspace')}
            title={currentMembership.org_name}
            role={roleLabel(currentMembership.role, t)}
            action={isCurrentOrgAdmin ? (
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          <OrganizationSelectActionTile
            ...
            title={t('org.createNew')}
            description={t('org.createOptionDescription')}
            onClick={() => setShowCreate(true)}
          />

          <OrganizationSelectActionTile
            icon={<KeyRound size={16} className="text-muted-foreground" />}
            title={t('org.joinWithCode')}
            description={t('org.joinOptionDescription')}
        ...
        {isCurrentOrgAdmin && (
          <section id="workspace-settings" className="mt-8 scroll-mt-6">
            <div className="mb-4">
              <h2 className="type-title font-semibold text-foreground">{t('org.settingsTitle')}</h2>
