...
test('macOS dock shortcuts mirror desktop root and recent jump entries', () => {
  ...
  assert.match(shortcutCommandsSource, /openToolWindow\(\{ route: ROUTES\.tools\.refImageGen \}\)/)
...
test('account settings page layout is owned by named shell styles', () => {
  ...
  assert.match(settingsSource, /desktopEmbeddedAgentEnabled\(\)/)
  assert.match(settingsSource, /visibleEnvironmentLinks[\s\S]*agentConsoleEnvironmentLinks\.filter\(\(link\) => link\.id === 'plugins'\)/)
  assert.match(settingsSource, /visibleEnvironmentLinks[\s\S]*\.map\(\(link\) =>/)
