...
test('macOS dock shortcuts mirror desktop root and recent jump entries', () => {
  ...
  assert.doesNotMatch(shortcutCommandsSource, /navigate\(ROUTES\.tools\.refImageGen\)/)
...
test('app window header exposes semantic icon regions and left control fill layout', () => {
  ...
  const projectEntryDeckHeaderSource = readFileSync(resolve('../../surface/project/src/features/project/components/ProjectEntryDeckHeader.tsx'), 'utf8')
  const projectEntryDeckHeaderStyles = readFileSync(resolve('../../surface/project/src/features/project/components/ProjectEntryDeckHeader.css'), 'utf8')
  ...
  assert.match(appSource, /<ProjectEntryDeckHeader[\s\S]*activeEntryId=\{routeLayout\.projectEntryId\}[\s\S]*activeSearch=\{search\}[\s\S]*projectId=\{currentProject\.ID\}[\s\S]*projectName=\{currentProject\.name\}/)
  assert.match(projectEntryDeckHeaderSource, /projectEntrySessionIdForRouteSearch\(activeEntryId, activeSearch\)/)
  assert.match(projectEntryDeckHeaderSource, /buildProjectEntryDeck\(\{[\s\S]*activeEntryId: resolvedActiveEntryId,[\s\S]*projectId,[\s\S]*snapshots,[\s\S]*\}\)/)
  ...
  assert.match(projectEntryDeckHeaderSource, /setEntryDeckOrders\(projectId, updates\)/)
  assert.match(projectEntryDeckHeaderSource, /projectEntryDeckTabPath\(tab\)/)
  assert.match(projectEntryDeckHeaderStyles, /\.project-entry-deck-header \{[\s\S]*max-width: min\(720px, 68vw\);/)
