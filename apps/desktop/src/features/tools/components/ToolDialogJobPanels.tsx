...
function DebugPanel({ job }: { job: Job }) {
  ...
  return (
    <ToolDialogDebugPanel>
      ...
      {debug?.resource_diagnostics?.length ? (
        <Section title={t('tools.debug.resourceDecisions', { defaultValue: 'Resource decisions' })}>
          <div className="space-y-2">
            {debug.resource_diagnostics.map((diagnostic) => (
              <div key={diagnostic.resource_id} className="rounded-md border border-border/70 p-2">
                <KV label="Resource" value={`#${diagnostic.resource_id}${diagnostic.resource_type ? ` · ${diagnostic.resource_type}` : ''}`} />
                <KV label="Mode" value={diagnostic.mode || '-'} color={diagnostic.mode === 'provider_asset_uri' ? 'success' : undefined} />
                <KV label="Reason" value={diagnostic.reason || '-'} mono={false} />
                <KV label="Provider" value={diagnostic.provider_id || '-'} />
                {diagnostic.asset_uri ? <KV label="Asset URI" value={diagnostic.asset_uri} /> : null}
                {diagnostic.next_action ? <KV label="Next action" value={diagnostic.next_action} mono={false} /> : null}
              </div>
            ))}
          </div>
        </Section>
