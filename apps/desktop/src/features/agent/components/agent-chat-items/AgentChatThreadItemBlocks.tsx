...
import {
  ...
  type AgentChatToolSurfaceView,
  type AgentChatContentKind,
} from '@movscript/agent-chat'
import { ResourceFileAudio } from '@movscript/resource-surface/resource-media-components'
import { ResourceFileImage } from '@movscript/resource-surface/resource-media-components'
import { ResourceFileVideo } from '@movscript/resource-surface/resource-media-components'
...
export function AgentChatSectionTitle({ title, meta }: { title: string; meta?: Array<string | undefined | null | false> }) {
  return (
    <div className="agent-chat-section-title">
      <span className="agent-chat-section-title-text">{title}</span>
...
function AgentChatItemMeta({ values }: { values: Array<string | undefined | null | false> }) {
  ...
  return (
    <span className="agent-chat-meta-list">
...
export function AgentChatInlineList({ label, values }: { label: string; values: string[] }) {
  ...
  return (
    <AgentMessageSection title={label} tone="process" defaultOpen={agentChatListDefaultOpen(compactValues.length)}>
      <div className="agent-chat-inline-list">
        {compactValues.map((value, index) => (
          <div key={`${index}:${value}`} className="agent-chat-inline-list-item">{value}</div>
        ))}
      </div>
    </AgentMessageSection>
  )
...
export function AgentChatSurfaceLinks({
  label = 'Open',
  surfaces,
}: {
  label?: string
  surfaces: AgentChatToolSurfaceView[]
}) {
  const visibleSurfaces = surfaces.filter((surface) => surface.url.trim())
  if (visibleSurfaces.length === 0) return null
  return (
    <AgentMessageSection title={label} tone="result" defaultOpen>
      <div className="agent-chat-surface-links">
        {visibleSurfaces.map((surface) => (
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="agent-chat-surface-link"
            title={surface.description ?? surface.usage ?? surface.route ?? surface.url}
          >
            <span className="agent-chat-surface-link__title">{surface.title}</span>
            {surface.intent ? <span className="agent-chat-surface-link__meta">{surface.intent}</span> : null}
          </a>
        ))}
      </div>
    </AgentMessageSection>
...
export function AgentChatImagePreviewGrid({
  ...
  return (
    <AgentMessageSection title={label} tone="process" defaultOpen>
      <div className="agent-chat-media-grid" data-kind="image">
        {renderedImages.map((image, index) => (
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="agent-chat-media-tile"
          >
            <ResourceFileImage
              ...
              className="agent-chat-media-tile__image"
            />
          </a>
        ))}
      </div>
...
export function AgentChatMediaPreviewGrid({
  ...
  return (
    <AgentMessageSection title={label} tone="process" defaultOpen>
      <div className="agent-chat-media-grid" data-kind="media">
        {renderedMedia.map((item, index) => (
          <div key={`${index}:${item.url}`} className="agent-chat-media-tile">
            {item.kind === 'video' ? (
              <ResourceFileVideo
                ...
                playsInline
                preload="metadata"
                className="agent-chat-media-tile__video"
              />
            ) : (
              <ResourceFileAudio
                resourceUrl={item.url}
                ...
                controls
                preload="metadata"
                className="agent-chat-media-tile__audio"
              />
            )}
            <div className="agent-chat-media-tile__caption" title={item.mimeType ? `${item.label} ${item.mimeType}` : item.label}>
  ...
  return (
    <AgentMessageSection title={label} tone={tone} defaultOpen={agentChatContentDefaultOpen(contentKind, value)}>
      <AgentChatResultStack>
        <pre className="agent-chat-pre">{agentChatValuePreview(value)}</pre>
      </AgentChatResultStack>
    </AgentMessageSection>
  )
}
...
export function AgentChatInspectBlock({
  ...
  return (
    <AgentMessageSection
      ...
      defaultOpen={false}
    >
      <div className="agent-chat-inspect" data-testid="agent-chat-inspect">
        <div className="agent-chat-inspect-toolbar">
        ...
        <AgentChatResultStack>
          {visibleEntries.map((entry) => (
            <div key={entry.label} className="agent-chat-inspect-entry" data-tone={entry.tone ?? 'neutral'}>
              <div className="agent-chat-inspect-label">{entry.label}</div>
              <pre className="agent-chat-pre">{agentChatValuePreview(entry.value, 8000)}</pre>
            </div>
          ))}
        </AgentChatResultStack>
      </div>
...
export function AgentChatTextBlock({
  ...
  return (
    <AgentMessageSection title={label} tone={tone} defaultOpen={agentChatContentDefaultOpen(contentKind, value)}>
      <pre className="agent-chat-pre">{value}</pre>
    </AgentMessageSection>
  )
}
