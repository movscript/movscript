...
test('agent chat active thread state is owned by the session registry', () => {
  ...
  const neutralRuntimeSource = readFileSync(resolve('../../packages/agent-chat/src/chat/agentChatRuntime.ts'), 'utf8')
...
test('agent chat queued composer inputs stay editable until sent or steered', () => {
  ...
  const queuedInputSource = readFileSync(resolve('../../packages/agent-chat/src/chat/agentChatQueuedInputs.ts'), 'utf8')

  assert.match(coreIndexSource, /export \* from '@movscript\/agent-chat'/)
  assert.doesNotMatch(coreIndexSource, /export \* from '\.\/agentChatQueuedInputs\.js'/)
...
test('agent chat goal state flows from protocol to composer UI', () => {
  const uiProtocolSource = readFileSync(resolve('../../packages/agent-chat/src/chat/agentChatProtocol.ts'), 'utf8')
  const goalStateSource = readFileSync(resolve('../../packages/agent-chat/src/chat/agentChatGoalState.ts'), 'utf8')
  const dispatcherSource = readFileSync(resolve('../../packages/agent-chat/src/chat/agentChatNotificationDispatcher.ts'), 'utf8')
  const dataSourceShellSource = readAgentChatDataSourceShellContractSource()
  const shellViewSource = readFileSync(resolve('src/features/agent/components/AgentChatShellView.tsx'), 'utf8')
  const composerSource = readFileSync(resolve('src/features/agent/components/AgentComposerSection.tsx'), 'utf8')
  const queuedInputPreviewSource = readFileSync(resolve('src/features/agent/components/AgentQueuedInputPreview.tsx'), 'utf8')
...
test('agent chat queued input summaries prefer text then attachments', async () => {
  const { agentChatQueuedInputSummary } = await import('@movscript/agent-chat')
