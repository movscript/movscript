...
import type { AgentTaskGraphSnapshot, AgentRun, AgentTask, DispatchTaskGraphResult, UpdateTaskGraphResult } from '@movscript/agent-protocol'
...
export async function dispatchTaskGraphAction(input: {
  ...
  try {
    ...
    deps.reportError(`计划调度失败：${errorMessage(error)}`)
    return false
  } finally {
    deps.setBusy(false)
  }
...
export async function replanPlanAction(input: {
  ...
  try {
    ...
    deps.reportError(`计划重规划失败：${errorMessage(error)}`)
    return false
  } finally {
    deps.setBusy(false)
  }
...
export async function acceptPlanTaskReviewAction(input: {
  ...
  try {
    ...
    deps.reportError(`验收任务失败：${errorMessage(error)}`)
    return false
  } finally {
    deps.setBusy(false)
  }
...
export async function rejectPlanTaskReviewAction(input: {
  ...
  try {
    ...
    deps.reportError(`拒绝任务失败：${errorMessage(error)}`)
    return false
  } finally {
    deps.setBusy(false)
  }
...
export async function reworkPlanTaskReviewAction(input: {
  ...
  try {
    ...
    deps.reportError(`返工任务失败：${errorMessage(error)}`)
    return false
  } finally {
    deps.setBusy(false)
  }
...
export async function cancelPlanTreeAction(input: {
  ...
  try {
    ...
    deps.reportError(`取消计划树失败：${errorMessage(error)}`)
    return false
  } finally {
    deps.setBusy(false)
  }
