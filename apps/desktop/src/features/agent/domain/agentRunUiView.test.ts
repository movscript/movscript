...
test('run trace labels localize categories and statuses', () => {
  ...
  assert.equal(approvalImpactLabel({ toolName: 'mcp__movscript__generation_submit', permission: 'generation.create', risk: 'generate', preview: undefined }), '批准后会按所选能力提交生成任务；content-unit 图像/视频成功后会写入候选，可能消耗生成额度。')
