...
import type { AgentTraceEvent } from '@movscript/agent-protocol'
...
test('run trace labels localize categories and statuses', () => {
  ...
  assert.equal(approvalImpactLabel({ toolName: 'mcp__movscript__system_generate_content_unit_image', permission: 'generation.create', risk: 'generate', preview: undefined }), '批准后会编译创作片段提示词并提交图像生成任务；成功后会写入候选，可能消耗生成额度。')
