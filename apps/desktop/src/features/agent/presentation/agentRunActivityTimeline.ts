import { generationStatusText } from '@/features/agent/domain/agentGenerationDisplay'
...
export interface AgentRunActivityTimeline {
  ...
  actionCount: number
  eventCount: number
  items: AgentRunActivityTimelineItem[]
}

export function buildAgentRunActivityTimeline(input: {
  activity?: ChatRunActivity
  run?: AgentRun | null
  events?: ChatRunActivityEvent[]
}): AgentRunActivityTimeline | undefined {
  const snapshot = buildRunActivitySnapshot(input)
  if (!snapshot) return undefined
  const { activity } = snapshot

  const approvalItems: AgentRunActivityTimelineItem[] = (activity.approvals ?? []).map((approval) => ({
  ...
  const inputItems: AgentRunActivityTimelineItem[] = (activity.inputs ?? []).map((request) => ({
    id: `input-${request.id}`,
    type: 'input_request',
    kind: '用户输入',
    title: request.title,
  ...
  const stepItems: AgentRunActivityTimelineItem[] = activity.steps.map((step) => ({
    id: `step-${step.id}`,
    type: step.type === 'tool_call' ? 'tool_call' : 'message',
    kind: step.type === 'tool_call' ? '工具调用' : '消息',
    title: step.toolName ? toolCallTimelineTitle(step.toolName) : step.title ?? (step.type === 'tool_call' ? '工具调用' : '历史消息'),
    status: step.status,
...
export function agentRunActivityTimelineSummary(activityTimeline: AgentRunActivityTimeline): string {
  const actionSummary = activityTimeline.actionCount > 0 ? `${activityTimeline.actionCount} 个交互` : ''
  if (activityTimeline.stepCount > 0) return [`${activityTimeline.completedStepCount}/${activityTimeline.stepCount} 个步骤`, actionSummary].filter(Boolean).join(' · ')
  if (actionSummary) return actionSummary
  return activityTimeline.eventCount > 0 ? `${activityTimeline.eventCount} 个事件` : '暂无工具调用'
}
...
export function formatToolCallStreamDetail(event: ChatRunActivityEvent) {
  ...
  return {
    label: toolCallTimelineTitle(name ?? id ?? 'tool'),
    parseStatus,
    args,
    parsedArgs,
  }
...
function eventTimelineItem(event: ChatRunActivityEvent, runId: string): AgentRunActivityTimelineItem {
  ...
  return {
    ...
    result: generationTrace ? generationTrace.generation : undefined,
