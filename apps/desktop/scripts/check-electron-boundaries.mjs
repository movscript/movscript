...
export function checkElectronBoundaries(frontendRoot) {
  ...
  for (const failure of checkDesktopProjectServiceBoundaries(frontendRoot)) {
    failures.push(failure)
  }

  for (const failure of checkDesktopMediaPipelineDataServiceBoundaries(frontendRoot)) {
    failures.push(failure)
  }
...
function checkDesktopProjectServiceBoundaries(frontendRoot) {
  const failures = []
  const localProjectPath = resolve(frontendRoot, 'electron/services/localProject.ts')
  if (!existsSync(localProjectPath)) return failures
  const source = readFileSync(localProjectPath, 'utf8')
  if (source.includes('createNodeMovScriptWorkspaceService') || source.includes('@movscript/workspace/node')) {
    failures.push('Desktop local project lifecycle must use Project Service, not @movscript/workspace/node')
  }
  if (!source.includes('createProjectServiceClientFromRuntime') || !source.includes('lifecycleCommand')) {
    failures.push('Desktop local project lifecycle must call ProjectServiceClient.lifecycleCommand')
  }
  return failures
}

function checkDesktopMediaPipelineDataServiceBoundaries(frontendRoot) {
  const failures = []
  const mediaPipelinePaths = [
    'electron/services/mediaPipeline/assetMaterializer.ts',
    'electron/services/mediaPipeline/exportUploader.ts',
  ]
  for (const rel of mediaPipelinePaths) {
    const filePath = resolve(frontendRoot, rel)
    if (!existsSync(filePath)) continue
    const source = readFileSync(filePath, 'utf8')
    if (
      source.includes('@movscript/core/backend/node')
      || source.includes('backendGetBinary')
      || source.includes('backendPostMultipart')
    ) {
      failures.push(`Desktop media pipeline resource IO must use Data Service client, not core backend helpers: ${rel}`)
    }
    if (!source.includes('@movscript/data-client')) {
      failures.push(`Desktop media pipeline resource IO must import @movscript/data-client: ${rel}`)
    }
  }
  return failures
}
