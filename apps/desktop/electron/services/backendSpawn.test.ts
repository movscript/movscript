...
test('backend spawn env defaults to local dependency providers', () => {
  ...
  assert.equal(env.GIT_PROXY_TOKEN_SECRET, 'secret')
...
test('backend spawn env preserves explicit gin mode', () => {
  const env = buildBackendSpawnEnv({
    ...
    inheritedEnv: {
      GIN_MODE: 'debug',
      GIT_PROXY_TOKEN_SECRET: 'explicit-git-token-secret',
    },
  })

  assert.equal(env.GIN_MODE, 'debug')
  assert.equal(env.GIT_PROXY_TOKEN_SECRET, 'explicit-git-token-secret')
})
