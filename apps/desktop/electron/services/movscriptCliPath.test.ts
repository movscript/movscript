...
test('resolveMovScriptCliBinDir uses packaged plugin CLI resources when available', () => {
  const resourcesPath = resolve('/Applications/Movscript.app/Contents/Resources')
  const pluginDir = resolve(resourcesPath, 'provider-plugins/movscript')
...
test('resolveMovScriptCliBinDir prefers workspace bin before packaged resources', () => {
  ...
  const resourcesPath = resolve('/Applications/Movscript.app/Contents/Resources')
  const packagedBinDir = resolve(resourcesPath, 'provider-plugins/movscript/bin')
  assert.equal(resolveMovScriptCliBinDir({
    workspaceDir,
    resourcesPath,
    platform: 'darwin',
...
test('resolveMovScriptCliBinDir finds repository plugin cli bin in development', () => {
  const repo = resolve('/repo/movscript')
  const binDir = resolve(repo, 'apps/plugin/bin')
  assert.equal(resolveMovScriptCliBinDir({
...
test('ensureWorkspaceMovScriptCliBin writes a workspace shim that points at packaged plugin CLI', () => {
