...
import { resolveMovScriptDataServiceSession, type MovScriptDataServiceSession } from '@movscript/data-client'
...
export type AgentRuntimeAccountSource =
  ...
  | 'movscript-data-service-session'
  | 'movscript-local-data-service'
...
export function resolveAgentRuntimeAccountConfig(input: {
  ...
  const backendSession = resolveMovScriptDataServiceSession({ workspaceDir: input.workspaceDir })
  const explicitBackendProviderSelected = usesBackendProvider(workspaceConfig, providerKey)
  const savedOrResolvedAccount = credentialMode === 'none'
    ? resolvedAccount
    : resolveSavedRuntimeApiKeyAccount(input, resolvedAccount, providerKey, providerKind, runtimeApi)
  const backendProviderSelected = explicitBackendProviderSelected
    || shouldPreferBackendSession({
      config: workspaceConfig,
      providerKey,
      resolvedAccount: savedOrResolvedAccount,
      backendSession,
      preferBackendSession: input.preferBackendSession,
    })
  const account = backendProviderSelected
    ? resolveMovScriptDataServiceRuntimeAccount(savedOrResolvedAccount, backendSession)
    : savedOrResolvedAccount
...
function shouldPreferBackendSession(input: {
  ...
  backendSession: MovScriptDataServiceSession
  preferBackendSession?: boolean
}): boolean {
  ...
  if (input.resolvedAccount.source === 'movscript-app-settings') {
  ...
  return Boolean(input.backendSession.token) || isLocalDataServiceSession(input.backendSession)
}

function resolveMovScriptDataServiceRuntimeAccount(
  account: RuntimeMaterializedAccount,
  backendSession: MovScriptDataServiceSession | undefined,
): RuntimeMaterializedAccount {
  if (account.kind === 'apiKey' && (account.apiKey.startsWith('mgw_') || account.apiKey.startsWith('mv1.'))) return account
  if (backendSession?.token) return { kind: 'apiKey', apiKey: backendSession.token, source: 'movscript-data-service-session' }
  if (backendSession && isLocalDataServiceSession(backendSession)) {
    return { kind: 'apiKey', apiKey: 'movscript-local-owner', source: 'movscript-local-data-service' }
  }
  return { kind: 'none', source: 'none' }
}

function isLocalDataServiceSession(session: MovScriptDataServiceSession): boolean {
  if (session.realm?.kind === 'local') return true
  return isLocalBaseURL(session.baseURL)
}
