...
test('Electron runtime config prefers daemon gateway endpoint and keeps data service discovery internal', () => {
  ...
  assert.match(source, /findRuntimeApp\(readRuntimeHomeSnapshot\(movScriptHomeDir\), LOCAL_NODE_RUNTIME_OWNER\)/)
  assert.match(source, /function resolveRuntimeIdentity/)
  assert.match(source, /identity: runtimeIdentity/)
  assert.match(source, /gatewayBaseURL/)
  assert.match(source, /findRuntimeEndpoint\(snapshot, LOCAL_NODE_GATEWAY_SERVICE\)/)
  assert.match(source, /function resolveRuntimeConnection/, 'runtime config must resolve a single runtime connection descriptor')
  assert.match(source, /if \(input\.mode === 'local'\)[\s\S]*?gatewayBaseURL[\s\S]*?LOCAL_BACKEND_URL/, 'local runtime connection must resolve to daemon gateway or stable local backend')
  assert.doesNotMatch(source, /shouldPreferLocalBackend && input\.dataServiceBaseURL/, 'renderer API base must not fall back to direct Data Service endpoint')
