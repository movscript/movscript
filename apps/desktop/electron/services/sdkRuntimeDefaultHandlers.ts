...
function sdkRuntimeCredentialProbe(
  ...
  if (api === 'claude-sdk') {
    return sdkRuntimeCredentialProbeForAccount({
      ...
      missingDetail: 'Claude Agent SDK credentials are missing. Configure the local Data Service model gateway, set ANTHROPIC_API_KEY in the Movscript launch environment, or save a Claude API key in the Agent console.',
    })
  }
  return sdkRuntimeCredentialProbeForAccount({
    account,
    ...
    missingDetail: `${api === 'mova-sdk' ? 'Mova SDK' : 'Codex SDK'} credentials are missing. Configure the local Data Service model gateway, or set OPENAI_API_KEY in the Movscript launch environment for direct OpenAI.`,
  })
}
...
function normalizeCodexLikeSdkRuntimeError(error: unknown, api: CodexLikeRuntimeApi): Error {
  ...
  if (/401|unauthorized|missing bearer|missing .*authentication/i.test(message)) {
    ...
    return new Error(`${label} credentials are missing. Movscript SDK mode can use the local Data Service model gateway without login, or set OPENAI_API_KEY in the Movscript launch environment for direct OpenAI.`)
  }
  return error instanceof Error ? error : new Error(message)
}
