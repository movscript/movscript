...
test('Codex SDK runtime handler resolves workspace context cwd and CODEX_HOME', async () => {
  ...
  writeMovScriptDataServiceAuth(workspaceDir, { token: 'mv1.local-session', userId: 1, realm: { kind: 'local', id: 'local' } })
...
test('Codex SDK runtime handler injects data service model gateway base URL and API key', async () => {
...
test('Codex SDK runtime handler defaults to the workspace data service session', async () => {
  const workspaceDir = mkdtempSync(join(tmpdir(), 'movscript-codex-sdk-default-backend-'))
  writeMovScriptDataServiceConfig(workspaceDir, { baseURL: 'http://localhost:8766' })
  writeMovScriptDataServiceAuth(workspaceDir, { token: 'mv1.workspace-backend-token' })
  const constructed: unknown[] = []
  class Codex {
    constructor(options?: unknown) {
      constructed.push(options)
...
test('Codex SDK runtime handler can use local data service model gateway without login token', async () => {
  const workspaceDir = mkdtempSync(join(tmpdir(), 'movscript-codex-sdk-local-data-service-no-login-'))
  writeMovScriptDataServiceConfig(workspaceDir, { baseURL: 'http://localhost:8766', realm: { kind: 'local', id: 'local' } })
  writeMovScriptWorkspaceConfig(resolveMovScriptWorkspacePaths(workspaceDir).configPath, {
    schema: MOVSCRIPT_WORKSPACE_CONFIG_SCHEMA,
    updatedAt: '2026-06-18T00:00:00.000Z',
    providers: {
  ...
  assert.equal(probe.credentials?.source, 'movscript-local-data-service')
  ...
  assert.equal(options.baseUrl, 'http://127.0.0.1:8766/v1')
  assert.equal(options.apiKey, 'movscript-local-owner')
  assert.equal(options.env?.OPENAI_API_KEY, 'movscript-local-owner')
})
...
test('Codex SDK runtime handler reports missing API key credentials with an actionable message', async () => {
  ...
  await assert.rejects(
    ...
    /Codex SDK credentials are missing[\s\S]*local Data Service model gateway[\s\S]*OPENAI_API_KEY/,
  )
})
...
test('Claude SDK runtime handler injects data service credentials through subprocess env', async () => {
  const workspaceDir = mkdtempSync(join(tmpdir(), 'movscript-claude-sdk-backend-'))
  writeMovScriptDataServiceConfig(workspaceDir, { baseURL: 'https://backend.example/api/v1' })
  writeMovScriptDataServiceAuth(workspaceDir, { token: 'mv1.backend-session-token' })
  writeMovScriptWorkspaceConfig(resolveMovScriptWorkspacePaths(workspaceDir).configPath, {
    schema: MOVSCRIPT_WORKSPACE_CONFIG_SCHEMA,
    updatedAt: '2026-06-18T00:00:00.000Z',
...
test('Claude SDK runtime handler defaults to the workspace data service session', async () => {
  const workspaceDir = mkdtempSync(join(tmpdir(), 'movscript-claude-sdk-default-backend-'))
  writeMovScriptDataServiceConfig(workspaceDir, { baseURL: 'http://localhost:8766' })
  writeMovScriptDataServiceAuth(workspaceDir, { token: 'mv1.default-backend-token' })
  const calls: unknown[] = []
  async function* query(input: unknown) {
    calls.push(input)
    yield { type: 'result', result: 'finished' }
  ...
  assert.equal(probe.ok, true)
  assert.equal(probe.credentials?.source, 'movscript-data-service-session')
