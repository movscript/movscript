...
async function loadAdminConsoleErrorPage(
  ...
  const likelyCause = input.rendererURL
    ? `The admin dev renderer is not reachable at ${input.rendererURL}. Start @movscript/admin dev or set ELECTRON_ADMIN_URL / MOVSCRIPT_ADMIN_RENDERER_URL.`
    : `The admin build was not found at ${input.adminDistDir}. Run the admin build before opening the desktop admin console.`
