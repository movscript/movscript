      ...
      inputSchema: withCandidateAttachAliasRequirements(objectSchema(
        {
          ...
          resource_id: { type: 'number', minimum: 1, description: 'Existing raw resource ID, usually generation provider output_resource_id.' },
          resourceId: { type: 'number', minimum: 1, description: 'Alias for resource_id.' },
          output_resource_id: { type: 'number', minimum: 1, description: 'Alias for resource_id when using generation provider output_resource_id directly.' },
          ...
          output_resource_ids: { type: 'array', items: { type: 'number', minimum: 1 }, description: 'Alias for resource_ids when using generation provider output_resource_ids directly.' },
