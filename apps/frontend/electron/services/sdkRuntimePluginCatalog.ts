...
export function listSdkRuntimeBundledPlugins(input: {
  workspaceDir?: string
} = {}): {
  marketplaces: Array<{
    name: string
...
export function installSdkRuntimeBundledPlugin(input: {
  pluginName?: string
  workspaceDir?: string
} = {}): {
...
export function ensureSdkRuntimeRootBundledPlugin(input: {
  workspaceDir: string
  ...
  const installed = installSdkRuntimeBundledPlugin({ workspaceDir: input.workspaceDir })
  return {
    installed: true,
    prepared: true,
