...
} from './mediaPipeline/ffmpegRunner'
import {
  getExpectedMediaPipelineBundledFFmpegPath,
...
import {
  buildMediaPipelineSingleClipArgs,
  buildMediaPipelineVideoFadeFilter,
  renderMediaPipelineSingleClip,
  runMediaPipelineClipWithFallback,
} from './mediaPipeline/singleClipRenderer'
import { normalizeMediaPipelineTimelineOutputName } from './mediaPipeline/timelineFiles'
import { renderMediaPipelineTimeline } from './mediaPipeline/timelineRenderer'

const execFileAsync = promisify(execFile)
const maxClipSourceBytes = 1024 * 1024 * 1024

test('renderMediaPipelineSingleClip rejects missing source input', async () => {
  const result = await renderMediaPipelineSingleClip({ startMs: 0, endMs: 1000 })
  assert.equal(result.ok, false)
  assert.equal(result.code, 'SOURCE_REQUIRED')
})

test('renderMediaPipelineTimeline rejects empty timelines before probing ffmpeg', async () => {
  const result = await renderMediaPipelineTimeline({ clips: [] })
  assert.equal(result.ok, false)
  assert.equal(result.code, 'TIMELINE_EMPTY')
})

test('renderMediaPipelineTimeline rejects invalid captions before probing ffmpeg', async () => {
...
test('renderMediaPipelineTimeline rejects invalid audio placement before probing ffmpeg', async () => {
  const result = await renderMediaPipelineTimeline({
    clips: [{ sourceData: new Uint8Array([1, 2, 3]), startMs: 0, endMs: 1000 }],
...
test('renderMediaPipelineTimeline rejects invalid overlays before probing ffmpeg', async () => {
  const result = await renderMediaPipelineTimeline({
    clips: [{ sourceData: new Uint8Array([1, 2, 3]), startMs: 0, endMs: 1000 }],
...
test('renderMediaPipelineSingleClip rejects invalid clip ranges before invoking ffmpeg', async () => {
  const result = await renderMediaPipelineSingleClip({
...
test('renderMediaPipelineSingleClip rejects empty source data before invoking ffmpeg', async () => {
  const result = await renderMediaPipelineSingleClip({
...
test('renderMediaPipelineSingleClip enforces a bounded local clip duration', async () => {
  const result = await renderMediaPipelineSingleClip({
    sourceData: new Uint8Array([1, 2, 3]),
    sourceName: 'sample.mp4',
    startMs: 0,
...
test('renderMediaPipelineSingleClip rejects empty local source paths before invoking ffmpeg', async () => {
...
test('renderMediaPipelineSingleClip rejects oversized local source paths before invoking ffmpeg', async () => {
...
test('getMediaPipelineFFmpegStatus reports local ffmpeg availability', async () => {
...
test('getExpectedMediaPipelineBundledFFmpegPath resolves platform and arch bundle locations', () => {
  assert.equal(
    getExpectedMediaPipelineBundledFFmpegPath({
...
test('getMediaPipelineFFmpegStatus includes actionable bundled path when ffmpeg is missing', async () => {
  const status = await getMediaPipelineFFmpegStatus({
...
test('getMediaPipelineFFmpegStatus reports non-missing ffmpeg failures separately', async () => {
  const status = await getMediaPipelineFFmpegStatus({
    resourcesPath: '/opt/MovScript/resources',
    platform: 'darwin',
    arch: 'arm64',
...
test('readMediaPipelineFFmpegVersion returns the first version output line', async () => {
  ...
  const promise = readMediaPipelineFFmpegVersion('ffmpeg', {
    timeoutMs: 0,
...
test('readMediaPipelineFFmpegVersion kills timed out status probes', async () => {
  const child = new EventEmitter() as EventEmitter & {
    stdout: EventEmitter
    stderr: EventEmitter
  ...
  await assert.rejects(
    readMediaPipelineFFmpegVersion('ffmpeg', {
      timeoutMs: 1,
      spawnProcess: (() => child) as never,
    }),
    MediaPipelineFFmpegTimeoutError,
...
test('readMediaPipelineFFmpegVersion reports stderr from failed status probes', async () => {
  const child = new EventEmitter() as EventEmitter & {
    stdout: EventEmitter
    stderr: EventEmitter
    kill: () => boolean
...
test('parseMediaPipelineFFmpegFilters extracts filter names from ffmpeg filter output', () => {
  const filters = parseMediaPipelineFFmpegFilters(`
Filters:
  T.. = Timeline support
...
test('readMediaPipelineFFmpegFilters returns parsed filter names from ffmpeg', async () => {
  ...
  const promise = readMediaPipelineFFmpegFilters('ffmpeg', {
    timeoutMs: 0,
    spawnProcess: (() => child) as never,
  })
...
test('readMediaPipelineFFmpegFilters reports stderr from failed filter probes', async () => {
  const child = new EventEmitter() as EventEmitter & {
    stdout: EventEmitter
    stderr: EventEmitter
    kill: () => boolean
...
test('getRequiredTimelineFFmpegFilters reflects timeline features', () => {
  ...
  assert.deepEqual(getRequiredTimelineFFmpegFilters({
    clips: [{ sourceData: new Uint8Array([1]), startMs: 0, endMs: 1000 }],
    captions: [{ startMs: 0, endMs: 1000, text: 'ASS Caption', renderer: 'ass' }],
  }), ['pad', 'scale', 'setsar', 'subtitles'])

  assert.deepEqual(getRequiredTimelineFFmpegFilters({
    clips: [{ sourceData: new Uint8Array([1]), startMs: 0, endMs: 1000 }],
    subtitleFiles: [{ sourceData: new Uint8Array([2]), sourceName: 'captions.ass', format: 'ass' }],
  }), ['pad', 'scale', 'setsar', 'subtitles'])
})

test('buildMediaPipelineSingleClipArgs uses input seek and stream copy for fast clips', () => {
  const args = buildMediaPipelineSingleClipArgs({
...
test('buildMediaPipelineSingleClipArgs uses output seek and re-encoding for accurate clips', () => {
  const args = buildMediaPipelineSingleClipArgs({
    sourcePath: '/tmp/source.mp4',
    startMs: 1250,
    endMs: 3250,
...
test('buildMediaPipelineSingleClipArgs applies video fade filters for accurate clips', () => {
  const args = buildMediaPipelineSingleClipArgs({
    sourcePath: '/tmp/source.mp4',
    startMs: 0,
    endMs: 5000,
...
test('buildMediaPipelineVideoFadeFilter clamps fades to half the clip duration', () => {
  assert.equal(buildMediaPipelineVideoFadeFilter(4000, 4000, 4000), 'fade=t=in:st=0:d=2.000,fade=t=out:st=2.000:d=2.000')
  assert.equal(buildMediaPipelineVideoFadeFilter(0, 0, 4000), '')
})

test('buildMediaPipelineCropFilter creates percentage crop expressions for visual clips', () => {
  assert.equal(buildMediaPipelineCropFilter({}), '')
  assert.equal(
    buildMediaPipelineCropFilter({ cropLeftPercent: 10, cropRightPercent: 5, cropTopPercent: 20, cropBottomPercent: 0 }),
...
test('buildAssSubtitleDocument creates libass-compatible ASS events', () => {
  const document = buildAssSubtitleDocument([
    {
      startMs: 1230,
      endMs: 4560,
      text: 'Styled {caption}',
  ...
  assert.match(document, /\[V4\+ Styles\]/)
  assert.match(document, /Dialogue: 40,0:00:01\.23,0:00:04\.56,Default/)
  assert.match(document, /\\fnInter/)
  assert.match(document, /\\fs48/)
  assert.match(document, /\\c&H00FF6633&/)
  assert.match(document, /\\an3/)
  assert.match(document, /Styled \\{caption\\}/)
})

test('buildSubtitleFileBurnArgs uses the subtitles filter and escapes paths', () => {
  const args = buildSubtitleFileBurnArgs('/tmp/base.mp4', '/tmp/final.mp4', "/tmp/subs:take 1/cap'tions.ass")
  assert.equal(args[args.indexOf('-vf') + 1], "subtitles=filename='/tmp/subs\\:take 1/cap\\'tions.ass'")
  assert.ok(args.includes('libx264'))
  assert.equal(args[args.length - 1], '/tmp/final.mp4')
})
...
test('normalizeMediaPipelineTimelineOutputName avoids overwriting the prepared input file', () => {
  assert.equal(normalizeMediaPipelineTimelineOutputName('source.mp4', 'source.mp4', 'source.mp4'), 'source_clip.mp4')
  assert.equal(normalizeMediaPipelineTimelineOutputName('SOURCE.mp4', 'source.mp4', 'source.mp4'), 'SOURCE_clip.mp4')
  assert.equal(normalizeMediaPipelineTimelineOutputName('source.mov', 'source.mp4', 'source.mp4'), 'source_clip.mp4')
  assert.equal(normalizeMediaPipelineTimelineOutputName('custom.mp4', 'source.mp4', 'source.mp4'), 'custom.mp4')
})

test('normalizeMediaPipelineTimelineOutputName sanitizes unsafe renderer-provided file names', () => {
  assert.equal(normalizeMediaPipelineTimelineOutputName('../clip?.mp4', 'source.mp4', 'source.mp4'), 'clip.mp4')
  assert.equal(normalizeMediaPipelineTimelineOutputName('folder\\clip.mp4', 'source.mp4', 'source.mp4'), 'folder_clip.mp4')
  assert.equal(normalizeMediaPipelineTimelineOutputName('..', 'source.mp4', 'source.mp4'), 'source_clip.mp4')
  assert.equal(normalizeMediaPipelineTimelineOutputName('clip.mov', 'source.mp4', 'source.mp4'), 'clip.mp4')
})

test('normalizeMediaPipelineTimelineOutputName avoids Windows reserved basenames', () => {
  assert.equal(normalizeMediaPipelineTimelineOutputName('CON.mp4', 'source.mp4', 'source.mp4'), 'CON_file.mp4')
  assert.equal(normalizeMediaPipelineTimelineOutputName('aux', 'source.mp4', 'source.mp4'), 'aux_file.mp4')
  assert.equal(normalizeMediaPipelineTimelineOutputName('LPT9.mp4', 'source.mp4', 'source.mp4'), 'LPT9_file.mp4')
})

test('normalizeMediaPipelineTimelineOutputName keeps generated default clip names within the basename limit', () => {
  const outputName = normalizeMediaPipelineTimelineOutputName(undefined, `${'a'.repeat(100)}.mp4`)
  assert.equal(outputName, `${'a'.repeat(75)}_clip.mp4`)
  assert.equal(outputName.replace(/\.mp4$/, '').length, 80)
})

test('runMediaPipelineClipWithFallback retries accurate mode when fast stream copy fails', async () => {
  const calls: string[][] = []
  const mode = await runMediaPipelineClipWithFallback('ffmpeg', {
    sourcePath: '/tmp/source.mp4',
    startMs: 1000,
    endMs: 3000,
    mode: 'fast',
...
test('runMediaPipelineClipWithFallback does not retry accurate mode failures', async () => {
  const calls: string[][] = []
  await assert.rejects(
    runMediaPipelineClipWithFallback('ffmpeg', {
      sourcePath: '/tmp/source.mp4',
      startMs: 1000,
      endMs: 3000,
      mode: 'accurate',
...
test('runMediaPipelineFFmpeg kills timed out ffmpeg processes', async () => {
  ...
  await assert.rejects(
    runMediaPipelineFFmpeg('ffmpeg', ['-version'], {
      timeoutMs: 1,
      spawnProcess: (() => child) as never,
    }),
    MediaPipelineFFmpegTimeoutError,
...
test('runMediaPipelineFFmpeg kills aborted ffmpeg processes', async () => {
  const child = new EventEmitter() as EventEmitter & {
    stdout: EventEmitter
    stderr: EventEmitter
    kill: (signal?: NodeJS.Signals) => boolean
  ...
  const promise = runMediaPipelineFFmpeg('ffmpeg', ['-version'], {
    timeoutMs: 0,
  ...
  controller.abort()
  await assert.rejects(promise, MediaPipelineFFmpegCanceledError)
  assert.deepEqual(killedSignals, ['SIGTERM'])
})

test('runMediaPipelineFFmpeg limits stderr included in failed clip errors', async () => {
  const child = new EventEmitter() as EventEmitter & { stderr: EventEmitter; kill: () => boolean }
  child.stderr = new EventEmitter()
  child.kill = () => true
  const promise = runMediaPipelineFFmpeg('ffmpeg', ['-i', 'bad.mp4'], {
    timeoutMs: 0,
    stderrLimit: 8,
    spawnProcess: (() => child) as never,
  })
...
test('runMediaPipelineFFmpeg emits raw stdout and stderr chunks through onOutput', async () => {
  const child = new EventEmitter() as EventEmitter & {
    stdout: EventEmitter
    stderr: EventEmitter
    kill: () => boolean
  ...
  assert.deepEqual(chunks, [
    { stream: 'stdout', chunk: 'stdout line\n' },
    { stream: 'stderr', chunk: 'stderr line\n' },
  ])
})

test('renderMediaPipelineSingleClip produces an mp4 from a real local sample when ffmpeg is available', async (t) => {
  const status = await getMediaPipelineFFmpegStatus()
  if (!status.available) {
...
test('renderMediaPipelineTimeline produces a caption-burned mp4 from a real local sample when ffmpeg is available', async (t) => {
  const status = await getMediaPipelineFFmpegStatus()
  if (!status.available) {
    t.skip('ffmpeg is not available in this environment')
    return
...
test('renderMediaPipelineTimeline burns ASS/libass captions from a real local sample when ffmpeg supports subtitles', async (t) => {
  const status = await getMediaPipelineFFmpegStatus()
  if (!status.available) {
    t.skip('ffmpeg is not available in this environment')
    return
  }
  if (!await ffmpegHasFilter(status.path || 'ffmpeg', 'subtitles')) {
    t.skip('ffmpeg subtitles/libass filter is not available in this environment')
    return
  }

  const dir = await mkdtemp(join(tmpdir(), 'movscript-video-timeline-ass-test-'))
  try {
    const sourcePath = join(dir, 'source.mp4')
    await execFileAsync(status.path || 'ffmpeg', [
      '-y',
    ...
    const result = await renderMediaPipelineTimeline({
      clips: [
        { sourceData, sourceName: 'source.mp4', startMs: 0, endMs: 2000 },
      ],
      captions: [
        {
          startMs: 250,
          endMs: 1750,
          text: 'MovScript ASS caption',
          renderer: 'ass',
          fontFamily: 'Arial',
          fontSize: 40,
          textColor: '#ffcc00',
      ...
      outputName: 'timeline-ass.mp4',
    })
    assert.equal(result.ok, true, result.error)
    assert.equal(result.mimeType, 'video/mp4')
    assert.equal(result.outputName, 'timeline-ass.mp4')
    assert.ok(result.data && result.data.byteLength > 0)
  } finally {
    await rm(dir, { recursive: true, force: true })
  }
...
test('renderMediaPipelineTimeline preserves timeline gaps with black video when ffmpeg is available', async (t) => {
  const status = await getMediaPipelineFFmpegStatus()
  if (!status.available) {
    t.skip('ffmpeg is not available in this environment')
    return
...
test('renderMediaPipelineTimeline renders upper video overlays when ffmpeg is available', async (t) => {
  const status = await getMediaPipelineFFmpegStatus()
  if (!status.available) {
    t.skip('ffmpeg is not available in this environment')
    return
...
test('renderMediaPipelineTimeline mixes external audio with fades when ffmpeg is available', async (t) => {
  const status = await getMediaPipelineFFmpegStatus()
  if (!status.available) {
    t.skip('ffmpeg is not available in this environment')
    return
