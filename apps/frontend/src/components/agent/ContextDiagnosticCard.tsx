...
export function ContextDiagnosticCard({ diagnostic }: { diagnostic: ChatContextDiagnostic }) {
  ...
  return (
    <div className="mt-1 space-y-2 rounded-md border border-border bg-background/70 p-2.5 type-label">
      <div className="flex items-start justify-between gap-2">
        <div className="min-w-0">
          ...
          <p className="mt-0.5 type-tiny leading-relaxed text-muted-foreground">
            本地诊断快照；不会发起模型网关调用。
          </p>
        </div>
        <Button
      ...
      {focusPart && (
        <details className="rounded-md border border-border bg-background/70" open>
          <summary className="flex cursor-pointer list-none items-center gap-1.5 px-2 py-1.5 type-tiny font-medium text-foreground marker:hidden">
            <Route size={10} />
            页面焦点
          </summary>
          <pre className="max-h-44 overflow-auto whitespace-pre-wrap break-words border-t border-border px-2 py-1.5 type-tiny leading-relaxed text-muted-foreground">
            {focusPart.content}
          </pre>
        </details>
      )}
      ...
      <details className="rounded-md border border-border bg-background/70" open>
        <summary className="flex cursor-pointer list-none items-center justify-between gap-2 px-2 py-1.5 type-tiny font-medium text-foreground marker:hidden">
          <span className="inline-flex items-center gap-1.5"><Wrench size={10} /> 随模型请求发送的工具</span>
          <span className="type-micro text-muted-foreground">{modelTools.length}</span>
        </summary>
        <div className="max-h-72 space-y-1.5 overflow-y-auto border-t border-border p-1.5">
          {modelTools.length === 0 ? (
            <p className="px-1 type-tiny text-muted-foreground">没有随请求发送可调用工具。</p>
          ) : modelTools.map((tool) => {
      ...
      {blockedTools.length > 0 && (
        <details className="rounded-md border border-border bg-background/70">
          <summary className="flex cursor-pointer list-none items-center justify-between gap-2 px-2 py-1.5 type-tiny font-medium text-foreground marker:hidden">
            ...
            <span className="type-micro text-muted-foreground">{blockedTools.length}</span>
          </summary>
      ...
      <details className="rounded-md border border-border bg-background/70">
        <summary className="flex cursor-pointer list-none items-center justify-between gap-2 px-2 py-1.5 type-tiny font-medium text-foreground marker:hidden">
          ...
          <span className="type-micro text-muted-foreground">{diagnostic.debugParts.length}</span>
        </summary>
            ...
            <div key={part.id} className="rounded border border-border/70 bg-muted/20">
              <div className="flex items-center justify-between gap-2 border-b border-border/60 px-2 py-1">
                <div className="flex min-w-0 items-center gap-1.5">
                  <Badge variant="outline" className="type-min leading-3 px-1 py-0">{part.kind}</Badge>
                  <span className="truncate type-tiny font-medium text-foreground">{part.title}</span>
                </div>
                <span className="shrink-0 type-micro text-muted-foreground">{part.content.length}</span>
              </div>
              <pre className="max-h-28 overflow-auto whitespace-pre-wrap break-words px-2 py-1.5 type-tiny leading-relaxed text-muted-foreground">
                {part.content}
              </pre>
            </div>
          ))}
      ...
      <details className="rounded-md border border-border bg-background/70">
        <summary className="flex cursor-pointer list-none items-center justify-between gap-2 px-2 py-1.5 type-tiny font-medium text-foreground marker:hidden">
          ...
          <span className="type-micro text-muted-foreground">{diagnostic.messages.length}</span>
        </summary>
        <div className="space-y-1.5 border-t border-border p-1.5">
          {diagnostic.messages.map((message, index) => (
            <div key={`${message.role}-${index}`} className="rounded border border-border/70 bg-muted/20">
              <div className="flex items-center justify-between gap-2 border-b border-border/60 px-2 py-1">
                <Badge variant="outline" className="type-min leading-3 px-1 py-0">{message.role}</Badge>
                <span className="type-micro text-muted-foreground">{message.content.length}</span>
              </div>
              <pre className="max-h-36 overflow-auto whitespace-pre-wrap break-words px-2 py-1.5 type-tiny leading-relaxed text-foreground">
                {message.content}
              </pre>
            </div>
          ))}
  ...
  return (
    <div className="rounded border border-border/70 bg-background px-2 py-1.5 type-tiny">
      <div className="flex min-w-0 items-center gap-1">
        ...
        {'risk' in tool && tool.risk && <Badge variant="outline" className="type-min leading-3 px-1 py-0">{localAgentApprovalRiskText(tool.risk, t)}</Badge>}
        {'approval' in tool && tool.approval && <Badge variant="secondary" className="type-min leading-3 px-1 py-0">{toolApprovalLabel(tool.approval)}</Badge>}
      ...
      {schema !== undefined && (
        <details className="mt-1 rounded border border-border/60 bg-muted/20">
          ...
          <pre className="max-h-32 overflow-auto whitespace-pre-wrap break-words border-t border-border/60 px-1.5 py-1 type-micro text-muted-foreground">
...
function DiagnosticSummaryItem({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-md border border-border bg-muted/20 px-2 py-1.5">
      <div className="type-micro uppercase tracking-wide text-muted-foreground/70">{label}</div>
      <div className="mt-0.5 truncate type-caption font-medium text-foreground">{value}</div>
    </div>
  )
}
