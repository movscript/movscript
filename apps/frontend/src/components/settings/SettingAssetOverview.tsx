    ...
    <section className={cn('rounded-lg border border-border bg-card', className)}>
      <div className="flex items-center justify-between border-b border-border px-4 py-3">
        <div className="flex items-center gap-2">
          ...
          <p className="text-sm font-semibold text-foreground">关联素材位</p>
        </div>
        <span className="text-xs text-muted-foreground">{slots.length}</span>
      </div>
      <div className="p-4">
        {isLoading ? (
          <p className="py-8 text-center text-xs text-muted-foreground">正在加载素材位</p>
        ) : slots.length === 0 ? (
          <div className="rounded-md border border-dashed border-border p-6 text-center">
            <Image size={22} className="mx-auto text-muted-foreground" />
            <p className="mt-2 text-sm font-medium text-foreground">暂无关联素材位</p>
            <p className="mt-1 text-xs text-muted-foreground">素材位会从资料继续传递到情节和内容生产。</p>
          </div>
        ) : (
