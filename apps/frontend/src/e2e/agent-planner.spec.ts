...
test('planner run exposes plan overview and run detail drilldown', async ({ page }, testInfo) => {
  ...
  await expect(page.getByTestId('agent-run-trace-summary')).toContainText('2 events')
  ...
  await expect(page.getByTestId('agent-run-header')).toContainText('cancelled')
  await expect(page.getByTestId('agent-run-plan-context')).toContainText('cancelled')
