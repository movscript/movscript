...
test('planner run exposes taskGraph overview and run detail drilldown', async ({ page }, testInfo) => {
  ...
  await expect(page.getByTestId('agent-run-debug-bundle-contract')).toContainText('movscript.agent-run-debug-bundle.v2')
  await expect(page.getByTestId('agent-run-debug-bundle-contract')).toContainText('14 项能力')
  ...
  expect(debugBundleText).toContain('"schema": "movscript.agent-run-debug-bundle.v2"')
