...
test('production orchestrate separates structure and review', async ({ page }, testInfo) => {
  await openProductionOrchestratePage(page, testInfo)
...
test('production orchestrate keeps the separation readable on mobile width', async ({ page }, testInfo) => {
...
test('production orchestrate opens both drafts for review', async ({ page }, testInfo) => {
  await openProductionOrchestratePage(page, testInfo, {
...
async function mockProductionOrchestrateEntities(page: Page) {
...
async function openProductionOrchestratePage(page: Page, testInfo: TestInfo, params?: { draftId?: string; projectDraftId?: string }) {
  ...
  await page.goto(`/production-orchestrate?${search.toString()}`)
}
