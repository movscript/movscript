...
test('run trace labels localize categories and statuses', () => {
  ...
  assert.equal(approvalRiskLabel('resume'), '接续运行')
  ...
  assert.equal(approvalPermissionLabel('runtime.continuation'), '接续异步任务')
  ...
  assert.equal(approvalImpactLabel({ toolName: 'runtime_continuation_resume', permission: 'runtime.continuation', risk: 'resume', preview: undefined }), '批准后会基于已完成的异步任务结果启动一个新的接续 run，继续原任务。')
