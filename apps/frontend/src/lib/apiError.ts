...
const EXACT_KEYS: Record<string, string> = {
  ...
  '节点不存在': 'apiErrors.nodeNotFound',
  '节点不能指向自身': 'apiErrors.nodeSelfDependency',
  '依赖关系已存在': 'apiErrors.dependencyExists',
  '该分场已关联到此分集': 'apiErrors.sceneAlreadyLinked',
  '只有草稿或被拒绝的节点可以提交审核': 'apiErrors.pipelineSubmitState',
  '只有审核中的节点可以批准': 'apiErrors.pipelineApproveState',
  '只有审核中的节点可以拒绝': 'apiErrors.pipelineRejectState',
  '节点已经是草稿状态': 'apiErrors.pipelineAlreadyDraft',
  '需要导演或所有者权限': 'apiErrors.directorOrOwnerRequired',
  '需要所有者、导演或节点负责人权限': 'apiErrors.pipelineManageAssignmentRequired',
  '需要执行者、负责人、导演或所有者权限': 'apiErrors.pipelineSubmitPermissionRequired',
  '需要所有者、导演或非自审负责人权限': 'apiErrors.pipelineReviewPermissionRequired',
  '执行者必须是项目成员': 'apiErrors.pipelineAssigneeMemberRequired',
  '负责人必须是项目成员': 'apiErrors.pipelineLeadMemberRequired',
