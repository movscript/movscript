...
test('content workbench write model builds proposal patch and merges metadata', () => {
  const patch = buildContentUnitProposalPatch(
    record({ ID: 50, metadata_json: JSON.stringify({ existing: true }) }),
    {
      ...
      description: '手敲门',
      prompt: '生成手敲门',
      visual_taskGraph: { blocking: '手靠近门' },
    },
  )
  ...
  assert.deepEqual(JSON.parse(String(patch.metadata_json)).visual_taskGraph, { blocking: '手靠近门' })
})

test('content workbench write model builds only changed reorder patches', () => {
  const taskGraph = buildContentUnitReorderPatchTaskGraph(row({
    units: [
  ...
  assert.deepEqual(buildContentUnitReorderPatchTaskGraph(row({
...
test('content workbench write model updates an existing timeline item', () => {
  ...
  assert.equal(taskGraph.kind, 'update_item')
  assert.equal(taskGraph.itemId, 80)
  assert.deepEqual(taskGraph.payload, {
...
test('content workbench write model creates a timeline item and missing timeline payload', () => {
  ...
  assert.equal(taskGraph.kind, 'create_item')
  assert.equal(taskGraph.timelineId, undefined)
  assert.deepEqual(taskGraph.timelinePayload, {
    production_id: 1,
    name: '手部特写 时间轴',
    duration_sec: 14,
    is_primary: true,
