import type { WorkbenchScenarioPriority, WorkbenchScenarioStatus } from './workbenchScenarios'
...
const ACTION_TERMS: Record<ProductionActionKey, ProductionTerm> = {
  complete_info: { key: 'complete_info', label: '补信息', tone: 'warning', detail: '基础字段还不完整，先补齐标题、描述、用途或归属。' },
  add_setting: { key: 'add_setting', label: '补设定', tone: 'warning', detail: '缺人物、场景、道具、风格或世界规则，先补齐制作依据。' },
  add_asset: { key: 'add_asset', label: '补素材', tone: 'warning', detail: '缺可用参考或主素材，先生成、上传或绑定素材。' },
  select_asset: { key: 'select_asset', label: '选素材', tone: 'info', detail: '已有候选，但还没有选定主素材。' },
  selected_asset: { key: 'selected_asset', label: '已选定', tone: 'success', detail: '已经选中主素材，可以进入下游制作。' },
  confirm_content: { key: 'confirm_content', label: '确认内容', tone: 'info', detail: '已有草案或建议，需要人工确认后进入下游。' },
  split_shots: { key: 'split_shots', label: '拆镜头', tone: 'info', detail: '情节已有基础信息，可以拆成制作项或镜头。' },
  add_keyframe: { key: 'add_keyframe', label: '补关键帧', tone: 'warning', detail: '视觉镜头缺画面锚点，先补首帧、尾帧或关键画面。' },
  ready_to_generate: { key: 'ready_to_generate', label: '可生成', tone: 'success', detail: '生成前条件已满足，可以进入画布或生成任务。' },
  generating: { key: 'generating', label: '生成中', tone: 'info', detail: '生成任务正在执行，等待结果或查看任务记录。' },
  review_result: { key: 'review_result', label: '审阅结果', tone: 'info', detail: '已有生成或制作结果，需要判断是否采用。' },
  rework: { key: 'rework', label: '处理返工', tone: 'warning', detail: '结果未达到要求，需要修改意见或重新生成。' },
  lock_version: { key: 'lock_version', label: '锁定版本', tone: 'success', detail: '内容可作为正式制作依据，确认后锁定主版本。' },
  prepare_delivery: { key: 'prepare_delivery', label: '准备交付', tone: 'success', detail: '制作结果已具备交付基础，进入预览、审片或导出检查。' },
  done: { key: 'done', label: '已完成', tone: 'success', detail: '当前对象已经完成闭环。' },
}

const COVERAGE_TERMS: Record<ProductionCoverageKey, ProductionTerm> = {
  empty: { key: 'empty', label: '无内容', tone: 'neutral', detail: '还没有关联的设定、素材或制作输入。' },
  gap: { key: 'gap', label: '有缺口', tone: 'warning', detail: '关键条件缺失，会影响下游制作。' },
  partial: { key: 'partial', label: '部分覆盖', tone: 'info', detail: '已有一部分内容，但还需要选择或补齐。' },
  covered: { key: 'covered', label: '已覆盖', tone: 'success', detail: '当前上下文需要的内容已经够用。' },
  locked: { key: 'locked', label: '已锁定', tone: 'success', detail: '内容已经成为正式制作依据。' },
}
...
export function scenarioAction(status: WorkbenchScenarioStatus): ProductionTerm {
...
export function gateActionLabel(done: boolean, tone?: 'warning' | 'success') {
  if (done) return '已通过'
  return tone === 'warning' ? '待补齐' : '待确认'
}

export function priorityActionLabel(priority: WorkbenchScenarioPriority) {
  if (priority === 'high') return '优先处理'
  if (priority === 'medium') return '正常处理'
  return '后续处理'
}

export function productionToneToWorkbenchTone(tone: ProductionTone): 'neutral' | 'info' | 'success' | 'warning' | 'danger' {
  return tone
}
