...
test('agent run page keeps plan context and trace drilldown hooks', () => {
  ...
  assertIncludes(source, "import { formatAgentTraceDebugData } from '@/lib/agentTraceDebugData'")
  ...
  assertIncludes(source, 'data-testid="agent-run-model-http-request"')
  assertIncludes(source, 'data-testid="agent-run-model-http-response"')
  ...
  assertIncludes(source, '耗时 {formatAgentRunDuration(event.createdAt, event.completedAt)}')
