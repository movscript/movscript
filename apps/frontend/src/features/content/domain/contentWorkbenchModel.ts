...
import { listSemanticEntities, semanticEntityConfig, type GenerationContext, type SemanticEntityRecord } from '@/shared/infrastructure/api/semanticEntities'
...
import type { WorkbenchGate, WorkbenchLinkRow, WorkbenchPriority as Priority, WorkbenchStatus as WorkStatus } from '@/shared/domain/workbenchTypes'
...
export function buildGenerationContextStandards(context?: GenerationContext): WorkbenchGate[] {
  ...
  if (!target) {
    return [
      { label: '目标提示可读', detail: '后端生成上下文缺少制作项目标，请重新检查生成上下文接口。', done: false, state: 'required' },
    ]
  }
...
function workbenchGateState(done: boolean): WorkbenchGate['state'] {
  return done ? 'passed' : 'required'
}
