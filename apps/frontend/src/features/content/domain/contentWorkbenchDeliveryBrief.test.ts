...
import { buildContentWorkbenchDeliveryBrief } from '@/features/content/domain/contentWorkbenchDeliveryBrief'
...
test('content workbench delivery brief lists production blockers', () => {
  ...
  assert.equal(brief.title, '下一步：补齐交付条件')
  assert.equal(brief.progress, 0)
  assert.deepEqual(brief.blockers, ['生成检查仍有 4 项待补齐'])
})
...
test('content workbench delivery brief reports checking while context is loading', () => {
  const brief = buildContentWorkbenchDeliveryBrief({
    hasSelectedUnit: true,
    unitTitle: '纸条特写',
    hasPrompt: true,
  ...
  assert.equal(brief.title, '交付包检查中')
})
...
test('content workbench delivery brief reports closed delivery loop', () => {
  ...
  assert.equal(brief.state, 'closed')
