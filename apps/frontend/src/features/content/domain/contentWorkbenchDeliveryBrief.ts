export type ContentWorkbenchDeliveryBriefState = 'empty' | 'blocked' | 'checking' | 'ready' | 'closed'
...
export interface ContentWorkbenchDeliveryBrief {
  state: ContentWorkbenchDeliveryBriefState
  title: string
  detail: string
  progress: number
  blockers: string[]
...
export function buildContentWorkbenchDeliveryBrief(input: ContentWorkbenchDeliveryBriefInput): ContentWorkbenchDeliveryBrief {
  ...
  const deliveryBlockers = preGenerationBlockers.length > 0
    ? [`生成检查仍有 ${preGenerationBlockers.length} 项待补齐`]
    : [
  ...
  const state: ContentWorkbenchDeliveryBriefState = input.generationContextLoading ? 'checking' : 'blocked'
  return {
    state,
    title: state === 'checking' ? '交付包检查中' : '下一步：补齐交付条件',
