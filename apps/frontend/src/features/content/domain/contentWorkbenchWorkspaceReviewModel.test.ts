...
test('content workbench workspace review parses fenced JSON blocks', () => {
  assert.deepEqual(parseWorkspaceJsonContent('```json\n{"ok":true}\n```'), { ok: true })
  assert.deepEqual(parseWorkspaceJsonContent('prefix {"ok":1} suffix'), { ok: 1 })
  assert.equal(parseWorkspaceJsonContent('not json'), null)
})
...
test('content workbench workspace review builds snapshot diffs for content units', () => {
  ...
  const model = buildContentWorkspaceReviewModel(workspace, {
...
test('content workbench workspace review reports invalid or risky workspaces', () => {
  const invalid = buildContentWorkspaceReviewModel({ ...baseWorkspace, content: 'bad' }, {
    rowByMomentId: new Map(),
    rowByUnitId: new Map(),
  })
  ...
  const risky = buildContentWorkspaceReviewModel({
    ...baseWorkspace,
    content: JSON.stringify({
      workspace: {
        timeline_items: [],
...
test('content workbench workspace review helpers dedupe and read entity ids', () => {
  ...
  assert.deepEqual(dedupeWorkspaceArtifacts([
    ...
    { ...baseWorkspace, id: 'b' },
  ]).map((workspace) => workspace.id), ['a', 'b'])
})
