import type { ContentWorkbenchNextActionKey } from '@/features/content/domain/contentWorkbenchNextActions'
...
export function buildContentWorkbenchActivityFeed(input: ContentWorkbenchActivityInput): ContentWorkbenchActivityFeed {
  ...
  return {
    title: blockedCount > 0 ? '生产活动需处理' : runningCount > 0 ? '生产活动执行中' : visibleItems.some((item) => item.state === 'done') ? '生产活动可追溯' : '生产活动待启动',
