...
import { ContentPageEmptyState, ContentPageMeta, ContentPageKeyValue, ContentPageMetricCard, ContentPagePanel, ContentPageSection, ContentPageTextEmptyState, ContentPageActionButton, ContentPageActionRow, ContentPageDetailActionStack, ContentPageDetailHeader, ContentPageDetailHero, ContentPageDetailIdentity, ContentPageDetailMetricGrid, ContentPageDetailPanel, ContentPageDetailSectionStack, ContentPageDetailSummary, ContentPageDisclosure, ContentPageFieldCard, ContentPageFieldGrid, ContentPageHeaderRow, ContentPageInfoBlock, ContentPageInfoGrid, ContentPageList, ContentPageListCard, ContentPageListCardDescription, ContentPageListCardHeader, ContentPageListCardIdentity, ContentPageListCardMetaRow, ContentPageListCardMetricGrid, ContentPageListCardReadiness, ContentPageListCardSubtitle, ContentPageListCardTitle, ContentPageRelatedDescription, ContentPageRelatedGrid, ContentPageRelatedHeader, ContentPageRelatedItem, ContentPageRelatedMetaRow, ContentPageRelatedStack, ContentPageRelatedTitle, ContentPageSectionHeading, ContentPageSplitColumns, ContentPageStack, ContentPageSummaryGrid, ContentPageToneText, ProjectSurfaceHeader, StatusBadge } from '@movscript/ui'
...
import { contentEntityStatusRecipe, contentGapRecipe } from '@/features/content/presentation/contentSemanticUi'
...
import { Badge, CheckboxField, Input, Label, NativeSelect, Textarea } from '@movscript/ui'
      ...
      <ContentWorkspaceLayout
        header={(
          <ProjectSurfaceHeader
            ...
            actions={(
              <>
            <ContentPageActionButton onClick={startCreateSegment}>
              <Plus size={14} />
              新建编排段
            ...
            <ContentPageActionButton variant="outline">
              <GitBranch size={14} />
              查看关系
            </ContentPageActionButton>
              </>
        ...
        overview={(
          ...
          <ContentPageMetricCard icon={BookOpenText} label="编排段" value={segmentWorkspaces.length} detail={`${visibleSegments.length} 个符合当前筛选`} tone="info" />
          <ContentPageMetricCard icon={Film} label="情景" value={sceneMoments.length} detail="编排段内部的具体时空与动作上下文" tone="info" />
        ...
        list={(
            ...
            >
              <ContentPageList>
                {isLoading ? (
                  <div>
                    <ContentPageEmptyState icon={Film} title="正在加载编排段" detail="读取编排段和关联对象" compact />
                  </div>
                ) : visibleSegments.length === 0 ? (
                  <div>
                    <ContentPageEmptyState icon={Film} title="暂无编排段" detail="可以直接新建编排段，剧本版本只是可选来源引用" compact />
                  </div>
                ) : (
        ...
        preview={(
          <>
            <ContentPageSection
              ...
              bodyClassName={selectedSegment ? 'content-page-section__body--padded' : undefined}
            >
              {!selectedSegment ? (
                <ContentPageEmptyState icon={Film} title="未选择编排段" detail="从编排段清单选择一个编排段查看制作项设计" />
              ) : (
                <ContentPageSplitColumns>
                  <ContentPageStack>
                    ...
                    {selectedSegment.sceneMoments.length === 0 ? (
                      <ContentPageEmptyState icon={Film} title="暂无情景" detail="编排段还没有拆分出情景" compact />
                    ) : (
                  ...
                  <ContentPageStack>
                    ...
                    {selectedSegment.contentUnits.length === 0 ? (
                      <ContentPageEmptyState icon={Film} title="暂无制作项设计" detail="确认情景后可生成镜头、字幕卡、旁白或转场制作项" compact />
                    ) : (
    ...
    >
      ...
      >
        <ContentPageListCardIdentity icon={<BookOpenText size={14} />} accent="cyan">
          <ContentPageListCardTitle>{titleOf(item.segment)}</ContentPageListCardTitle>
          <ContentPageListCardSubtitle>{sectionKinds[String(item.segment.kind ?? '')] ?? item.segment.kind ?? '编排段'}</ContentPageListCardSubtitle>
        </ContentPageListCardIdentity>
        <ContentPageListCardDescription>{item.segment.summary || item.segment.content || '暂无情绪、节奏或戏剧功能说明'}</ContentPageListCardDescription>
        {item.scriptBlock ? (
      ...
      <ContentPageListCardMetricGrid>
        <ContentPageKeyValue label="情景" value={item.sceneMoments.length} strong />
...
function SegmentDetailCard({
  ...
  if (!record && !defaults) {
    return (
      <ContentPagePanel>
        <ContentPageEmptyState icon={Film} title="未选择编排段" detail="从左侧编排段列表选择一个情绪、节奏或戏剧功能段，或新建编排段后直接编辑详情" />
      </ContentPagePanel>
    )
  }
  ...
  return (
    <>
    <ContentPageDetailPanel bodyMode="flush">
      <form id={formId} onSubmit={submit}>
        <ContentPageDetailHero accent="cyan">
        <ContentPageDetailHeader>
          <div className="min-w-0">
            <ContentPageDetailIdentity accent="cyan" icon={<BookOpenText size={18} />}>
                ...
                {isEditing && fieldByKey.get('title') ? (
                  <SegmentInlineField
                    ...
                    surface="plain"
                    inputClassName="mt-1 h-10 bg-background/90 type-title font-semibold"
                  />
                ) : (
          ...
          <ContentPageDetailActionStack>
            {isEditing && fieldByKey.get('status') ? (
              ...
              <StatusBadge {...contentEntityStatusRecipe(status)}>{statusLabel(status)}</StatusBadge>
            )}
            {record && !isEditing ? (
              <ContentPageActionRow>
                <ContentPageActionButton type="button" size="sm" variant="outline" surface="overlay" onClick={() => setPreviewOpen(true)}>
                  <Clapperboard size={14} />
                  预览
                </ContentPageActionButton>
                <ContentPageActionButton type="button" size="sm" variant="outline" surface="overlay" onClick={() => setIsEditing(true)} disabled={deleteMutation.isPending}>
                  <Pencil size={14} />
                  编辑
                </ContentPageActionButton>
                {canDeleteRecord ? <ContentPageActionButton type="button" size="sm" variant="solid" tone="danger" onClick={removeRecord} loading={deleteMutation.isPending}>
                  <Trash2 size={14} />
                  删除
                </ContentPageActionButton> : null}
              </ContentPageActionRow>
            ) : (
              <ContentPageActionRow>
                {record && canDeleteRecord ? (
                  <ContentPageActionButton type="button" size="sm" variant="solid" tone="danger" onClick={removeRecord} loading={deleteMutation.isPending}>
                    <Trash2 size={14} />
                    删除
                  </ContentPageActionButton>
                ) : null}
                ...
                <ContentPageActionButton type="submit" size="sm" loading={saveMutation.isPending} disabled={!canSave || deleteMutation.isPending}>
                  <Save size={14} />
                  保存
                </ContentPageActionButton>
              </ContentPageActionRow>
        ...
        {isEditing ? (
          <ContentPageFieldCard variant="overlay" className="mt-4">
            <ContentPageHeaderRow className="mb-3">
        ...
        {item ? (
          ...
          <ContentPageDetailMetricGrid columns="three">
        ...
        {isEditing ? (
          <ContentPageDetailSectionStack>
            ...
            {advancedFields.filter((field) => !compactEditFields.includes(field.key)).length > 0 ? (
              <ContentPageDisclosure title="高级字段">
...
function SceneMomentRow({
  ...
  return (
    ...
    >
      ...
      <ContentPageListCardDescription>{sceneMoment.description || sceneMoment.action_text || sceneMoment.condition_text || '暂无情景描述'}</ContentPageListCardDescription>
      <ContentPageListCardMetricGrid>
        <ContentPageKeyValue label="时间" value={sceneMoment.time_text || '未定'} />
        <ContentPageKeyValue label="地点" value={sceneMoment.location_text || '未定'} />
      </ContentPageListCardMetricGrid>
      <ContentPageListCardMetaRow>
        ...
        {assetGapCount > 0 ? <StatusBadge {...contentGapRecipe(assetGapCount)} className="type-tiny">缺口 {assetGapCount}</StatusBadge> : null}
      </ContentPageListCardMetaRow>
    </ContentPageListCard>
  )
}
...
function ContentUnitRow({
  ...
  return (
    ...
    >
      ...
      <ContentPageListCardDescription>{item.description || item.prompt || '暂无制作项描述或提示词'}</ContentPageListCardDescription>
      <ContentPageListCardMetaRow>
...
function SceneMomentDetail({ sceneMoment, segment }: { sceneMoment: SceneMomentRecord | null; segment: SegmentRecord | null }) {
  ...
  return (
    ...
    >
        ...
        {sceneIdentifier(sceneMoment) ? <ContentPageInfoBlock label="编号" value={sceneIdentifier(sceneMoment)} /> : null}
        <ContentPageInfoBlock label="描述" value={sceneMoment.description || '暂无描述'} />
        <ContentPageInfoGrid>
          <ContentPageKeyValue label="时间" value={sceneMoment.time_text || '-'} strong />
          <ContentPageKeyValue label="地点" value={sceneMoment.location_text || '-'} strong />
        </ContentPageInfoGrid>
        <ContentPageInfoBlock label="条件" value={sceneMoment.condition_text || '-'} />
        <ContentPageInfoBlock label="动作" value={sceneMoment.action_text || '-'} />
        <ContentPageInfoBlock label="情绪" value={sceneMoment.mood || '-'} />
    </ContentPageDetailPanel>
  )
}
...
function ContentUnitDetail({ contentUnit, sceneMoment, scriptBlock }: { contentUnit: RelatedRecord | null; sceneMoment: SceneMomentRecord | null; scriptBlock?: ScriptBlockRecord | null }) {
  ...
  return (
    ...
    >
        ...
        {productionIdentifier(sceneMoment, contentUnit) ? <ContentPageInfoBlock label="编号" value={productionIdentifier(sceneMoment, contentUnit)} /> : null}
        <ContentPageInfoBlock label="所属情景" value={sceneIdentifier(sceneMoment) || (sceneMoment ? titleOf(sceneMoment) : '未绑定情景')} />
        <ContentPageInfoBlock label="来源剧本块" value={scriptBlock ? scriptBlockSourceLabel(scriptBlock) : contentUnit.script_block_id ? `剧本块 #${contentUnit.script_block_id}` : '未绑定剧本块'} />
        {scriptBlock ? <ContentPageInfoBlock label="来源文本" value={String(scriptBlock.content ?? '').trim() || '暂无剧本块正文'} /> : null}
        <ContentPageInfoBlock label="创作提示" value={contentUnit.prompt || contentUnit.description || '暂无提示词'} />
        <ContentPageInfoBlock label="运镜" value={compactJoin([
    ...
    <ContentPageDetailSectionStack>
      ...
      {compactFields.length > 0 ? (
        <SegmentPreviewSection title="全部字段">
          <ContentPageFieldGrid columns="responsive-three">
...
function SegmentPreviewSection({ title, children }: { title: string; children: React.ReactNode }) {
  return <ContentPagePanel title={title}>{children}</ContentPagePanel>
}
  ...
  return (
    <ContentPageInfoBlock
      ...
      valueClassName={cn(
        ...
        field.key.endsWith('_json') && 'app-info-block__code-value',
      )}
    />
  )
}
...
function SegmentEditSection({ title, description, children }: { title: string; description?: string; children: React.ReactNode }) {
  return <ContentPageSection title={title} description={description}>{children}</ContentPageSection>
}
...
function SegmentInlineField({
  ...
  const content = (
    <>
      ...
      {lockReason ? (
        <ContentPageToneText as="p" tone="warning" className="mt-1 type-caption font-medium">
          {lockReason}
        </ContentPageToneText>
      ) : field.helper ? <p className="mt-1 type-caption text-muted-foreground">{field.helper}</p> : null}
    </>
  )

  return surface === 'card'
    ? <ContentPageFieldCard>{content}</ContentPageFieldCard>
...
function RelatedPanel({
  ...
  return (
    <ContentPagePanel icon={Icon} title={title} action={<Badge variant="outline" className="type-tiny">{records.length}</Badge>}>
      <ContentPageRelatedStack>
        {records.length === 0 ? (
          <ContentPageTextEmptyState>{empty}</ContentPageTextEmptyState>
        ) : (
...
function RelatedRow({ record, scriptBlocksById }: { record: RelatedRecord | SceneMomentRecord | SegmentRecord | ScriptBlockRecord; scriptBlocksById?: Map<number, ScriptBlockRecord> }) {
  ...
  const hasMeta = Boolean(item.kind || item.duration_sec)
  return (
    <ContentPageRelatedItem>
      ...
      >
        <ContentPageRelatedTitle prefix={identifier ? <Badge variant="outline" className="shrink-0 type-tiny">{identifier}</Badge> : null}>
          {title}
        </ContentPageRelatedTitle>
        <ContentPageRelatedDescription>{detail}</ContentPageRelatedDescription>
      </ContentPageRelatedHeader>
      {hasMeta ? (
      ...
      {item.script_block_id && !isScriptBlockRecord(item) ? (
        <ContentPageRelatedMetaRow>
          <ContentPageMeta icon={ScrollText}>{sourceBlock ? scriptBlockSourceLabel(sourceBlock) : `剧本块 #${item.script_block_id}`}</ContentPageMeta>
        </ContentPageRelatedMetaRow>
      ) : null}
    </ContentPageRelatedItem>
  )
...
function HeroStat({ icon: Icon, label, value, tone = 'neutral' }: { icon: LucideIcon; label: string; value: string | number; tone?: 'success' | 'warning' | 'info' | 'neutral' }) {
  return <ContentPageMetricCard icon={Icon} label={label} value={value} tone={tone} compact />
}

function SectionTitle({ icon: Icon, title, count }: { icon: LucideIcon; title: string; count: number }) {
  return (
    <ContentPageSectionHeading icon={Icon} title={title} count={<Badge variant="outline" className="type-tiny">{count}</Badge>} />
  )
}

function InfoChip({ icon: Icon, label }: { icon: LucideIcon; label: string }) {
  return (
    <ContentPageMeta icon={Icon} className="w-full py-1.5 type-caption">{label}</ContentPageMeta>
  )
}
