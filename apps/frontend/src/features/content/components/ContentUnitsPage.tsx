...
import { abandonContentUnit, listSemanticEntities, semanticEntityConfig, type SemanticEntityRecord } from '@/shared/infrastructure/api/semanticEntities'
import {
  Badge,
  Button,
...
import { contentEntityStatusRecipe, contentProgressRecipe } from '@/features/content/presentation/contentSemanticUi'
      ...
      <ContentWorkspaceLayout
        header={(
          <ProjectSurfaceHeader
            icon={Boxes}
            title="制作项"
            description="集中查看制作项的来源、输入缺口和关联对象；拆解、关键帧、素材补齐和生成决策统一在内容编辑中完成。"
            actions={(
              <>
            <ContentPageActionButton asChild>
              <Link to={ROUTES.project.contentUnitEditor}>
                <Wand2 size={14} />
                进入内容编辑
              </Link>
            </ContentPageActionButton>
        ...
        overview={(
          <ContentPageSummaryGrid columns="two-to-four" data-testid="content-units-summary-strip">
          <ContentPageMetricCard icon={Boxes} label="制作项" value={unitViewModels.length} detail="从候选收敛到最终目标" tone="info" compact />
          <ContentPageMetricCard icon={ShieldCheck} label="可编排" value={readyCount} detail="情景、设定资料和素材需求输入已满足" tone="success" compact />
          <ContentPageMetricCard icon={Play} label="候选目标" value={contentTargetCount} detail="关键帧、画面、语音和字幕" tone="info" compact />
          <ContentPageMetricCard icon={LockKeyhole} label="已锁定" value={lockedCount} detail={`${averageReadiness}% 平均编排准备度`} tone="success" compact />
          </ContentPageSummaryGrid>
        )}
        filters={(
          <ContentFilterBar
        ...
        list={(
          <ContentPageSection
            title="制作项清单"
            description="每一项都是可被内容编辑检查、复核和锁定的生产颗粒。"
          >
            <ContentPageListViewport>
              {isLoading ? (
                <ContentPageEmptyState icon={Boxes} title="正在加载制作项" detail="读取制作项、情景和创作输入关系" />
              ) : filteredUnits.length === 0 ? (
                <ContentPageEmptyState icon={Boxes} title="暂无制作项" detail="可先进入内容编辑拆解镜头方案，再回到这里查看清单和关联对象。" />
              ) : (
                <ContentPageList>
  ...
  return (
    ...
    >
      <ContentPageListCardHeader
        aside={(
          <ContentPageListCardStatusGroup>
            <StatusBadge {...contentEntityStatusRecipe(status)} className="shrink-0 type-tiny">{statusMeta[status]?.label ?? (status || '未知')}</StatusBadge>
            <StatusBadge {...contentProgressRecipe(item.readiness)} className="type-tiny">{item.readiness}%</StatusBadge>
          </ContentPageListCardStatusGroup>
        )}
      >
        <ContentPageListCardTitle prefix={identifier ? <ContentPageMeta>{identifier}</ContentPageMeta> : null}>
          {titleOf(item.unit)}
        </ContentPageListCardTitle>
        <ContentPageListCardSubtitle>{kindLabel(item.unit.kind)} · {formatDuration(item.unit.duration_sec)} · {sceneMomentTitle}</ContentPageListCardSubtitle>
      </ContentPageListCardHeader>
      <ContentPageListCardDescription>{item.unit.description || item.unit.prompt || '暂无内容描述或创作提示'}</ContentPageListCardDescription>
      <ContentPageListCardMetaRow>
        <span>来源 {item.scriptBlock ? scriptBlockSourceLabel(item.scriptBlock) : item.unit.script_block_id ? `剧本块 #${item.unit.script_block_id}` : '待补'}</span>
        ...
        {item.missingAssets.length > 0 ? <ContentPageToneText tone="warning">缺口 {item.missingAssets.length}</ContentPageToneText> : null}
      </ContentPageListCardMetaRow>
    </ContentPageListCard>
  )
}
...
function ContentUnitDetail({
  ...
  if (!item) {
    return (
      <ContentPagePanel>
        <ContentPageEmptyState icon={Boxes} title="未选择制作项" detail="从中间清单选择一个制作项查看上下文" compact />
      </ContentPagePanel>
    )
  }
  ...
  return (
    <>
    <ContentPagePanel bodyClassName="p-0">
      <div className="border-b border-border p-3">
        <div className="flex items-start justify-between gap-3">
          <ContentPageIconFrame className="text-muted-foreground">
            <Eye size={16} />
          </ContentPageIconFrame>
          <div className="flex shrink-0 items-center gap-2">
            ...
            <ContentPageActionButton size="sm" asChild>
              <Link to={`${ROUTES.project.contentUnitEditor}${workbenchSearch}`}>
              <Wand2 size={14} />
              进入内容编辑
              </Link>
            </ContentPageActionButton>
        ...
        <h2 className="mt-2.5 type-title font-semibold text-foreground">{titleOf(item.unit)}</h2>
      ...
      <div className="space-y-3 p-3">
        ...
        <div className="grid grid-cols-2 gap-2">
          ...
          <ContentPageKeyValue label="候选目标" value={`${item.targets.filter((target) => target.status !== 'missing').length}/4`} strong />
          <ContentPageKeyValue label="候选总数" value={candidateTotal(item.targets)} strong />
        </div>
        <ContentPageDisclosure
          title={(
          ...
          data-testid="content-unit-detail-context"
        >
            <ContentPageInfoBlock label="情景" value={item.sceneMoment?.description || item.sceneMoment?.action_text || '暂无情景描述'} />
            <ContentPageInfoBlock label="来源剧本块" value={item.scriptBlock ? scriptBlockSourceLabel(item.scriptBlock) : item.unit.script_block_id ? `剧本块 #${item.unit.script_block_id}` : '未绑定剧本块'} />
            {item.scriptBlock ? <ContentPageInfoBlock label="来源文本" value={String(item.scriptBlock.content ?? '').trim() || '暂无剧本块正文'} /> : null}
            <ContentPageInfoBlock label="创作提示" value={item.unit.prompt || item.unit.description || '暂无提示词'} />
            <ContentPageInfoBlock label="运镜设计" value={cameraSummary(item.unit) || '暂无运镜参数'} />
            <div className="grid grid-cols-2 gap-2">
              <ContentPageKeyValue label="景别" value={cameraOptionLabel('shot_size', item.unit.shot_size)} strong />
              <ContentPageKeyValue label="机位" value={cameraOptionLabel('camera_angle', item.unit.camera_angle)} strong />
              <ContentPageKeyValue label="运镜" value={cameraOptionLabel('camera_motion', item.unit.camera_motion)} strong />
              <ContentPageKeyValue label="速度" value={cameraOptionLabel('camera_speed', item.unit.camera_speed)} strong />
            </div>
            <ContentPageInfoBlock label="起始构图" value={item.unit.composition_start || '-'} />
            <ContentPageInfoBlock label="结束构图" value={item.unit.composition_end || '-'} />
            <ContentPageInfoBlock label="运镜备注" value={item.unit.camera_notes || '-'} />
        </ContentPageDisclosure>
      </div>
    </ContentPagePanel>
    {projectId && (
        ...
        {records.length === 0 ? (
          ...
          records.slice(0, 6).map((record) => (
            <ContentPageRelatedItem key={record.id}>
              ...
              >
                <ContentPageRelatedTitle>{record.title}</ContentPageRelatedTitle>
              </ContentPageRelatedHeader>
              <ContentPageRelatedDescription>{record.subtitle}</ContentPageRelatedDescription>
            </ContentPageRelatedItem>
          ))
        )}
      </ContentPageRelatedStack>
...
function ContentTargetPanel({ targets }: { targets: ContentTargetViewModel[] }) {
  return (
    <ContentPagePanel icon={Play} title="候选目标" action={<Badge variant="outline" className="type-tiny">{targets.filter((target) => target.status !== 'missing').length}/4</Badge>}>
      <ContentPageRelatedStack>
        {targets.map((target) => {
          const Icon = target.icon
          return (
            <ContentPageRelatedItem key={target.kind}>
              <ContentPageListCardIdentity icon={<Icon size={14} />} accent="cyan">
                <ContentPageListCardHeader
                  aside={(
                    <StatusBadge {...contentEntityStatusRecipe(target.status === 'ready' ? 'locked' : target.status)} className="shrink-0 type-tiny">
                      {statusMeta[target.status === 'ready' ? 'locked' : target.status]?.label ?? (target.status === 'ready' ? 'locked' : target.status)}
                    </StatusBadge>
                  )}
                >
                  <ContentPageListCardTitle>{target.label}</ContentPageListCardTitle>
                  <ContentPageListCardSubtitle>{target.description}</ContentPageListCardSubtitle>
                </ContentPageListCardHeader>
                <ContentPageListCardMetricGrid>
                  <ContentPageKeyValue label="槽" value={target.slots.length + target.keyframes.length} strong />
                  <ContentPageKeyValue label="候选" value={target.candidateCount} strong />
                  <ContentPageKeyValue label="锁定" value={target.lockedCount} strong />
                </ContentPageListCardMetricGrid>
              </ContentPageListCardIdentity>
            </ContentPageRelatedItem>
          )
...
function CheckRow({ ok, label, detail }: { ok: boolean; label: string; detail: string }) {
  return <ContentPageCheckRow ok={ok} label={label} detail={detail} />
}
