...
export type PreProductionProposalEntryKind = 'creative_references' | 'asset_slots'
export type PreProductionProposalEntryChangeType = 'added' | 'modified' | 'deleted' | 'unchanged'

export interface PreProductionProposalRecord {
  ID: number
  id?: number
  title?: string
  name?: string
...
export interface PreProductionProposalData {
  creativeReferences: PreProductionProposalRecord[]
  assetSlots: PreProductionProposalRecord[]
}

export interface PreProductionProposalEntry {
  key: string
  kind: PreProductionProposalEntryKind
  index: number
  changeType: PreProductionProposalEntryChangeType
  inferred?: boolean
  applied: boolean
  label: string
  detail: string
...
export interface PreProductionProposalView {
  mode: 'snapshot'
  summary: string
  creativeReferences: PreProductionProposalEntry[]
  assetSlots: PreProductionProposalEntry[]
  impactNotes: string[]
}

export interface PreProductionProposalDiffRow {
  label: string
  before?: string
  after: string
  changeType: PreProductionProposalEntryChangeType
...
export function buildPreProductionDraftContentForEntries(
  draft: AgentDraft,
...
function applyPreProductionEntriesToSnapshot(base: Record<string, unknown>[], entries: PreProductionProposalEntry[]) {
  const next = [...base]
  for (const entry of entries) {
    const id = preProductionProposalItemId(entry.raw)
...
function preProductionCreativeReferenceSnapshot(record: PreProductionProposalRecord): Record<string, unknown> {
  return {
    id: record.ID,
...
function preProductionAssetSlotSnapshot(record: PreProductionProposalRecord): Record<string, unknown> {
  return {
    id: record.ID,
...
function rebaseAssetSlotOwner(item: Record<string, unknown>, data: PreProductionProposalData): Record<string, unknown> {
...
function resolveAssetSlotOwnerReferenceID(item: Record<string, unknown>, data: PreProductionProposalData) {
...
function uniquePreProductionReferenceMatch(
  references: PreProductionProposalRecord[],
  predicate: (referenceText: string, reference: PreProductionProposalRecord) => boolean,
): PreProductionProposalRecord | undefined {
...
export function buildPreProductionProposalEntryDiffRows(
  entry: PreProductionProposalEntry,
  data: PreProductionProposalData,
  referenceLabels: Map<string, string> = new Map(),
): PreProductionProposalDiffRow[] {
  const current = draftEntryCurrentRecord(entry, data)
  const rows: PreProductionProposalDiffRow[] = []
...
export function formatPreProductionProposalEntry(entry: PreProductionProposalEntry) {
  return [entry.label, entry.target].filter(Boolean).join(' · ')
}

export function preProductionProposalEntryLabel(entry: PreProductionProposalEntry) {
  return entry.kind === 'creative_references' ? '设定资料' : '素材需求'
}

export function preProductionProposalEntryChangeLabel(entry: PreProductionProposalEntry) {
...
function inferPreProductionEntryChangeType(
  ...
  data: PreProductionProposalData,
): PreProductionProposalEntryChangeType {
...
function preProductionRecordHasFieldDiff(kind: PreProductionProposalEntryKind, item: Record<string, unknown>, current: PreProductionProposalRecord): boolean {
  const currentField = (keys: string[]) => {
    for (const key of keys) {
...
function inferSnapshotDeletionEntries(
  ...
  options: { includeCreativeReferences: boolean; includeAssetSlots: boolean },
): { creativeReferences: PreProductionProposalEntry[]; assetSlots: PreProductionProposalEntry[] } {
...
function draftEntryCurrentRecord(entry: PreProductionProposalEntry, data: PreProductionProposalData) {
  const id = preProductionProposalItemId(entry.raw)
  if (id <= 0) return null
...
function draftEntryOwnerLabel(entry: PreProductionProposalEntry, referenceLabels: Map<string, string>, rawOwnerValue?: string) {
...
function titleOf(record: PreProductionProposalRecord, fallback: string) {
  return asString(record.title, asString(record.name, asString(record.label, fallback)))
}

function bodyOf(record: PreProductionProposalRecord, fallback = '暂无说明') {
