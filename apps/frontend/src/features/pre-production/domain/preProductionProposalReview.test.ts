...
test('project layer proposal review can isolate setting proposal entries', () => {
  const view = parsePreProductionProposalDraft(
    draft({
      id: 'setting-draft',
...
test('project layer proposal review can isolate asset slot proposal entries and diff owner', () => {
  ...
  const rows = buildPreProductionProposalEntryDiffRows(
    view!.assetSlots[0]!,
    {
      creativeReferences: [],
      assetSlots: [{ ID: 12, name: '角色半身照', prompt_hint: '侧面站姿', creative_reference_id: 8 }],
...
test('buildPreProductionDraftContentForEntries keeps unselected backend rows in snapshot apply payload', () => {
  ...
  const view = parsePreProductionProposalDraft(sourceDraft, data, { includeAssetSlots: false })
  const payload = JSON.parse(buildPreProductionDraftContentForEntries(sourceDraft, [view!.creativeReferences[0]!], data)) as Record<string, any>
...
test('buildPreProductionDraftContentForEntries scopes asset proposal payload to asset slots', () => {
  const sourceDraft = draft({
    id: 'asset-draft',
    kind: 'asset_proposal',
    content: JSON.stringify({
  ...
  const view = parsePreProductionProposalDraft(sourceDraft, data, { includeCreativeReferences: false })
  const payload = JSON.parse(buildPreProductionDraftContentForEntries(sourceDraft, [view!.assetSlots[0]!], data)) as Record<string, any>
...
test('buildPreProductionDraftContentForEntries rebases stale asset owner ids from current references', () => {
  const sourceDraft = draft({
    id: 'asset-draft',
    kind: 'asset_proposal',
    content: JSON.stringify({
