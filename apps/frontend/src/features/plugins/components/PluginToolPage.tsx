...
import { usePluginBridge } from '@/features/plugins/presentation/usePluginBridge'
...
function ParamField({
  ...
  if (isModelSelector) {
    ...
    return (
      <PluginToolField>
        <PluginToolFieldLabel>{label}</PluginToolFieldLabel>
        {prop.description && <PluginToolFieldDescription>{prop.description}</PluginToolFieldDescription>}
        <ModelSelector
...
export default function PluginToolPage() {
  ...
  if (notFound || !plugin) {
    return (
      <PluginToolNotFoundState
        icon={<AlertCircle size={24} className="text-muted-foreground" />}
        title={t('plugins.notFound')}
        action={(
        <PluginToolActionButton size="sm" variant="outline" onClick={() => navigate(ROUTES.plugins)}>
