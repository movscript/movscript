...
} from './productionExpressionUnits'

test('production expression units prefer persisted expression rows', () => {
  const lines = buildExpressionUnitLines(
  ...
  assert.deepEqual(lines[0]?.editTarget, { kind: 'expressionUnits', id: 40 })
  assert.equal(lines[0]?.speaker, '李四')
  assert.equal(lines[0]?.persisted, true)
})

test('production expression units build fallback lines from moment script and content units', () => {
  const lines = buildExpressionUnitLines(
...
test('production expression units normalize write payloads and speaker choices', () => {
