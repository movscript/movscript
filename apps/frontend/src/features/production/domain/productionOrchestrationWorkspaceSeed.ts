...
import type { WorkspaceSegmentNode } from '@/features/production/domain/productionWorkspaceReviewModel'

export interface ProductionWorkspaceSeedEntity {
  ID: number
  UpdatedAt?: string
  [key: string]: unknown
}

export function buildProductionWorkspaceSeedMetadata(input: {
  projectId: number
  production?: (ProductionWorkspaceSeedEntity & { script_version_id?: number; name?: string }) | null
  productionSnapshot?: { segments: WorkspaceSegmentNode[] }
  ...
  return {
    ...
    data: {
      production: input.production ? summarizeWorkspaceSeedEntity(input.production) : null,
