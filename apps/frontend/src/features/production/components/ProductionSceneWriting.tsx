...
import { Check, Loader2, Plus, ScrollText, Trash2, Users, X } from 'lucide-react'
...
function EditableWritingExpressionLine({
  ...
  return (
    <ProductionExpressionLineShell
      ...
      badges={(
        <>
            <ProductionExpressionBadge variant="outline">{typeLabel}</ProductionExpressionBadge>
      ...
      preview={draft.text || textPlaceholderForWritingType(draft.kind)}
      meta={(draft.intent || draft.note) ? [draft.intent, draft.note].filter(Boolean).join(' · ') : undefined}
