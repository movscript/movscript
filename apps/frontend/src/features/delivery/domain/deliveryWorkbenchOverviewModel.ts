import type { DeliveryTimelineItem, DeliveryVersion, ExportRecord, Production } from '@/shared/infrastructure/api/deliveryEntities'
import {
  deliveryStatusLabel,
...
export type DeliveryOverviewMetricState =
  | 'version_inventory'
  | 'timeline_inventory'
  | 'delivery_gaps'
  | 'delivery_ready'
...
export interface DeliveryOverviewMetric {
  ...
  value: number
  detail: string
  state: DeliveryOverviewMetricState
}
...
export function buildDeliveryOverviewMetrics({
  ...
  return [
    ...
    },
    {
      ...
      state: versionReadiness.missingCount + versionReadiness.noResourceCount > 0 ? 'delivery_gaps' : 'delivery_ready',
    },
    {
      id: 'exports',
      label: '导出记录',
      ...
      state: exportRecords.length > 0 ? 'export_available' : 'not_exported',
    },
  ]
}
