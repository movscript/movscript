...
test('delivery workbench overview model builds stable metric view models', () => {
  ...
  assert.deepEqual(metrics.map((metric) => ({
    ...
    { id: 'versions', value: 2, detail: '1 个可导出', state: 'version_inventory' },
    { id: 'items', value: 2, detail: '0:10 总时长', state: 'timeline_inventory' },
    { id: 'missing', value: 2, detail: 'missing / needs_asset / 无资源', state: 'delivery_gaps' },
    { id: 'exports', value: 1, detail: '成功', state: 'export_available' },
  ])
})
