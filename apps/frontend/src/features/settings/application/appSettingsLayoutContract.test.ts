...
test('app settings exposes a native directory picker for MovScript Home', () => {
  ...
  const dialogPreloadSource = readFileSync(resolve(repoRoot, 'apps/frontend/electron/preload/api/dialog.ts'), 'utf8')
  const dialogIpcSource = readFileSync(resolve(repoRoot, 'apps/frontend/electron/ipc/dialogIpc.ts'), 'utf8')
