...
test('script version API uses workspace semantic repository instead of backend entity routes', () => {
  ...
  const semanticEntityConfigSource = readFileSync(resolve('src/shared/infrastructure/api/semanticEntityConfigs.ts'), 'utf8')
  ...
  assert.match(semanticEntityConfigSource, /scriptVersions: 'script_version'/)
})
