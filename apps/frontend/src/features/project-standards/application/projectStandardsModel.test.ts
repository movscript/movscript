...
function workspace(content: Record<string, unknown>): AgentWorkspace {
  return {
...
test('project standards model parses workspace diffs and style reference resources', () => {
  ...
  const workspaceWorkspace = workspace({
    workspace: {
      project_style: {
        aspect_ratio: '9:16',
  ...
  const rows = parseProjectStyleWorkspaceRows(workspaceWorkspace, current)
  ...
  assert.match(buildProjectStyleApplyPayload(workspaceWorkspace), /"project_style"/)
