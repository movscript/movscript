...
test('macOS dock shortcuts mirror desktop root and recent jump entries', () => {
  ...
  const editingListSource = readFileSync(resolve('src/pages/editing/EditingListPage.tsx'), 'utf8')
  const editingListControllerSource = readFileSync(resolve('src/features/editing/application/useEditingListController.ts'), 'utf8')
  const editingWorkspaceSource = readFileSync(resolve('src/pages/editing/EditingWorkspacePage.tsx'), 'utf8')
  const editingProjectPersistenceSource = readFileSync(resolve('src/features/editing/application/useEditingProjectPersistence.ts'), 'utf8')
  const editingProjectSaveSource = readFileSync(resolve('src/features/editing/application/editingProjectSave.ts'), 'utf8')
...
test('app window header exposes semantic icon regions and left control fill layout', () => {
  ...
  const projectEntryDeckHeaderSource = readFileSync(resolve('src/features/project/components/ProjectEntryDeckHeader.tsx'), 'utf8')
  const projectEntryDeckHeaderStyles = readFileSync(resolve('src/features/project/components/ProjectEntryDeckHeader.css'), 'utf8')
  ...
  assert.match(appSource, /const useProjectHomeHeaderControl = !!currentProject && \([\s\S]*projectChrome[\s\S]*agentChrome[\s\S]*settingsActive && \(settingsExitPath \?\? ''\)\.startsWith\(ROUTES\.project\.root\)/)
  ...
  assert.match(appSource, /navigationControls=\{<>\{navigationHomeControl\}\{settingsExitControl\}<\/>\}/)
  ...
  assert.match(appSource, /const settingsCenterHeader = \([\s\S]*navigationControls=\{settingsSidebarHidden \? <>\{navigationHomeControl\}\{settingsExitControl\}<\/> : undefined\}/)
  ...
  assert.match(appSource, /const agentNavigationControls = <>\{navigationHomeControl\}\{settingsExitControl\}<\/>/)
...
test('canvas list and tools are standalone page programs outside the main shell layout', () => {
  ...
  assert.match(editingShellSource, /<WorkspaceShell[\s\S]*surface="tool"/)
  assert.match(editingShellSource, /<RouteContentShell width="xwide">[\s\S]*<EditingListPage \/>/)
  assert.match(editingShellSource, /<EditingWorkspacePage \/>/)
