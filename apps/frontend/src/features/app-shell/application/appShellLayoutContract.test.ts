...
test('app sidebar resizing is owned by the shared layout controller', () => {
  ...
  assert.match(sidebarSource, /function openShortcutMenu\(/)
  assert.match(sidebarSource, /useAppShortcutOpenCommands\(\)/)
  assert.match(sidebarSource, /useAppShortcutRecentItems\(5\)/)
  assert.match(sidebarSource, /readRecentEditingProjectShortcuts\(5\)/)
  assert.match(sidebarSource, /shortcutCommands\.openProject/)
  assert.match(sidebarSource, /shortcutCommands\.openEditingProject/)
  assert.match(sidebarSource, /shortcutCommands\.openCanvas/)
  assert.match(sidebarSource, /id: 'app-home'[\s\S]*label: t\('sidebar\.shortcuts\.appHome'/)
  assert.match(sidebarSource, /id: 'tool-home'[\s\S]*label: t\('sidebar\.shortcuts\.toolHome'/)
  assert.match(sidebarSource, /id: 'edit-home'[\s\S]*label: t\('sidebar\.shortcuts\.editHome'/)
  assert.match(sidebarSource, /id: 'canvas-home'[\s\S]*label: t\('sidebar\.shortcuts\.canvasHome'/)
  assert.doesNotMatch(sidebarSource, /Project Home/)
  assert.match(sidebarSource, /item\.active[\s\S]*已打开/)
...
test('macOS dock shortcuts mirror desktop root and recent jump entries', () => {
  ...
  assert.match(editingWorkspaceSource, /pendingSaveRevisionRef/)
