...
test('buildAgentRunActivityTimeline creates one stable activity timeline for approvals inputs tools and generation events', () => {
  ...
  const activityTimeline = buildAgentRunActivityTimeline({ activity })

  assert.ok(activityTimeline)
  assert.equal(activityTimeline.stepCount, 1)
  assert.equal(activityTimeline.actionCount, 2)
  assert.equal(agentRunActivityTimelineSummary(activityTimeline), '1/1 个步骤 · 2 个交互')
  assert.deepEqual(activityTimeline.items.map((item) => item.id), [
    'approval-approval_1',
    'step-step_1',
    'input-input_1',
    'event-trace_generation',
  ])
  assert.equal(activityTimeline.items[0]?.type, 'approval')
  assert.equal(activityTimeline.items[0]?.statusLabel, '已同意')
  assert.equal(activityTimeline.items[1]?.title, '创建生成任务')
  assert.equal(activityTimeline.items[2]?.summary, '选择：a')
  assert.equal(activityTimeline.items[3]?.type, 'generation_job')
  assert.equal(activityTimeline.items[3]?.title, '生成任务 #42')
  assert.match(activityTimeline.items[3]?.summary ?? '', /100%/)
})
...
test('buildAgentRunActivityTimeline keeps approval and input trace events as action items', () => {
  ...
  const activityTimeline = buildAgentRunActivityTimeline({ events })

  assert.ok(activityTimeline)
  assert.equal(activityTimeline.actionCount, 2)
  assert.deepEqual(activityTimeline.items.map((item) => item.type), ['approval', 'input_request'])
  assert.equal(activityTimeline.items[0]?.statusLabel, '已同意')
  assert.equal(activityTimeline.items[1]?.statusLabel, '已回答')
})

test('buildAgentRunActivityTimeline deduplicates live events already present on activity', () => {
  const activity: ChatRunActivity = {
    runId: 'run_1',
    threadId: 'thread_1',
    status: 'in_progress',
  ...
  const activityTimeline = buildAgentRunActivityTimeline({
    activity,
    events: [{
      id: 'trace_1',
      kind: 'tool_call',
  ...
  assert.ok(activityTimeline)
  assert.deepEqual(activityTimeline.items.map((item) => item.id), ['event-trace_1'])
})
