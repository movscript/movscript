...
import { AGENT_CHAT_NOTIFICATION_EVENT_DISPATCH_COVERAGE } from '@/features/agent/domain/agentChatNotificationDispatchCoverage'
...
>(
  ...
  if (notification.method === 'turn/completed') {
    ...
    target.updatePendingServerRequests((current) => removeAgentChatPendingServerRequests(current, (pending) => {
...
>(
  ...
  if (event.type === 'threadLifecycle') {
    ...
    target.updatePendingServerRequests((current) => removeAgentChatPendingServerRequests(current, (item) => item.request.threadId === event.threadId))
    target.updateStreamingAgentItems((current) => {
      const next = { ...current }
      for (const [itemId, item] of Object.entries(current)) {
  ...
  if (event.type === 'serverRequestResolved') {
    target.updatePendingServerRequests((current) => removeAgentChatPendingServerRequests(current, (item) => agentChatPendingServerRequestMatchesResolvedEvent(item.request, event)))
    return
  }
