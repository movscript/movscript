...
} from '@/features/agent/domain/agentGenerationDisplay'
...
test('generationJobBadge maps terminal states to user-facing badges', () => {
  assert.deepEqual(generationJobBadge({ status: 'failed', terminal: true }), { label: '失败', state: 'failed' })
  assert.deepEqual(generationJobBadge({ status: 'cancelled', terminal: true }), { label: '已取消', state: 'cancelled' })
  assert.deepEqual(generationJobBadge({ status: 'running', stage: 'timeout', terminal: true }), { label: '超时', state: 'timeout' })
  assert.deepEqual(generationJobBadge({ status: 'succeeded', terminal: true }), { label: '完成', state: 'completed' })
  assert.deepEqual(generationJobBadge({ status: 'unknown', terminal: true }), { label: '已结束', state: 'ended' })
  assert.deepEqual(generationJobBadge({ status: 'running', terminal: false }), { label: '监控中', state: 'monitoring' })
})
