...
export function runHasRunInteraction(run: AgentRun | null | undefined): boolean {
  if (!run) return false
...
export function upsertInteractionRunSnapshot(current: AgentRun[], nextRun: AgentRun): AgentRun[] {
  return [...current.filter((run) => run.id !== nextRun.id), nextRun]
}

export function interactionRunsForChat(submittedRuns: AgentRun[], actionableRuns: AgentRun[] | AgentRun | null): AgentRun[] {
...
export function isRunInteractionAnswerEchoMessage(message: ChatMessage, echoes: Set<string>): boolean {
  if (message.role !== 'user') return false
  const content = message.content.trim()
  return echoes.has(content) || content.startsWith('[用户补充信息]')
}

export function runInteractionFromActivity(activity: ChatRunActivity | undefined): AgentRun | null {
