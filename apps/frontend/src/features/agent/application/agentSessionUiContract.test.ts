...
test('agent new conversation drafts bind selected project without starting a thread before send', () => {
  ...
  assert.match(agentModePageSource, /useState<AgentWorkspaceScopeSelection>\(\(\) => \(/)
  assert.match(agentModePageSource, /project\?\.ID \? \{ scope: 'project', projectId: project\.ID \} : \{ scope: 'global' \}/)
  assert.match(agentModePageSource, /setNewConversationWorkspaceScope/)
  assert.match(agentModePageSource, /选择新建会话范围/)
  assert.doesNotMatch(agentModePageSource, /setNewConversationWorkspaceScope\(\{ scope: 'global' \}\)/)
  assert.match(agentModePageSource, /openAgentPanelNewConversation\(\{[\s\S]*workspaceContext: newConversationWorkspaceContext/)
  assert.match(agentModePageSource, /sortedProjectOptions\.map/)
  assert.match(agentModePageSource, /item\.ID === project\?\.ID \? '当前项目'/)
  assert.match(agentModePageSource, /setNewConversationWorkspaceScope\(\{ scope: 'project', projectId: item\.ID \}\)/)
  ...
  assert.match(agentModePageSource, /selectedNewConversationProjectId !== undefined \? \{ projectId: selectedNewConversationProjectId \} : \{\}/)
