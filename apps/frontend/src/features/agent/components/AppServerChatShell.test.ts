...
test('agent chat active thread storage is owned by the presentation helper', () => {
  ...
  assert.match(dataSourceShellSource, /removeAgentConversationOpenRecords/)
  ...
  assert.match(dataSourceShellSource, /writeAgentConversationOpenState\(userId, removeAgentConversationOpenRecords\(readAgentConversationOpenState\(userId\), \[threadId\]\)\)/)
