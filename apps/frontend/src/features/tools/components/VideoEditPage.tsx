...
import {
  ...
  type VideoEditTrack,
} from '@/features/resources/domain/videoEditTimeline'
          ...
          <div className="flex items-center gap-1">
            {[
              ...
              <Button
                ...
                variant="outline"
                size="xs"
                className="h-7 gap-1 px-2 type-caption text-muted-foreground hover:text-foreground"
                title={title}
                aria-label={title}
  ...
  return (
    <div className="grid grid-cols-[160px_minmax(0,1fr)] items-stretch gap-2">
      <ToolVideoEditSurface className="flex items-center gap-1 px-1.5 type-label font-medium text-muted-foreground">
        ...
        <ToolVideoEditTrackControlButton
          state={track.muted ? 'danger' : 'default'}
        ...
        <Button
          type="button"
          variant={track.solo ? 'soft' : 'ghost'}
          size="xs"
          className="h-6 min-w-6 px-1 type-tiny text-muted-foreground hover:text-foreground"
        ...
        <Button
          ...
          variant="ghost"
          size="xs"
          className="h-6 min-w-0 flex-1 justify-start truncate px-1 text-left type-label text-muted-foreground hover:text-foreground"
...
function ClipMediaPreview({
  ...
  if (clip.kind === 'caption') {
    return <div className="tool-video-edit-media-grid" />
  }
  if (clip.kind === 'audio') {
    return <ToolVideoEditWaveform peaks={waveform ?? fallbackWaveform(clip.resourceId ?? clip.id.length)} />
  }
  if (thumbnail) {
    return (
...
function Inspector({
  ...
  return (
    <div className="space-y-4">
            ...
            {(selectedClip.kind === 'video' || selectedClip.kind === 'audio') && (
              <>
                ...
                <CheckboxField
                  ...
                  onCheckedChange={(checked) => onPatchClip({ muted: checked })}
                  className="h-auto justify-start gap-2 px-0 py-0 type-label text-muted-foreground"
                >
                  静音
                </CheckboxField>
              </>
            ...
            {selectedClip.kind === 'caption' && (
              <>
                <Textarea
                  ...
                  className="h-24 w-full resize-none px-3 py-2 type-body"
                />
      ...
      <ToolVideoEditPanel title="脚本粗剪" icon={Captions}>
        <Textarea
          value={scriptDraft}
          onChange={event => onScriptDraftChange(event.target.value)}
          className="h-32 w-full resize-none px-3 py-2 type-label leading-5"
...
function ToolbarButton({ icon, label, onClick, disabled }: { icon: ReactNode; label: string; onClick: () => void; disabled?: boolean }) {
  return (
    <Button
      ...
      variant="outline"
      size="sm"
      className="h-8 gap-1.5 px-2.5 type-label text-muted-foreground hover:text-foreground"
    >
      {icon}
      <span>{label}</span>
    </Button>
