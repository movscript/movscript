    ...
    <div className="flex h-full min-h-0 flex-col bg-background">
      ...
      <div className="grid min-h-0 flex-1 grid-cols-[minmax(280px,420px)_minmax(0,1fr)]">
        <aside className="flex min-h-0 flex-col border-r border-border">
          <div className="space-y-2 border-b border-border p-3">
            <input
              ...
              className="h-8 w-full rounded-md border border-input bg-background px-2 type-label outline-none focus-visible:ring-1 focus-visible:ring-ring"
            />
          ...
          <div className="min-h-0 flex-1 overflow-y-auto p-2">
            {draftsQuery.isLoading ? (
              <div className="flex items-center gap-2 rounded-md border border-border bg-muted/20 p-3 type-label text-muted-foreground">
                <Loader2 size={14} className="animate-spin" />
                {t('common.loading')}
              </div>
            ) : draftsQuery.error ? (
              <div className="rounded-md border border-destructive/30 bg-destructive/10 p-3 type-label text-destructive">
                {draftsQuery.error instanceof Error ? draftsQuery.error.message : String(draftsQuery.error)}
              </div>
            ) : drafts.length === 0 ? (
              <div className="rounded-md border border-border bg-muted/20 p-3 type-label text-muted-foreground">
              ...
              >
                <div className="flex items-start justify-between gap-2">
                  ...
                  <Badge variant={draftStatusVariant(draft.status)} className="shrink-0 type-micro">{t(`agents.chat.drafts.status.${draft.status}`)}</Badge>
                </div>
        ...
        <main className="min-h-0 overflow-y-auto p-4">
          {!selectedDraft ? (
            <div className="rounded-md border border-border bg-muted/20 p-4 type-body text-muted-foreground">
            ...
            <div className="space-y-3">
              <div className="rounded-md border border-border bg-background p-3">
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div className="min-w-0">
                    <h2 className="truncate type-body-lg font-semibold text-foreground">{selectedDraft.title}</h2>
              ...
              <section className="rounded-md border border-border bg-background">
                ...
                <pre className="max-h-[48vh] overflow-auto whitespace-pre-wrap break-words p-3 type-label leading-5 text-foreground">
...
function JSONPanel({ title, value }: { title: string; value: unknown }) {
  return (
    <div className="rounded-md border border-border bg-background">
      ...
      <pre className="max-h-52 overflow-auto whitespace-pre-wrap break-words p-3 type-caption leading-5 text-muted-foreground">
