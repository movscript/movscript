    ...
    <div className="project-agent-mode flex h-full min-h-0 flex-col overflow-hidden bg-background">
      {fullscreen && !embeddedInShell && (
        <div className="flex min-h-0 flex-1 overflow-hidden">
          ...
          <ProjectAgentChatSurface userId={userId} className="p-2.5" />
        </div>
      )}
      {(!fullscreen || embeddedInShell) && (
        <ProjectAgentChatSurface userId={userId} className="p-0" />
      )}
    </div>
  )
}
...
export function ProjectAgentModeSidebar() {
  ...
  return (
    <aside className="flex h-full w-72 shrink-0 flex-col overflow-hidden border-r border-sidebar-border bg-sidebar">
      <div className="shrink-0 space-y-1 border-b border-sidebar-border p-2">
        <button
          type="button"
          onClick={startNewConversation}
          className="flex h-9 w-full items-center gap-2 rounded-md px-2 type-body font-medium text-foreground transition-colors hover:bg-muted/60"
        >
        ...
        <NavLink
          ...
          className={({ isActive }) => cn(
            'flex h-9 w-full items-center gap-2 rounded-md px-2 type-body text-muted-foreground transition-colors hover:bg-muted/60 hover:text-foreground',
      ...
      <div className="min-h-0 flex-1 overflow-y-auto px-2 py-3">
        ...
        >
          {projectGroups.length === 0 ? (
            <p className="px-2 py-1.5 type-label text-muted-foreground">{t('agents.chat.agentModeSidebar.noProjectConversations')}</p>
          ) : (
      ...
      <div className="shrink-0 border-t border-sidebar-border p-2">
        <NavLink
          ...
          className={({ isActive }) => cn(
            'mb-1 flex h-9 w-full items-center gap-2 rounded-md px-2 type-body text-muted-foreground transition-colors hover:bg-muted/60 hover:text-foreground',
            isActive && 'bg-accent text-accent-foreground',
          )}
        >
        ...
        >
          <Avatar className="h-6 w-6 shrink-0">
            <AvatarFallback className="bg-muted type-label font-semibold text-muted-foreground">
              {currentUser?.username[0]?.toUpperCase() ?? <UserRound size={13} />}
            </AvatarFallback>
          </Avatar>
          <div className="min-w-0 flex-1">
            <p className="truncate type-label font-medium text-foreground">{currentUser?.username ?? t('agents.chat.agentModeSidebar.defaultUser')}</p>
  ...
  return (
    <section className="mb-3">
      <button
        ...
        className="mb-1 flex h-7 w-full items-center gap-2 rounded-md px-2 text-left type-label font-semibold text-muted-foreground transition-colors hover:bg-muted/50 hover:text-foreground"
        aria-expanded={open}
      >
        {icon}
  ...
  return (
    ...
    >
      ...
      <span className="min-w-0 flex-1">
        <span className="block truncate type-label font-medium leading-4">{title}</span>
