...
import { ArrowUpIcon, PaperclipIcon, StopIcon } from "@movscript/ui/primitives";
import { cn } from "@/shared/ui/cn";
import "./AgentComposerUi.css";

export const AgentComposer = React.forwardRef<HTMLFormElement, React.FormHTMLAttributes<HTMLFormElement>>(
  ({ className, ...props }, ref) => {
    return <form ref={ref} className={cn("ms-grid-stack ms-agent-composer", className)} {...props} />;
  },
);
...
export const AgentComposerToolbar = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => {
    return <div ref={ref} className={cn("ms-action-row ms-agent-cluster--between ms-agent-composer__toolbar", className)} {...props} />;
  },
);
