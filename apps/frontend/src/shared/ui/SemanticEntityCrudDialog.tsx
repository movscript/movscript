...
import { createSemanticEntity, deleteSemanticEntity, getSourceLockStatus, listSemanticEntities, semanticEntityConfig, updateSemanticEntity, type SemanticEntityConfig, type SemanticEntityPayload, type SemanticEntityRecord, type SourceLockStatus } from '@/shared/infrastructure/api/semanticEntities'
...
export function SemanticEntityCrudDialog({
  ...
  return (
    <DetailEntityDialogShell open={open} onOpenChange={onOpenChange} quickCreate={quickCreate} onSubmit={submit}>
      <DetailEntityDialogHeader
      ...
      <DetailEntityDialogBody>
        {config.requiredHint && mode === 'create' && !quickCreate ? (
          <DetailEntityRequiredHint>{config.requiredHint}</DetailEntityRequiredHint>
        ) : null}
        {sourceLock?.locked ? (
          <DetailEntitySourceLockNotice
            reason={sourceLockReason}
            fieldsText={sourceLock.locked_fields.map((key) => fieldLabel(fields, key)).join('、')}
            suffix="标题、描述、状态等非来源内容仍可继续编辑。"
          />
        ) : null}
        <DetailEntityFieldGrid columns={quickCreate ? 'single' : 'responsive'}>
        ...
        {advancedFields.length > 0 ? (
          <AppDisclosure
            ...
            className={quickCreate ? 'mt-4' : 'mt-5'}
            bodyClassName="detail-entity-field-grid"
            title={(
              <span className="flex min-w-0 items-center gap-2">
                ...
                <ChevronDown size={14} className={showAdvanced ? 'ml-auto shrink-0 rotate-180 transition-transform' : 'ml-auto shrink-0 transition-transform'} />
              </span>
            )}
          >
