...
export function codexServerRequestResponseFromAgentChat(
  ...
  if (request.method === 'item/tool/requestUserInput') {
    return {
      answers: response.action === 'answer' ? codexToolRequestUserInputAnswers(request, response) : {},
    }
  }
  if (request.method === 'item/tool/call') {
    return {
      contentItems: response.action === 'toolResult' ? codexDynamicToolOutputContentItems(response.contentItems) : [],
...
export function codexUserInputFromAgentChat(input: AgentChatInput): CodexUserInput {
  ...
  const path = codexMentionPathFromAgentChat(input)
  return { type: 'mention', name: codexMentionNameFromAgentChat(input, path), path }
}

function codexMentionPathFromAgentChat(input: Extract<AgentChatInput, { type: 'mention' }>): string {
  if (codexResourceMentionId(input.path) !== undefined) return input.path
  const url = nonEmptyString(input.url)
  if (agentChatMentionIsMedia(input) && url) return url
  return input.path
}

function codexMentionNameFromAgentChat(input: Extract<AgentChatInput, { type: 'mention' }>, path: string): string {
...
function codexResourceMentionId(path: string): number | undefined {
...
type CodexDynamicToolOutputContentItem = { type: 'inputText'; text: string } | { type: 'inputImage'; imageUrl: string }

function codexDynamicToolOutputContentItems(contentItems: unknown[] | undefined): CodexDynamicToolOutputContentItem[] {
  if (!Array.isArray(contentItems)) return []
  return contentItems.flatMap<CodexDynamicToolOutputContentItem>((item) => {
    if (!isRecord(item)) return []
    const text = codexDynamicToolOutputText(item)
    if (text) return [{ type: 'inputText', text }]
    const imageUrl = codexDynamicToolOutputImageUrl(item)
    if (imageUrl) return [{ type: 'inputImage', imageUrl }]
    const mediaReference = codexDynamicToolOutputMediaReference(item)
    if (mediaReference) return [{ type: 'inputText', text: mediaReference }]
    return []
  })
}

function codexDynamicToolOutputText(item: Record<string, unknown>): string | undefined {
  const type = stringField(item.type)
  if (type !== 'inputText' && type !== 'input_text' && type !== 'output_text' && type !== 'text') return undefined
  return nonEmptyString(item.text)
}

function codexDynamicToolOutputImageUrl(item: Record<string, unknown>): string | undefined {
  const type = stringField(item.type)
...
function codexDynamicToolOutputMediaReference(item: Record<string, unknown>): string | undefined {
  const type = stringField(item.type)
  if (type === 'inputVideo' || type === 'input_video' || type === 'video') {
    return codexDynamicToolOutputMediaReferenceText('Video result', item)
  }
  if (type === 'inputAudio' || type === 'input_audio' || type === 'audio') {
    return codexDynamicToolOutputMediaReferenceText('Audio result', item)
  }
  if (type === 'resource' || type === 'inputResource' || type === 'input_resource') {
    return codexDynamicToolOutputResourceReferenceText(item)
  }
  return undefined
}

function codexDynamicToolOutputMediaReferenceText(label: string, item: Record<string, unknown>): string {
  const details = [
...
function codexDynamicToolOutputResourceReferenceText(item: Record<string, unknown>): string {
...
function codexToolRequestUserInputAnswers(
  request: AgentChatServerRequest,
  response: Extract<AgentChatServerRequestResponse, { action: 'answer' }>,
): Record<string, { answers: string[] }> {
  const answers = codexToolRequestUserInputAnswersFromRecord(response.answers)
  ...
  const questionId = codexToolRequestUserInputQuestionId(request)
  return questionId ? { [questionId]: { answers: fallbackAnswers } } : answers
}

function codexToolRequestUserInputAnswersFromRecord(answers: unknown): Record<string, { answers: string[] }> {
...
function codexToolRequestUserInputQuestionId(request: AgentChatServerRequest): string | undefined {
  const params = isRecord(request.params) ? request.params : {}
