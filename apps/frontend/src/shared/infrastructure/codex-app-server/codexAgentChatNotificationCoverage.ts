import type { CodexServerNotification } from '@/shared/infrastructure/codex-app-server/codexAppServerProtocol'

export type CodexAgentChatNotificationHandling =
  | 'thread-state'
  | 'thread-item'
  | 'thread-item-notice'
  | 'capability-event'
...
export const CODEX_AGENT_CHAT_NOTIFICATION_COVERAGE: Record<CodexServerNotification['method'], {
  handling: CodexAgentChatNotificationHandling
  invalidationOwner?: 'agent-catalog' | 'thread-settings' | 'app-metadata'
  ...
  'skills/changed': { handling: 'metadata-invalidation', invalidationOwner: 'agent-catalog', note: 'Invalidates skill metadata; not a transcript item.' },
