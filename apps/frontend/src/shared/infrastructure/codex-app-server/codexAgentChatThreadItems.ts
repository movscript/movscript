  ...
  switch (item.type) {
    ...
    case 'commandExecution':
      return {
        ...
        commandActions: item.commandActions.map(agentChatCommandActionFromCodex),
...
function agentChatCommandActionFromCodex(action: Extract<CodexThreadItem, { type: 'commandExecution' }>['commandActions'][number]) {
  if (action.type === 'read') {
    return {
      type: action.type,
...
function agentChatInputFromCodex(input: CodexUserInput): AgentChatInput {
  ...
  return agentChatMentionInputFromCodex(input)
}
...
function agentChatMentionInputFromCodex(input: Extract<CodexUserInput, { type: 'mention' }>): AgentChatInput {
  const media = agentChatMentionMediaFromCodex(input)
  const mentionUrl = agentChatMentionUrlFromCodex(input.path, media)
  return {
    type: 'mention',
    name: agentChatMentionDisplayNameFromCodex(input.name),
    path: input.path,
...
function agentChatMentionUrlFromCodex(path: string, media: { kind: string; mimeType: string } | null): string | undefined {
  const resourceId = agentChatResourceMentionId(path)
  if (resourceId !== undefined) return `/api/v1/resources/${resourceId}/file`
  if (media) return agentChatRenderableMediaUrlFromCodexPath(path)
  return undefined
}

function agentChatMentionMediaFromCodex(input: Extract<CodexUserInput, { type: 'mention' }>): { kind: string; mimeType: string } | null {
  const explicitMimeType = agentChatMentionMimeTypeFromCodex(input)
  if (explicitMimeType) return { kind: explicitMimeType.split('/')[0] ?? 'resource', mimeType: explicitMimeType }
  const explicitKind = agentChatMentionKindHintFromCodex(input.name)
...
function agentChatMentionDisplayNameFromCodex(name: string): string {
  return name.replace(/\s+\[(?:image|video|audio)(?:\/[^\]\s]+)?\]$/i, '').trim() || name
}

function agentChatMentionMimeTypeFromCodex(input: Extract<CodexUserInput, { type: 'mention' }>): string | undefined {
...
function agentChatMentionKindHintFromCodex(name: string): string | undefined {
...
function agentChatRenderableMediaUrlFromCodexPath(path: string): string | undefined {
  const trimmed = path.trim()
