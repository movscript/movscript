...
test('runtime API catalog binds runtime backends to provider-neutral capability contracts', () => {
  ...
  assert.equal(mova.binaryPackageName, undefined)
  ...
  assert.equal(codexAppServer.binaryPackageName, '@movscript/mova-app-server')
  assert.equal(movaAppServer.binaryPackageName, '@movscript/mova-app-server')
