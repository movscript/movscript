...
export async function providerSessionResponseError(res: Response): Promise<ProviderSessionHTTPError> {
  const text = await res.text()
  const message = providerSessionErrorMessage(text)
  return new ProviderSessionHTTPError(res.status, text, message)
}

export async function providerSessionStreamError(stream: { status: number; responseText: () => Promise<string> }): Promise<ProviderSessionHTTPError> {
...
function providerSessionErrorMessage(text: string): string {
