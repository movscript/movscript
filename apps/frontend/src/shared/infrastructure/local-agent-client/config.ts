...
export function runtimeLocalAgentTransport(input: { agentRuntimeDirName?: string; workspaceDir?: string; sessionId?: string } = {}): AgentRuntimeTransport {
  return createAgentRuntimeTransport({
    ...(input.agentRuntimeDirName ? { agentRuntimeDirName: input.agentRuntimeDirName } : {}),
