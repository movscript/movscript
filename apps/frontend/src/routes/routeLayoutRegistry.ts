...
const routeLayoutRegistry: RouteLayoutRegistryEntry[] = [
  ...
  route({
    ...
    notes: 'Content orchestration uses the redesigned node canvas workspace.',
  }, exact(ROUTES.project.contentCanvas)),
  route({
    routeId: 'project.contentCanvasNext',
    pathnamePattern: ROUTES.project.contentCanvasNext,
    kind: 'page',
    surface: 'project',
    scrollMode: 'canvas',
    shellLayout: 'stacked',
    workbenchId: 'content_orchestration',
    panes: [...APP_SHELL_PROJECT_PANES, ...CONTENT_CANVAS_WORKBENCH_PANES],
    notes: 'Compatibility alias for the redesigned content canvas.',
  }, exact(ROUTES.project.contentCanvasNext)),
  route({
