...
const generationTools = [
  {
    ...
    description: 'List enabled MovScript generation models and versioned model contracts. Use this before image, video, audio, or subtitle generation to select a valid model_id and supported parameters.',
    inputSchema: objectSchema({
      capability: { type: 'string', description: 'Optional AI capability filter such as image, image_edit, video, video_i2v, video_v2v, audio_tts, audio_transcribe, audio_music, audio_sfx, subtitle_align, or subtitle_translate. Timeline render belongs to editing tools.' },
  ...
  {
    name: 'generation_content_unit_image_generate',
    description: 'Compile a content-unit backend prompt, submit an image generation job, and bind the job to automatic content-candidate creation when the job succeeds. Use this as the primary content-unit image generation path; generation_image_generate remains the low-level prompt channel.',
    inputSchema: objectSchema({
  ...
  {
    name: 'generation_content_unit_image_job_get',
    description: 'Fetch a content-unit image generation job and automatically create or refresh content-unit candidates for successful output_resource_ids.',
    inputSchema: objectSchema({
      jobId: { type: 'number', minimum: 1 },
      job_id: { type: 'number', minimum: 1 },
  ...
  {
    name: 'generation_content_unit_video_generate',
    description: 'Compile a content-unit backend prompt, submit a video generation job, and bind the job to automatic content-candidate creation when the job succeeds. Use this as the primary content-unit video generation path; generation_video_generate remains the low-level prompt channel.',
    inputSchema: objectSchema({
      contentUnitId: { type: ['string', 'number'], description: 'Target MovScript content unit id. The tool compiles this content unit prompt before generation.' },
      content_unit_id: { type: ['string', 'number'], description: 'Alias for contentUnitId.' },
      title: { type: 'string' },
  ...
  {
    name: 'generation_content_unit_video_job_get',
    description: 'Fetch a content-unit video generation job and automatically create or refresh content-unit candidates for successful output_resource_ids.',
    inputSchema: objectSchema({
      jobId: { type: 'number', minimum: 1 },
      job_id: { type: 'number', minimum: 1 },
      contentUnitId: { type: ['string', 'number'] },
  ...
  audioSubmitTool('generation_voiceover_generate', 'Submit a voiceover/text-to-speech AI generation job. Creates an audio RawResource when complete; it does not edit timelines or write candidates.'),
  audioSubmitTool('generation_music_generate', 'Submit an AI music generation job. Creates an audio RawResource when complete; it does not edit timelines or write candidates.'),
  audioSubmitTool('generation_sfx_generate', 'Submit an AI sound-effect generation job. Creates an audio RawResource when complete; it does not edit timelines or write candidates.'),
  audioSubmitTool('generation_subtitle_generate', 'Submit an audio/video transcription or subtitle generation job. Creates subtitle/timing resources when complete; it does not burn subtitles into video.'),
  audioSubmitTool('generation_subtitle_align', 'Submit a subtitle forced-alignment job. Aligns script/subtitle text to audio/video timing; it does not burn subtitles into video.'),
  audioSubmitTool('generation_subtitle_translate', 'Submit a subtitle translation job. Translates subtitle text while preserving timing metadata when supported.'),
  {
    name: 'generation_audio_job_get',
    description: 'Fetch the latest state of an audio, music, sound-effect, or subtitle AI generation job. Terminal results include output_resource_ids when available.',
    inputSchema: objectSchema({
      jobId: { type: 'number', minimum: 1 },
      job_id: { type: 'number', minimum: 1 },
      verbosity: { type: 'string', enum: ['summary', 'debug'] },
...
const queryTools = [
  ...
  },
  {
    ...
    description: 'Create a reusable MovScript image RawResource from an existing image resource by cropping, resizing, and/or re-encoding it. This is a neutral resource transform, not a domain candidate write.',
    inputSchema: objectSchema({
      resource_id: { type: 'number', description: 'MovScript image RawResource ID.' },
      resourceId: { type: 'number', description: 'Camel-case alias for resource_id.' },
      id: { type: 'number', description: 'Alias for resource_id.' },
      crop: { type: 'object', additionalProperties: true, description: 'Optional crop rectangle, usually x/y/width/height in source pixels.' },
      width: { type: 'number', description: 'Optional output width.' },
      height: { type: 'number', description: 'Optional output height.' },
      max_width: { type: 'number', description: 'Optional maximum output width.' },
      maxWidth: { type: 'number', description: 'Camel-case alias for max_width.' },
      max_height: { type: 'number', description: 'Optional maximum output height.' },
      maxHeight: { type: 'number', description: 'Camel-case alias for max_height.' },
      output_format: { type: 'string', enum: ['jpeg', 'png', 'webp'], description: 'Output image format.' },
      ...
      filename: { type: 'string', description: 'Optional output resource filename.' },
      name: { type: 'string', description: 'Alias for filename.' },
      folder_id: { type: 'string', description: 'Optional resource library folder ID.' },
      folderId: { type: 'string', description: 'Camel-case alias for folder_id.' },
      max_source_bytes: { type: 'number', description: 'Maximum source image file size to download.' },
      maxSourceBytes: { type: 'number', description: 'Camel-case alias for max_source_bytes.' },
      max_upload_bytes: { type: 'number', description: 'Maximum transformed upload size.' },
      maxUploadBytes: { type: 'number', description: 'Camel-case alias for max_upload_bytes.' },
    }),
  },
  {
    name: 'movscript_resource_video_probe',
    description: 'Probe a MovScript video RawResource and return technical metadata such as duration, dimensions, frame rate, codecs, streams, and container details.',
    inputSchema: objectSchema({
      resource_id: { type: 'number', description: 'MovScript video RawResource ID.' },
      resourceId: { type: 'number', description: 'Camel-case alias for resource_id.' },
      id: { type: 'number', description: 'Alias for resource_id.' },
      max_video_bytes: { type: 'number', description: 'Maximum source video file size to download for probing.' },
      maxVideoBytes: { type: 'number', description: 'Camel-case alias for max_video_bytes.' },
    }),
  },
  {
    name: 'movscript_resource_video_extract_frame_to_resource',
    description: 'Extract one frame from a MovScript video RawResource and upload it as a reusable image RawResource. This prepares source material; it does not edit a timeline.',
    inputSchema: objectSchema({
      resource_id: { type: 'number', description: 'MovScript video RawResource ID.' },
      resourceId: { type: 'number', description: 'Camel-case alias for resource_id.' },
      id: { type: 'number', description: 'Alias for resource_id.' },
      ...
      max_video_bytes: { type: 'number', description: 'Maximum source video file size to download.' },
      maxVideoBytes: { type: 'number', description: 'Camel-case alias for max_video_bytes.' },
      max_upload_bytes: { type: 'number', description: 'Maximum output image upload size.' },
      maxUploadBytes: { type: 'number', description: 'Camel-case alias for max_upload_bytes.' },
      max_width: { type: 'number', description: 'Maximum output frame width.' },
      maxWidth: { type: 'number', description: 'Camel-case alias for max_width.' },
      image_format: { type: 'string', enum: ['jpeg', 'png'], description: 'Output frame format.' },
      imageFormat: { type: 'string', enum: ['jpeg', 'png'], description: 'Camel-case alias for image_format.' },
      filename: { type: 'string', description: 'Optional output resource filename.' },
      name: { type: 'string', description: 'Alias for filename.' },
      folder_id: { type: 'string', description: 'Optional resource library folder ID.' },
  ...
  },
  {
    ...
    description: 'Extract multiple frames from a MovScript video RawResource and upload them as reusable image RawResources. Use this for generation references or review artifacts, not timeline editing.',
    inputSchema: objectSchema({
      resource_id: { type: 'number', description: 'MovScript video RawResource ID.' },
      resourceId: { type: 'number', description: 'Camel-case alias for resource_id.' },
      id: { type: 'number', description: 'Alias for resource_id.' },
      mode: { type: 'string', enum: ['overview', 'timestamps', 'range', 'burst'], description: 'Sampling mode.' },
      count: { type: 'number', description: 'Overview frame count.' },
      frame_count: { type: 'number', description: 'Alias for count.' },
      max_frames: { type: 'number', description: 'Maximum extracted frame count.' },
      maxFrames: { type: 'number', description: 'Camel-case alias for max_frames.' },
      timestamps_sec: { type: 'array', items: { type: 'number' }, description: 'Exact timestamps in seconds.' },
      timestampsSec: { type: 'array', items: { type: 'number' }, description: 'Camel-case alias for timestamps_sec.' },
      ...
      center_sec: { type: 'number', description: 'Center timestamp for burst sampling.' },
      centerSec: { type: 'number', description: 'Camel-case alias for center_sec.' },
      window_sec: { type: 'number', description: 'Window length in seconds for burst sampling.' },
      windowSec: { type: 'number', description: 'Camel-case alias for window_sec.' },
      fps: { type: 'number', description: 'Range/burst sampling frequency.' },
      interval_sec: { type: 'number', description: 'Sampling interval in seconds.' },
      intervalSec: { type: 'number', description: 'Camel-case alias for interval_sec.' },
      max_width: { type: 'number', description: 'Maximum output frame width.' },
      maxWidth: { type: 'number', description: 'Camel-case alias for max_width.' },
      image_format: { type: 'string', enum: ['jpeg', 'png'], description: 'Output frame format.' },
      imageFormat: { type: 'string', enum: ['jpeg', 'png'], description: 'Camel-case alias for image_format.' },
      filename_prefix: { type: 'string', description: 'Optional output filename prefix.' },
      filenamePrefix: { type: 'string', description: 'Camel-case alias for filename_prefix.' },
      folder_id: { type: 'string', description: 'Optional resource library folder ID.' },
      folderId: { type: 'string', description: 'Camel-case alias for folder_id.' },
      max_video_bytes: { type: 'number', description: 'Maximum source video file size to download.' },
      maxVideoBytes: { type: 'number', description: 'Camel-case alias for max_video_bytes.' },
      max_upload_bytes: { type: 'number', description: 'Maximum output image upload size per frame.' },
      maxUploadBytes: { type: 'number', description: 'Camel-case alias for max_upload_bytes.' },
    }),
  },
  {
    name: 'movscript_resource_video_trim_to_resource',
    description: 'Trim a segment from a MovScript video RawResource and upload it as a reusable video RawResource. This is a resource preparation tool, not the product editing/render path.',
    inputSchema: objectSchema({
      resource_id: { type: 'number', description: 'MovScript video RawResource ID.' },
      resourceId: { type: 'number', description: 'Camel-case alias for resource_id.' },
      id: { type: 'number', description: 'Alias for resource_id.' },
      start_sec: { type: 'number', description: 'Trim start timestamp in seconds.' },
      startSec: { type: 'number', description: 'Camel-case alias for start_sec.' },
      end_sec: { type: 'number', description: 'Trim end timestamp in seconds.' },
      endSec: { type: 'number', description: 'Camel-case alias for end_sec.' },
      duration_sec: { type: 'number', description: 'Trim duration in seconds.' },
      durationSec: { type: 'number', description: 'Camel-case alias for duration_sec.' },
      mode: { type: 'string', enum: ['accurate', 'fast'], description: 'Trim mode.' },
      volume: { type: 'number', description: 'Optional audio volume multiplier.' },
      muted: { type: 'boolean', description: 'When true, remove audio.' },
      filename: { type: 'string', description: 'Optional output resource filename.' },
      name: { type: 'string', description: 'Alias for filename.' },
      folder_id: { type: 'string', description: 'Optional resource library folder ID.' },
      folderId: { type: 'string', description: 'Camel-case alias for folder_id.' },
      max_video_bytes: { type: 'number', description: 'Maximum source video file size to download.' },
      maxVideoBytes: { type: 'number', description: 'Camel-case alias for max_video_bytes.' },
      max_upload_bytes: { type: 'number', description: 'Maximum output video upload size.' },
      maxUploadBytes: { type: 'number', description: 'Camel-case alias for max_upload_bytes.' },
    }),
  },
  {
    name: 'movscript_resource_video_contact_sheet_to_resource',
    description: 'Create a contact-sheet image from a MovScript video RawResource and upload it as a reusable image RawResource for review or generation reference.',
    inputSchema: objectSchema({
      resource_id: { type: 'number', description: 'MovScript video RawResource ID.' },
      resourceId: { type: 'number', description: 'Camel-case alias for resource_id.' },
      id: { type: 'number', description: 'Alias for resource_id.' },
      count: { type: 'number', description: 'Number of sampled frames.' },
      columns: { type: 'number', description: 'Contact-sheet column count.' },
      thumb_width: { type: 'number', description: 'Thumbnail width in pixels.' },
      ...
      max_video_bytes: { type: 'number', description: 'Maximum source video file size to download.' },
      maxVideoBytes: { type: 'number', description: 'Camel-case alias for max_video_bytes.' },
      max_upload_bytes: { type: 'number', description: 'Maximum contact-sheet upload size.' },
      maxUploadBytes: { type: 'number', description: 'Camel-case alias for max_upload_bytes.' },
    }),
  },
  {
    name: 'movscript_resource_video_extract_audio_to_resource',
    description: 'Extract audio from a MovScript video RawResource and upload it as a reusable audio RawResource. Use editing tools to place or mix audio on timelines.',
    inputSchema: objectSchema({
      resource_id: { type: 'number', description: 'MovScript video RawResource ID.' },
      resourceId: { type: 'number', description: 'Camel-case alias for resource_id.' },
      id: { type: 'number', description: 'Alias for resource_id.' },
      start_sec: { type: 'number', description: 'Optional extraction start timestamp in seconds.' },
      startSec: { type: 'number', description: 'Camel-case alias for start_sec.' },
      end_sec: { type: 'number', description: 'Optional extraction end timestamp in seconds.' },
      endSec: { type: 'number', description: 'Camel-case alias for end_sec.' },
      duration_sec: { type: 'number', description: 'Optional extraction duration in seconds.' },
      durationSec: { type: 'number', description: 'Camel-case alias for duration_sec.' },
      filename: { type: 'string', description: 'Optional output resource filename.' },
      name: { type: 'string', description: 'Alias for filename.' },
      folder_id: { type: 'string', description: 'Optional resource library folder ID.' },
      ...
      max_video_bytes: { type: 'number', description: 'Maximum source video file size to download.' },
      maxVideoBytes: { type: 'number', description: 'Camel-case alias for max_video_bytes.' },
      max_upload_bytes: { type: 'number', description: 'Maximum output audio upload size.' },
      maxUploadBytes: { type: 'number', description: 'Camel-case alias for max_upload_bytes.' },
    }),
  },
  {
  ...
  },
  {
    ...
    description: 'Upload multiple agent-created artifacts to the MovScript RawResource library in one call. Use returned resource IDs as generation inputs or references.',
    inputSchema: objectSchema({
      items: { type: 'array', items: { type: 'object', additionalProperties: true }, description: 'Upload items; each item may use local_path, artifact_path, workspace_path, data_url, base64, filename, mime_type, and folder_id.' },
      workspaceDir: { type: 'string', description: 'Optional MovScript workspace root directory. Defaults to the desktop workspace root.' },
      workspace_dir: { type: 'string', description: 'Alias for workspaceDir.' },
      max_bytes: { type: 'number', description: 'Maximum upload input size per item.' },
      maxBytes: { type: 'number', description: 'Camel-case alias for max_bytes.' },
    }),
  },
  {
...
const editingTools = [
  {
    ...
    description: 'Create an empty MovScript MediaEditingProject. This creates project data only; it does not render media or write content candidates.',
  ...
  },
  {
    ...
    description: 'Read a persisted MediaEditingProject by id. Requires the Electron editing runtime.',
    inputSchema: projectSchema({ ...workspaceLocatorProperties(), editingProjectId: { type: 'string' }, editing_project_id: { type: 'string' } }),
  },
  {
  ...
  },
  {
    ...
    inputSchema: projectSchema({ ...editingProjectInput, asset: { type: 'object', additionalProperties: true } }),
  },
  {
    ...
    inputSchema: projectSchema({ ...editingProjectInput, assetId: { type: 'string' }, asset_id: { type: 'string' } }),
  },
  {
    name: 'editing_project_save',
    description: 'Persist a MediaEditingProject revision in the Electron media workspace.',
    inputSchema: projectSchema({ ...editingProjectInput }),
  },
  {
    name: 'editing_timeline_apply_commands',
    description: 'Apply MovScript MediaTimelineCommand objects to a MediaEditingProject. This modifies project data only; it does not render media or create candidates.',
    inputSchema: projectSchema({
      ...editingProjectInput,
  ...
  },
  {
    ...
    inputSchema: projectSchema({ ...editingProjectInput, trackId: { type: 'string' }, track_id: { type: 'string' } }),
  },
  {
  ...
  },
  {
    ...
    inputSchema: projectSchema({ ...editingProjectInput, clipId: { type: 'string' }, clip_id: { type: 'string' }, patch: { type: 'object', additionalProperties: true } }),
  },
  {
  ...
  },
  {
    ...
    inputSchema: projectSchema({ ...editingProjectInput, clipId: { type: 'string' }, clip_id: { type: 'string' } }),
  },
  {
    name: 'editing_timeline_validate',
    description: 'Validate a MediaEditingProject timeline for missing assets, duplicate ids, invalid timing, overlaps, track mismatch, and missing subtitle references.',
    inputSchema: projectSchema({ ...editingProjectInput }),
  },
  {
  ...
  },
  {
    ...
    inputSchema: projectSchema({ ...editingProjectInput, output: { type: 'object', additionalProperties: true } }),
  },
  {
    name: 'editing_task_hls_create',
    description: 'Create an Electron mediaPipeline timeline_hls task. HLS packaging, including optional output.hlsVariants adaptive renditions, runs in Electron.',
    inputSchema: projectSchema({ ...editingProjectInput, output: { type: 'object', additionalProperties: true } }),
  },
  {
    ...
    inputSchema: projectSchema({ source: { type: 'object', additionalProperties: true }, output: { type: 'object', additionalProperties: true } }),
  },
  {
    name: 'editing_task_reframe_create',
    description: 'Create an Electron mediaPipeline media_reframe task for deterministic crop/contain/pad reframe. Pass projectId so the task is stored in the correct Electron media workspace.',
    inputSchema: projectSchema({ source: { type: 'object', additionalProperties: true }, output: { type: 'object', additionalProperties: true }, target: { type: 'string' }, mode: { type: 'string' } }),
  },
  {
    ...
    inputSchema: projectSchema({ taskId: { type: 'string' }, task_id: { type: 'string' } }),
  },
  {
...
const systemTools = [
  ...
  ...renameTools(generationTools, {
    ...
    generation_content_unit_image_generate: 'system_generate_content_unit_image',
    generation_image_job_get: 'system_generate_image_job_get',
    generation_content_unit_image_job_get: 'system_generate_content_unit_image_job_get',
    generation_image_job_get_batch: 'system_generate_image_job_get_batch',
    generation_video_generate: 'system_generate_video',
    generation_content_unit_video_generate: 'system_generate_content_unit_video',
    generation_video_job_get: 'system_generate_video_job_get',
    generation_content_unit_video_job_get: 'system_generate_content_unit_video_job_get',
...
const domainTools = [
  ...
  },
  {
    ...
    description: 'Query MovScript setting-state-owned asset slots such as front view, side view, turnaround sheet, material reference, voice timbre, or instrument tone, optionally including inline candidates.',
  ...
  },
  {
    ...
    description: 'Convert a production preview timeline plus selected scene_moment video candidates into a MediaEditingProject handoff. Product editing should use editing_project_create_from_edit_plan and editing_* tools for new workflows. Returns media_editing_project plus compose_inputs.',
    inputSchema: projectSchema({
      ...workspaceLocatorProperties(),
  ...
  },
  {
    ...
    description: 'Convert a scene_moment edit plan into a MediaEditingProject handoff. Product editing should use editing_project_create_from_edit_plan and editing_* tools for new workflows. Returns media_editing_project plus compose_inputs.',
    inputSchema: projectSchema({
      ...workspaceLocatorProperties(),
  ...
  ...[
    ['domain_upsert_project_standards', 'Create or update project-wide creative standards in source project_standards.json. Run domain_inspect, then domain_interpret when derived artifact tools need refreshed context.'],
    ['domain_upsert_setting', 'Create or update a MovScript setting source entity. A setting must be a concrete film/music production entity to make or reuse, not an abstract style/rule. Put the setting data to write in required payload; record/entity are optional existing-context objects only. Prefer this API over direct file edits for setting records.'],
    ['domain_upsert_asset', 'Create or update a MovScript asset slot source entity under a setting state. Assets describe one state asset such as front view, side view, turnaround sheet, material reference, voice timbre, or instrument tone; image assets should prefer plain white or very clean backgrounds. Put the asset data to write in required payload; record/entity are optional existing-context objects only. Store RawResource references by resource_id, not binaries or external URLs.'],
    ...
    ['domain_select_content_unit_candidate', 'Select a content candidate for a content unit through the backend decision API. Run domain_inspect and domain_interpret when derived artifact tools need refreshed context.'],
...
function audioSubmitTool(name, description) {
  return {
    name,
    description,
    inputSchema: objectSchema({
...
function generationJobGetBatchTool(name, description) {
  return {
    name,
    description,
    inputSchema: objectSchema({
...
function projectSchema(properties, required = []) {
  return objectSchema({ ...workspaceLocatorProperties(), ...properties }, required)
}
