...
- Name: agent-planner.spec.ts >> planner worker approval failure is visible on run detail
- Location: src/e2e/agent-planner.spec.ts:137:1
...
Call log:
  - waiting for getByTestId('agent-run-approval-action').filter({ hasText: 'Reject' })
