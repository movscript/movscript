import assert from 'node:assert/strict'
import test from 'node:test'
import { isToolVisibleForActiveBehavior } from './toolVisibility.js'
import type { ResolvedAgentSkill } from '../../../../state/shared/types.js'

test('isToolVisibleForActiveBehavior exposes only core management tools by default', () => {
  assert.equal(isToolVisibleForActiveBehavior({
    toolName: 'core_skill_update',
    activeSkills: [],
    userMessage: 'hello',
  }), true)
  assert.equal(isToolVisibleForActiveBehavior({
    toolName: 'core_update_plan',
    activeSkills: [],
    userMessage: '生成一个plan',
  }), true)
  assert.equal(isToolVisibleForActiveBehavior({
    toolName: 'movscript_focus_get',
    activeSkills: [],
    userMessage: 'hello',
  }), false)
})

test('isToolVisibleForActiveBehavior exposes command required tools only for matching commands', () => {
  assert.equal(isToolVisibleForActiveBehavior({
    toolName: 'core_work_start',
    activeSkills: [],
    userMessage: '/image rainy store',
  }), true)
  assert.equal(isToolVisibleForActiveBehavior({
    toolName: 'core_work_wait',
    activeSkills: [],
    userMessage: '/video rainy store',
  }), true)
  assert.equal(isToolVisibleForActiveBehavior({
    toolName: 'core_work_start',
    activeSkills: [],
    userMessage: 'generate image',
  }), false)
})

test('isToolVisibleForActiveBehavior scopes task tools to active hints', () => {
  const activeSkills = [buildSkill({
    id: 'skill.example',
    toolHints: ['studio.production_context'],
  })]

  assert.equal(isToolVisibleForActiveBehavior({
    toolName: 'studio.production_context',
    activeSkills,
    userMessage: 'make production workspace',
  }), true)
  assert.equal(isToolVisibleForActiveBehavior({
    toolName: 'studio.general_context',
    activeSkills,
    userMessage: 'make production workspace',
  }), false)
})

test('isToolVisibleForActiveBehavior exposes explicit skill tool refs and allows union task scope', () => {
  assert.equal(isToolVisibleForActiveBehavior({
    toolName: 'studio.production_context',
    activeSkills: [buildSkill({
      id: 'skill.base',
      toolGrants: ['studio.production_context'],
    })],
    userMessage: 'hello',
  }), true)

  assert.equal(isToolVisibleForActiveBehavior({
    toolName: 'studio.production_context',
    activeSkills: [buildSkill({
      id: 'skill.union',
      metadata: { toolScope: 'union' },
    })],
    userMessage: 'hello',
  }), true)
})

function buildSkill(input: Partial<ResolvedAgentSkill>): ResolvedAgentSkill {
  return {
    id: 'skill',
    name: 'Skill',
    description: '',
    enabled: true,
    instruction: '',
    resolvedPriority: 1,
    activationReason: 'default',
    compiledInstruction: '',
    warnings: [],
    ...input,
  }
}
