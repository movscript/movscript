import type { JSONValue, MCPResource, MCPTool } from '../../../state/shared/types.js'
import {
  findToolGrant,
  type AgentManifest,
...
import { defaultToolApproval, requiresToolApproval } from '../../permissions/approval/toolApprovalRules.js'
import { getToolAuthorizationUnavailableReason } from '../../permissions/authorization/toolAuthorization.js'
import { isToolVisibleForActiveBehavior } from '../visibility/behavior/toolVisibility.js'
import { normalizeToolExecutionMetadata, type ToolExecutionMetadata } from '../../registry/core/toolRegistry.js'
...
export function resolveToolCatalog(options: {
  ...
  for (const name of Array.from(names).sort()) {
    ...
    const unavailableReason = authorizationReason ?? (visible ? undefined : 'skill_scope')
    const source = mcpTool ? 'mcp' : registeredTool?.source === 'local' ? 'local' : registeredTool?.source === 'plugin' ? 'plugin' : 'runtime'
...
function activeSkillToolGrantIds(skills: ResolvedAgentSkill[], toolName: string): string[] {
  ...
  return skills
    .filter((skill) => {
      const grantMatches = (skill.toolGrants ?? []).some((ref) => publicToolName(ref.trim()) === normalizedToolName)
      const hintMatches = (skill.toolHints ?? []).some((hint) => publicToolName(normalizeToolHint(hint)) === normalizedToolName)
      return grantMatches || hintMatches
    })
    .map((skill) => skill.id)
    .sort()
}
