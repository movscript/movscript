import type { LocalReferenceChunk, LocalReferenceSearchResult } from '../shared/types.js'

export function searchReferenceChunks(input: {
  chunks: LocalReferenceChunk[]
  query?: string
  domain?: string
  tags?: string[]
  limit?: number
}): LocalReferenceSearchResult[] {
...
function scoreChunk(chunk: LocalReferenceChunk, queryTokens: string[], wantedTags: Set<string>): number {
  let score = 0
  const haystack = [
    chunk.id,
    chunk.domain,
