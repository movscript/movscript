...
test('server.ts stays a composition root and delegates runtime protocol projection', () => {
  ...
  assert.equal(serverSource.includes("from './serverRuntimeProtocol.js'"), true)
  assert.equal(serverSource.includes("from './serverRuntimeStreams.js'"), true)
  assert.equal(serverSource.includes("from './serverModelConfigRoutes.js'"), true)
})
