...
): Promise<{ content: string; trace: RuntimeModelHTTPTrace }> {
  const started = Date.now()
...
function parseJSONResponse(responseText: string): { choices?: Array<{ message?: { content?: string; tool_calls?: unknown[] } }> } | undefined {
  try {
    return JSON.parse(responseText) as { choices?: Array<{ message?: { content?: string; tool_calls?: unknown[] } }> }
  } catch {
    return undefined
  }
}

function extractAssistantContent(parsed: { choices?: Array<{ message?: { content?: string; tool_calls?: unknown[] } }> } | undefined): string | undefined {
