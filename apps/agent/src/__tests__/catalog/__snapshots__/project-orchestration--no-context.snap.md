...
Tool reference:
- Context: Read the current UI route, project, selection, user, resource summary, and draft count.
- Draft creation/editing: Create a local review draft or proposal draft for the selected schema kind. Patch, replace, validate, or dry-run preview_apply for one local draft.
