...
test('default model router exposes reasoning, text, and multimodal routes through the chat config fallback', () => {
  const routes = describeRuntimeModelCapabilities(CONFIG)

  assert.deepEqual(routes.map((route) => route.capability), ['reasoning', 'text', 'multimodal'])
