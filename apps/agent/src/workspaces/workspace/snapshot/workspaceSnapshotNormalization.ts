...
export function normalizeSettingWorkspaceSnapshotReferences(value: JSONValue[]): JSONValue[] {
  return value.map((item) => {
    if (!isJSONRecord(item)) return item
    const normalized: Record<string, JSONValue> = {}
    setNormalizedField(normalized, 'client_id', normalizedString(item.client_id) ?? normalizedString(item.workspace_client_id) ?? normalizedString(item.ClientID) ?? normalizedString(item.WorkspaceClientID))
...
export function normalizeAssetWorkspaceSnapshotSlots(value: JSONValue[]): JSONValue[] {
  return value.map(normalizeAssetWorkspaceSnapshotSlot)
}

export function normalizeAssetWorkspaceSnapshotSlot(item: JSONValue): JSONValue {
  if (!isJSONRecord(item)) return item
  const normalized: Record<string, JSONValue> = {}
  setNormalizedField(normalized, 'client_id', normalizedString(item.client_id) ?? normalizedString(item.ClientID) ?? normalizedString(item.workspace_client_id) ?? normalizedString(item.WorkspaceClientID))
  setNormalizedField(normalized, 'id', normalizedNumber(item.id) ?? normalizedNumber(item.ID))
...
function setNormalizedOwner(out: Record<string, JSONValue>, value: JSONValue | undefined): void {
  ...
  setNormalizedField(owner, 'client_id', normalizedString(value.client_id) ?? normalizedString(value.ClientID) ?? normalizedString(value.workspace_client_id) ?? normalizedString(value.WorkspaceClientID))
