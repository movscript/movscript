...
test('evaluateToolExecutionGate short-circuits user input requests before tool policy', () => {
...
test('evaluateToolExecutionGate exposes approval-required decisions for policy and pipeline callers', () => {
  const decision = evaluateToolExecutionGate([
  ...
  assert.equal(decision.policyResult.blockedToolCalls[0], decision.approvalBlockedToolCalls[0])
...
test('preflightToolExecutionPipeline owns pending approval materialization before graph pause', () => {
  ...
  assert.equal(preflight.policy.decision, 'approval_required')
  assert.equal(preflight.policy.approvalBlockedToolCalls[0]?.call.name, 'studio_write')
...
test('preflightToolExecutionPipeline owns skill activation repair checks', () => {
  ...
  assert.equal(preflight.kind === 'repair' ? preflight.policy.blockedToolCalls[0]?.call.name : undefined, 'movscript_script_locate')
  assert.equal(preflight.kind === 'repair' ? preflight.repairCalls[0]?.name : undefined, 'core_skill_update')
  assert.deepEqual(preflight.kind === 'repair' ? preflight.repairCalls[0]?.args?.load : undefined, ['movscript.workflow.script_reading'])
})

test('evaluateToolExecutionGate returns normalized allowed calls from the runtime policy', () => {
  const decision = evaluateToolExecutionGate([
