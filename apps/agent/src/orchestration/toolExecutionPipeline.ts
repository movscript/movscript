import { AgentFileSystem } from '../files/agentFileSystem.js'
...
import type { JSONValue, ResolvedAgentSkill, ResolvedToolCatalog, ToolCall } from '../state/types.js'
import type { BlockedToolCall, ToolPolicyResult } from '../tools/toolPolicy.js'
import { normalizeToolExecutionMetadata, type RegisteredTool, type ToolExecutionMetadata, type ToolRiskLevel } from '../tools/toolRegistry.js'
import type { ToolExecutionResult, ToolExecutorOptions } from './toolExecutor.js'
import { buildSkillActivationRepairCalls } from './agentGraphSkillActivationRepair.js'
import {
...
export interface ToolExecutionPipelinePreflightPolicyView {
...
export type ToolExecutionPipelinePreflightResult =
  | {
    ...
    policy: ToolExecutionPipelinePreflightPolicyView
...
export function preflightToolExecutionPipeline(input: {
  ...
  const policy = toolExecutionPipelinePreflightPolicyView(gate)
...
function toolExecutionPipelinePreflightPolicyView(gate: ToolExecutionGateDecision): ToolExecutionPipelinePreflightPolicyView {
  return {
    decision: gate.decision,
    policyResult: gate.policyResult,
...
export async function runToolExecutionPipeline(call: ToolCall, options: ToolExecutorOptions): Promise<ToolExecutionResult> {
  ...
  const policyGate = applyPipelinePolicyGate(effectiveCall, options)
  if (policyGate.status === 'blocked') {
    stages.push({
      name: 'policy_gate',
      status: 'failed',
      message: policyGate.message,
    })
    return {
      call: effectiveCall,
      error: `Tool execution blocked by policy: ${policyGate.message}`,
      errorData: {
  ...
  if (policyGate.status === 'allowed') {
    stages.push({
      name: 'policy_gate',
      status: 'completed',
      message: policyGate.message,
    })
    effectiveCall = policyGate.call
    args = normalizeToolArgs(effectiveCall)
  } else {
    stages.push({
      name: 'policy_gate',
      status: 'skipped',
      message: 'no policy gate context',
    })
  }
...
function applyPipelinePolicyGate(call: ToolCall, options: ToolExecutorOptions): {
  status: 'skipped'
} | {
  status: 'allowed'
  call: ToolCall
  ...
  if (!options.policyGate) return { status: 'skipped' }
  const gate = evaluateToolExecutionGate([call], {
    currentProjectId: options.policyGate.currentProjectId,
    manifest: options.policyGate.manifest,
    catalog: options.policyGate.catalog,
    registry: options.registry,
    approvedToolNames: options.policyGate.approvedToolNames,
    approvalMode: options.policyGate.approvalMode,
    sandboxMode: options.sandboxMode,
    runRole: options.policyGate.runRole,
  })
  ...
  if (allowedCall) {
    return {
      status: 'allowed',
      call: allowedCall,
      message: 'tool call satisfies runtime policy',
    }
  }
  ...
  return {
    ...
    message: blocked?.message ?? gate.warnings[0] ?? `${call.name} blocked by runtime policy`,
  }
}
