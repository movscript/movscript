...
import { buildModelToolResultRef } from '../contextManager/toolResultContext.js'
import { buildGenerationEvent, extractGenerationMonitorRequest } from '../generation/generationEvents.js'
...
import { buildAgentToolResultRecord, buildModelToolResultContextFromRecord } from '../state/toolResultStore.js'
import type { AgentGraphInput, AgentGraphTraceInput } from './agentGraphTypes.js'
import { normalizeToolCall } from './agentGraphForcedToolCalls.js'
import { monitorGenerationJob } from './agentGraphGenerationMonitor.js'
import { buildRollbackRecord } from './agentGraphRollbackPolicy.js'
import { recordToolResultContext } from './agentGraphToolContextTrace.js'
import {
  buildToolCompletedTrace,
  buildToolFailedTrace,
  buildToolReplayGuardTrace,
  buildToolResultDroppedTrace,
} from './agentGraphToolExecutionTrace.js'
import { findReplayableToolStep, toolReplayGuardData } from './toolReplayGuard.js'
...
export async function executeToolTurn(input: AgentGraphInput, options: {
  ...
  if (replay) {
    ...
    const modelToolResult = storedToolResult?.dropped === true
      ...
      : contextManager.buildToolResultContext({
        run: input.run,
        call,
        result,
  ...
  try {
    ...
    if (execResult.error) {
      ...
      const modelToolResult = contextManager.buildToolResultContext({
        run: input.run,
        call,
