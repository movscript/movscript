...
import type {
  ResourceFileDownloadAuthContext,
  ResourceFileDownloadPort,
  ResourceFileDownloadResult,
} from '../../ports/files/resourceDownloadPort.js'
import type { JSONValue } from '../../shared/protocol/types.js'

export interface BackendResourceFileDownloaderOptions {
  baseURL?: string
...
export interface BackendResourceFileDownloadErrorDetail {
  method: 'GET'
  path: string
  status: number
  responseText: string
...
export class BackendResourceFileDownloadHTTPError extends Error {
  readonly detail: BackendResourceFileDownloadErrorDetail

  constructor(message: string, detail: BackendResourceFileDownloadErrorDetail) {
    super(message)
    this.name = 'BackendResourceFileDownloadHTTPError'
    this.detail = detail
  }
}

export class BackendResourceFileDownloader implements ResourceFileDownloadPort {
  ...
  constructor(options: BackendResourceFileDownloaderOptions = {}) {
  ...
  private resolveBaseURL(auth?: ResourceFileDownloadAuthContext): string | undefined {
    return normalizeBaseURL(auth?.backendAPIBaseURL) ?? this.baseURL
  }
}

function resourceFileCacheKey(baseURL: string, resourceId: number, auth?: ResourceFileDownloadAuthContext): string {
...
function resourceFileCacheAuthScope(auth?: ResourceFileDownloadAuthContext): string {
  const userId = normalizeResourceFileAuthUserId(auth?.userId)
...
async function downloadResourceFileToCache(input: ResourceFileCacheDownloadInput): Promise<ResourceFileCacheMetadata> {
  ...
  if (!response.ok) {
    ...
    throw new BackendResourceFileDownloadHTTPError(`backend GET ${input.path} failed: HTTP ${response.status}${responseText ? ` ${responseText}` : ''}`, {
      method: 'GET',
      path: input.path,
...
function buildHeaders(auth?: ResourceFileDownloadAuthContext): Record<string, string> {
...
function normalizeResourceFileAuthUserId(value: unknown): number | string | undefined {
