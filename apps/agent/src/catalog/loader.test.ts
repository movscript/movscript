...
test('loads built-in MovScript platform catalog by default', () => {
  ...
  try {
    ...
    assert.ok(catalog.layeredSkills.some((skill) => skill.id === 'movscript.policy.workspace'))
    assert.ok(catalog.layeredSkills.some((skill) => skill.id === 'draft.policy.lifecycle'))
    assert.ok(catalog.layeredSkills.some((skill) => skill.id === 'kernel.workflow.proposal_first'))
    assert.ok(catalog.layeredSkills.some((skill) => skill.id === 'movscript.workflow.project_standards_proposal'))
...
test('loads built-in content unit proposal catalogs by default', () => {
  ...
  assert.ok(catalog.knowledgeCollections.some((collection) => collection.id === 'film.knowledge.storyboard'))
  assert.ok(catalog.layeredTools.some((tool) => tool.name === 'draft_create'))
  assert.ok(catalog.layeredTools.some((tool) => tool.name === 'draft_validate'))
