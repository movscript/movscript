...
每个制作节点使用 action: create | reuse | update；reuse/update 必须带已有实体 ID。`,
  skills: [
    skill({
      ...
      description: 'Draft production-level structure and storyboard proposals.',
      enabled: true,
      priority: 830,
      ...
      outputContract: 'Return the draft id, production id, project id when known, current draft status, and counts for segment, scene moment, content unit, and keyframe planning.',
      toolHints: [
        'movscript_get_current_context',
        'movscript_list_productions',
        'movscript_read_current_production',
...
const CONTENT_UNIT_SUGGEST_MANIFEST = manifest({
  ...
  version: '1.0.0',
  name: '内容单元建议助手',
  description: '基于当前情节上下文建议可创建的内容单元。',
  soul: `你是 MovScript 的内容单元建议助手。你的任务是基于当前选中的情节、制作上下文和用户补充，提出 3-6 条尚未创建的内容单元建议。
...
避免与已有内容单元重复；每条建议聚焦一个清晰动作、信息揭示、情绪节拍或转场功能。`,
  skills: [
    skill({
      ...
      description: 'Suggest missing content units for the selected scene moment.',
      enabled: true,
      priority: 770,
      appliesWhen: 'content-unit-suggest, content unit suggest, 内容单元建议, 镜头建议, 情节补镜头',
      instruction: 'Use the current work context, selected scene moment, existing content units, and user supplement to suggest missing content units. Prefer concise, production-ready entries and avoid duplicates.',
      outputContract: 'Return only JSON: {"units":[{"title":"...","kind":"shot|visual_segment|caption_card|narration|transition|music_beat|product_showcase","description":"...","prompt":"...","duration_sec":number,"shot_size":"...","camera_angle":"...","camera_motion":"..."}]}. Optional fields may be omitted.',
