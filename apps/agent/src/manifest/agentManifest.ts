...
export const DEFAULT_AGENT_MANIFEST: AgentManifest = {
  ...
  description: 'Default local agent with project read and page-owned draft editing permissions.',
  skills: [
    {
      id: 'movscript.default.safe-project-assistant',
      name: 'Safe Project Assistant',
      description: 'Read project info and edit page-owned local draft review artifacts without changing existing project data.',
      enabled: true,
      priority: 100,
      instruction: '优先读取当前项目信息；涉及剧本事实、正文、拆分或编排时，先使用 movscript_read_project_scripts 查看当前项目剧本，再判断是否缺少剧本。需要产出或修改内容时，只创建或编辑本地审阅草稿，不直接修改既有正式项目内容。正式写入、应用、生成任务和项目创建必须由 UI 或明确审批流程接管。',
      toolHints: ['movscript_get_context_pack', 'movscript_list_projects', 'movscript_read_project_scripts', 'movscript_get_draft', 'movscript_list_drafts', 'movscript_update_draft', 'movscript_patch_draft', 'movscript_validate_draft', 'movscript_list_memories', 'movscript_search_memories', 'movscript_get_memory', 'movscript_create_memory', 'movscript_delete_memory'],
    },
  ],
  permissions: [
    'project.read',
