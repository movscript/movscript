...
test('loads bundled MovScript platform catalog by default', () => {
  ...
  try {
    ...
    assert.ok(catalog.manifest.skills.some((skill) => skill.id === 'movscript.intent.content-unit-proposal'))
    assert.ok(catalog.manifest.skills.some((skill) => skill.id === 'movscript.intent.content-unit-media-proposal'))
...
test('loads bundled content unit proposal catalogs by default', () => {
  const catalog = loadAgentPluginCatalog()

  const contentUnitProposalBundle = catalog.bundles.find((bundle) => bundle.id === 'movscript.bundle.content-unit-proposal')
  const contentUnitMediaBundle = catalog.bundles.find((bundle) => bundle.id === 'movscript.bundle.content-unit-media-proposal')

  assert.ok(contentUnitProposalBundle)
  assert.ok(contentUnitMediaBundle)
  assert.ok(contentUnitProposalBundle?.tools.includes('movscript_create_draft'))
  assert.ok(contentUnitProposalBundle?.tools.includes('movscript_read_draft'))
  assert.ok(contentUnitProposalBundle?.tools.includes('movscript_update_draft'))
  assert.ok(contentUnitMediaBundle?.tools.includes('movscript_create_draft'))
  assert.ok(contentUnitMediaBundle?.tools.includes('movscript_read_draft'))
  assert.ok(contentUnitMediaBundle?.tools.includes('movscript_update_draft'))
  assert.ok(catalog.tools.some((tool) => tool.name === 'movscript_create_draft'))
  ...
  assert.equal(catalog.bundles.find((bundle) => bundle.id === 'movscript.bundle.production-proposal'), undefined)
  assert.equal(catalog.bundles.find((bundle) => bundle.id === 'movscript.bundle.script-split'), undefined)
