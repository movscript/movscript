...
test('buildContext renders planner subagent Skill when runtime layers activate it', () => {
  const baseInput = {
    manifest: DEFAULT_AGENT_MANIFEST,
    context: {
      route: { pathname: '/agent' },
  ...
  assert.equal(withoutSubagents.debugParts.some((part) => part.id === 'skill.core.subagent_planning'), false)
  ...
  const policy = withSubagents.debugParts.find((part) => part.id === 'skill.core.subagent_planning')
  ...
  const plannerPolicy = withPlannerIntent.debugParts.find((part) => part.id === 'skill.core.subagent_planning')
...
test('buildContext orders activated behavior by priority rather than authoring tag', () => {
  const built = buildContext({
    manifest: DEFAULT_AGENT_MANIFEST,
    skills: [
      {
...
test('buildContext lets manifest prompt options disable project standards guidance', () => {
  const built = buildContext({
    manifest: {
      ...DEFAULT_AGENT_MANIFEST,
      metadata: {
...
test('buildContext lets manifest prompt options provide custom project standards guidance', () => {
  const built = buildContext({
    manifest: {
      ...DEFAULT_AGENT_MANIFEST,
      metadata: {
...
test('buildContext degrades oversized prompts using manifest prompt limit', () => {
  ...
  assert.equal(built.debugParts.some((part) => part.id === 'skill.test.example'), true)
