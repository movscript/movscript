...
test('compactPromptHistory keeps recent messages and summarizes older continuity', () => {
  ...
  assert.equal(compacted.inputCount, 8)
  assert.equal(compacted.retainedCount, 3)
  assert.equal(compacted.filteredCount, 0)
  assert.equal(compacted.projectionDecisions[0]?.stage, 'history_window')
  assert.equal(compacted.projectionDecisions[0]?.messageCount, 5)
...
test('thread context summary keeps refs and prompt compaction renders persisted summary', () => {
  ...
  const summary = buildThreadContextSummary({
    ...
    run: {
      ...
      metadata: {
        contextLedger: {
          retrieved: [{
            ref: { type: 'reference', id: 'storyboard.rhythm.basic', title: '分镜节奏基础' },
            source: 'reference',
            evidence: 'advisory',
            title: '分镜节奏基础',
          ...
          artifactRefs: [{ type: 'reference', id: 'storyboard.rhythm.basic', title: '分镜节奏基础' }],
        },
      },
  ...
  assert.equal(compacted.projectionDecisions.some((decision) => decision.stage === 'thread_summary'), true)
...
test('thread context summary rejects old schema records instead of migrating them', () => {
  const oldSummary = {
    ...
    recentRunRefs: [{
      ...
      summary: 'old summary',
      artifactRefs: [],
      retrievedRefs: [{ type: 'reference', id: 'storyboard.rhythm.basic' }],
    }],
  }
