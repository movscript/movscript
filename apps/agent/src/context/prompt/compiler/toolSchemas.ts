...
const CREATE_WORKSPACE_TOOL_SCHEMA = {
  ...
  properties: {
    kind: {
      type: 'string',
      description: 'Business workspace kind. Concrete values and semantics come from the active workspace MCP contract.',
    },
    title: { type: 'string', description: 'Optional. Auto-generated from kind + project when omitted for workspace workspaces.' },
    content: { type: 'string', description: 'Optional initial workspace content. Prefer omitting it when MCP can hydrate initial content. After creation, edit agent://workspace/{workspaceId}/content with standard file tools instead of replacing content through workspace tools.' },
