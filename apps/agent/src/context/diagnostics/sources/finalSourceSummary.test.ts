...
test('final source summary renders ledger refs by source type', () => {
  const summary = buildFinalSourceSummary({
    run: testRun({
      contextLedger: {
        retrieved: [
          retrieved('reference', 'storyboard.rhythm.basic', 'reference', 'advisory', '分镜节奏基础'),
  ...
  assert.match(summary, /参考资料：reference#storyboard\.rhythm\.basic《分镜节奏基础》（source=reference; evidence=advisory）/)
})

test('final source summary is appended once to model content', () => {
  const run = testRun({
    contextLedger: {
      retrieved: [retrieved('reference', 'storyboard.hook.short_drama', 'reference', 'advisory', '短剧钩子')],
    },
  })
...
test('final source summary can stand alone when model content is empty', () => {
  const run = testRun({
    contextLedger: {
      retrieved: [retrieved('reference', 'storyboard.rhythm.basic', 'reference', 'advisory', '分镜节奏基础')],
    },
  })
...
test('final source summary omits large reference bodies from final content', () => {
  ...
  assert.match(content, /已省略 reference 正文：reference#storyboard\.rhythm\.basic《分镜节奏基础》/)
  assert.match(content, /参考资料：reference#storyboard\.rhythm\.basic《分镜节奏基础》（source=reference; evidence=advisory）/)
})

test('final source summary ignores non-plain tool result objects when omitting reference bodies', () => {
  const body = '分镜节奏正文。'.repeat(80)
  const run = testRun({
    contextLedger: {
      retrieved: [retrieved('reference', 'storyboard.rhythm.basic', 'reference', 'advisory', '分镜节奏基础')],
  ...
  assert.doesNotMatch(content, /已省略 reference 正文/)
})
