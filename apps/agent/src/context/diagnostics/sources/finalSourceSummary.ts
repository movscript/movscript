...
import { buildRetrievedContextStore, uniqueRetrievedContextRefs } from '../../ledger/retrieval/retrievedContextStore.js'
import type { ContextRef } from '../../ledger/shared/contextLedgerTypes.js'

const MIN_OMITTED_REFERENCE_BODY_CHARS = 400
...
export function appendFinalSourceSummary(content: string, input: BuildFinalSourceSummaryInput): string {
  ...
  const bounded = omitLargeReferenceBodies(trimmed, input.run)
...
export function renderFinalAssistantContent(content: string, input: Pick<BuildFinalSourceSummaryInput, 'run'>): string {
  return omitLargeReferenceBodies(content.trim(), input.run)
}
...
function finalSourceLines(input: BuildFinalSourceSummaryInput): string[] {
  ...
  if (referenceRefs.length > 0) {
    lines.push(formatSourceLine({
      label: '参考资料',
      refs: referenceRefs,
      fallback: 'reference tool result',
      records,
      defaultSources: ['reference'],
...
function omitLargeReferenceBodies(content: string, run: AgentRun | undefined): string {
  if (!run) return content
  let bounded = content
  for (const reference of loadedReferenceBodies(run)) {
    if (reference.body.length < MIN_OMITTED_REFERENCE_BODY_CHARS) continue
    if (!bounded.includes(reference.body)) continue
    const ref = `reference#${reference.id}${reference.title ? `《${reference.title}》` : ''}`
    bounded = bounded.split(reference.body).join(`[已省略 reference 正文：${ref}；请通过来源引用或读取工具查看。]`)
  }
  return bounded
}
