...
export interface BuildVolatileTraceEventInput {
  id: string
  run: AgentRun
  now: string
  kind: AgentTraceEventKind
...
export function buildVolatileTraceEvent(input: BuildVolatileTraceEventInput): AgentTraceEvent {
  return {
    id: input.volatileKey ? `trace_live_${input.volatileKey}` : input.id,
    runId: input.run.id,
    kind: input.kind,
    title: input.title,
    status: input.status,
