...
test('buildRuntimeTraceDebugView summarizes model context, tools, pending actions, and attention events', () => {
  ...
  const events: AgentTraceEvent[] = [
      ...
      skillOmissions: [
        {
          ...
          reason: 'Task skill trigger did not match the current request/context.',
        },
      ],
    }),
    trace('trace_00', 'context', 'Context ledger updated before prompt', {
      ...
      mutationSummary: {
        ...
        amended: 0,
        deleted: 0,
        affectedContextKeys: ['reference:brief:sha256:pre'],
        appendedContextKeys: ['reference:brief:sha256:pre'],
        amendedContextKeys: [],
        deletedContextKeys: [],
        latest: {
          id: 'ctx_mut_pre_1',
      ...
      refs: [{
        key: 'reference:brief:sha256:pre',
        type: 'reference',
        id: 'brief',
        status: 'active',
        hash: 'sha256:pre',
    ...
    trace('trace_10', 'context', 'Round 1: context projection updated', {
      ...
      historicalVisualProjection: {
        ...
        metadataOnlyCount: 0,
        droppedCount: 1,
        decisions: [{ action: 'retain', stage: 'historical_visual_context', reason: 'rebuilt image payload', resourceId: 7 }],
      },
      attachmentProjection: {
        attachmentCount: 1,
        inlineImageCount: 0,
        metadataOnlyCount: 1,
        decisions: [{ action: 'retain', stage: 'user_attachments', reason: 'metadata' }],
      },
    }),
    ...
    trace('trace_8', 'permission', 'Turn 1: tool permission result', {
  ...
  const view = buildRuntimeTraceDebugView({
    ...
    summary: {
      runId: run.id,
      total: events.length,
      byKind: { prompt: 1, model_call: 2, assistant: 1, skill: 2, tool_call: 2, permission: 1, approval: 1, context: 3 },
