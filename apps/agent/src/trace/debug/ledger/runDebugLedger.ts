import type { AgentRun, AgentRunStatus, AgentTraceEvent } from '../../../state/shared/types.js'
import {
  arrayValue,
  clone,
  jsonLength,
  ...
  uniqueBy,
  uniqueStrings,
} from './run-debug-ledger/values.js'
...
export interface AgentRunDebugLedger {
  ...
  context: {
    ...
    droppedSummary: {
      ...
      samples: Array<{ eventId: string; originalChars: number; renderedChars: number; reason?: string; resultHash?: string; refKey?: string }>
    }
...
export interface AgentRunDebugLedgerDecision {
  eventId: string
  kind: 'permission' | 'approval' | 'input' | 'skill' | 'context'
  summary: string
  impact?: string
}
...
export interface FindRunDebugEvidenceRefsInput {
...
export function applyTraceEventToDebugLedger(input: {
  ...
  if (eventType === 'context.prompt_too_long_recovery') applyPromptTooLongRecoveryEvent(next, input.event, data)
  if (eventType === 'context.ledger_updated' || eventType === 'context.run_built') {
    ...
    if (eventType === 'context.ledger_updated') applyContextLedgerUpdatedEvent(next, input.event, data)
  }
  ...
  if (input.event.kind === 'assistant' || input.event.kind === 'message') applyMessageEvent(next, input.event, data)
  if (input.event.kind === 'permission') {
    addDecision(next, {
      eventId: input.event.id,
      kind: 'permission',
      summary: input.event.summary ?? input.event.title,
      impact: input.event.status === 'blocked' ? '工具执行被权限规则阻塞或需要审批。' : '工具执行权限规则已判定。',
    })
  }
...
export function resolveRunDebugEvidence(input: {
  ...
  switch (parsed.kind) {
    ...
    case 'tool_args':
      ...
      break
    case 'tool_result':
      value = toolResultSummary(data) ?? data
      break
    case 'raw_event':
      value = event
      break
...
export function findRunDebugEvidenceRefs(input: FindRunDebugEvidenceRefsInput): AgentRunDebugLedgerEvidenceRef[] {
  const hasSelector = Boolean(input.contextBundleId || input.refKey || input.contentHash || input.resultHash)
  if (!hasSelector) return []
  return input.ledger.evidenceIndex.filter((ref) => {
    if (input.kind && ref.kind !== input.kind) return false
    if (input.contextBundleId && !ref.contextBundleIds?.includes(input.contextBundleId)) return false
    if (input.refKey && !ref.refKeys?.includes(input.refKey)) return false
    if (input.contentHash && !ref.contentHashes?.includes(input.contentHash)) return false
    if (input.resultHash && !ref.resultHashes?.includes(input.resultHash)) return false
    return true
  })
}

function applyMessageEvent(ledger: AgentRunDebugLedger, event: AgentTraceEvent, data: Record<string, unknown> | undefined): void {
  if (!stringValue(data?.contentHash)) return
...
function applyPromptEvent(ledger: AgentRunDebugLedger, event: AgentTraceEvent, data: Record<string, unknown> | undefined): void {
  ...
  const budgetDecisions = arrayValue(budgetLedger?.decisions) ?? []
  for (const decision of budgetDecisions) {
    const record = recordValue(decision)
    const action = stringValue(record?.action)
    const partId = stringValue(record?.partId)
    if (!partId || (action !== 'drop' && action !== 'strip_examples')) continue
    const originalChars = numberValue(record?.originalChars) ?? 0
    const renderedChars = numberValue(record?.renderedChars) ?? 0
    const reason = stringValue(record?.reason)
    const stage = stringValue(record?.stage) ?? 'unknown'
    ...
    addDecision(ledger, {
      eventId: event.id,
      kind: 'context',
      summary: `${action === 'drop' ? '丢弃' : '压缩'}上下文片段 ${partId}`,
      impact: `预算决策 ${stage}：${originalChars} -> ${renderedChars} 字符。`,
    })
  }
  addDecision(ledger, {
    eventId: event.id,
  ...
  if (data?.contextBundleRef !== undefined || data?.contextBundleId !== undefined) {
    addEvidence(ledger, buildEvidenceRef({
      event,
      kind: 'raw_event',
      label: '上下文 Bundle 引用',
...
function applyPromptTooLongRecoveryEvent(ledger: AgentRunDebugLedger, event: AgentTraceEvent, data: Record<string, unknown> | undefined): void {
  const droppedHistoryMessageCount = numberValue(data?.droppedHistoryMessageCount) ?? 0
  const summaryChars = numberValue(data?.summaryChars) ?? 0
  ledger.context.droppedSummary.count += droppedHistoryMessageCount
  ledger.context.droppedSummary.totalRenderedChars += summaryChars
  ledger.context.droppedSummary.samples.push({
    eventId: event.id,
    originalChars: 0,
    renderedChars: summaryChars,
    reason: 'prompt.too_long.recovery',
  })
  addDecision(ledger, {
    eventId: event.id,
    kind: 'context',
    summary: event.summary ?? 'Prompt-too-long recovery collapsed thread history.',
    impact: `${droppedHistoryMessageCount} 条历史消息折叠为 ${summaryChars} 字符摘要后重试模型调用。`,
  })
}
...
function applyDroppedContextEvent(ledger: AgentRunDebugLedger, event: AgentTraceEvent, data: Record<string, unknown> | undefined): void {
  ...
  ledger.context.droppedSummary.samples.push({
    ...
    ...(stringValue(data?.resultHash) ? { resultHash: stringValue(data?.resultHash) } : {}),
    ...(stringValue(data?.refKey) ? { refKey: stringValue(data?.refKey) } : {}),
  })
  if (stringValue(data?.resultHash) || stringValue(data?.refKey)) {
    addEvidence(ledger, buildEvidenceRef({
      event,
      kind: 'raw_event',
      label: '上下文裁剪结果引用',
...
function applyContextLedgerUpdatedEvent(ledger: AgentRunDebugLedger, event: AgentTraceEvent, data: Record<string, unknown> | undefined): void {
  const mutationSummary = recordValue(data?.mutationSummary)
  const refs = arrayValue(data?.refs) ?? []
  if (!mutationSummary && refs.length === 0) return
  addEvidence(ledger, buildEvidenceRef({
    event,
    kind: 'raw_event',
    label: '上下文 Ledger 变动摘要',
    value: {
      mutationSummary,
      refs,
      retrievedCount: data?.retrievedCount,
      activeCount: data?.activeCount,
      amendedCount: data?.amendedCount,
      deletedCount: data?.deletedCount,
      artifactRefCount: data?.artifactRefCount,
      refsTruncated: data?.refsTruncated,
    },
  }))
}
...
function applyToolCallEvent(ledger: AgentRunDebugLedger, event: AgentTraceEvent, data: Record<string, unknown> | undefined): void {
  ...
  const resultSummary = toolResultSummary(data)
  const resultRef = resultSummary ? evidenceId(event, 'tool_result') : undefined
...
function toolResultSummary(data: Record<string, unknown> | undefined): Record<string, unknown> | undefined {
  if (!data) return undefined
  if (data.result !== undefined) return { result: data.result }
  const resultHash = stringValue(data.resultHash)
  if (!resultHash) return undefined
  return {
    resultHash,
    ...(numberValue(data.resultChars) !== undefined ? { resultChars: numberValue(data.resultChars) } : {}),
    ...(stringValue(data.resultMode) ? { resultMode: stringValue(data.resultMode) } : {}),
    ...(recordValue(data.generation) ? { generation: recordValue(data.generation) } : {}),
  }
}
...
function buildEvidenceRef(input: {
  ...
  const data = recordValue(input.event.data)
  const links = traceEvidenceLinks(data, input.value)
  return {
    ...
    ...(links.refKeys.length > 0 ? { refKeys: links.refKeys } : {}),
    ...(links.contentHashes.length > 0 ? { contentHashes: links.contentHashes } : {}),
    ...(links.resultHashes.length > 0 ? { resultHashes: links.resultHashes } : {}),
    ...(links.contextBundleIds.length > 0 ? { contextBundleIds: links.contextBundleIds } : {}),
  }
}

function traceEvidenceLinks(data: Record<string, unknown> | undefined, value: unknown): {
  refKeys: string[]
  contentHashes: string[]
  resultHashes: string[]
  contextBundleIds: string[]
  ...
  return {
    refKeys: uniqueStrings([
      stringValue(data?.refKey),
      stringValue(valueRecord?.refKey),
      ...contextRefKeys(data?.contextRefs),
      ...contextRefKeys(data?.refs),
      ...contextMutationKeys(data?.mutationSummary),
      ...contextRefKeys(valueRecord?.contextRefs),
      ...contextRefKeys(valueRecord?.refs),
      ...contextMutationKeys(valueRecord?.mutationSummary),
    ]),
    contentHashes: uniqueStrings([
      stringValue(data?.contentHash),
      stringValue(valueRecord?.contentHash),
    ].filter((item): item is string => !!item)),
    resultHashes: uniqueStrings([
      stringValue(data?.resultHash),
      stringValue(valueRecord?.resultHash),
    ].filter((item): item is string => !!item)),
    contextBundleIds: uniqueStrings([
      stringValue(data?.contextBundleId),
      stringValue(recordValue(data?.contextBundleRef)?.id),
      stringValue(valueRecord?.contextBundleId),
      stringValue(recordValue(valueRecord?.contextBundleRef)?.id),
    ].filter((item): item is string => !!item)),
  }
}

function contextMutationKeys(value: unknown): string[] {
  const summary = recordValue(value)
  if (!summary) return []
  return [
    ...stringArray(summary.affectedContextKeys),
    ...stringArray(summary.appendedContextKeys),
    ...stringArray(summary.amendedContextKeys),
    ...stringArray(summary.deletedContextKeys),
  ]
}

function contextRefKeys(value: unknown): string[] {
  const refs = arrayValue(value) ?? []
  return refs.flatMap((item): string[] => {
    const record = recordValue(item)
    const key = stringValue(record?.key)
    if (key) return [key]
    const ref = recordValue(record?.ref)
    const type = stringValue(ref?.type)
    const id = stringValue(ref?.id)
    if (!type || !id) return []
    return [`${type}:${id}:${stringValue(ref?.version) ?? stringValue(ref?.hash) ?? ''}`]
  })
}
