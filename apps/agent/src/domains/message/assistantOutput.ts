...
import { isValidAgentEntityId, parseToolResult } from '../../context/runtimeContext.js'
import type { RuntimeModelChatMessage } from '../../model/modelConfig.js'
import type { AgentMemory } from '../../memory/types.js'
import type { AgentRun, ToolCall, ToolCallOutcome } from '../../state/types.js'
import { formatToolNameForDisplay, publicToolName } from '../../tools/toolNames.js'
import { runtimeModelTextContent } from './modelMessage.js'
...
export function buildAssistantMessages(
  ...
  const messages: Array<RuntimeModelChatMessage | undefined> = [
    ...
    {
      role: 'system',
      content: runtimeModelTextContent(`Execution policy JSON:\n${JSON.stringify(run?.policy ?? null)}`),
    },
    warnings.length > 0 ? {
      role: 'system' as const,
