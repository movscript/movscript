...
export function resolveRuntimeIntents(message: string, debugContext: AgentDebugContextPanel): RuntimeIntentResolution {
  ...
  const hasExplicitAssetTask = routeIntentSet.has('asset_proposal')
    || routeIntentSet.has('asset_candidate_generation')
    || routeIntentSet.has('visual_generation')
  if (route.includes('creative-references') || (route.includes('pre-production') && !hasExplicitAssetTask)) addSignal(signals, 'setting_proposal', 'route', 'high', `route:${debugContext.route.pathname}`)
  ...
  if (route.includes('asset-slots') || route.includes('pre-production')) {
    if (!hasExplicitAssetTask) addSignal(signals, 'asset_proposal', 'route', 'high', `route:${debugContext.route.pathname}`)
  }
