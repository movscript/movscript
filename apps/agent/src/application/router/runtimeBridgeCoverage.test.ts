...
const files = listApplicationFiles(applicationDir)
...
function bridgeExportNames(moduleName: string): { bridgeName: string; factoryName: string } {
  const aliases: Record<string, string> = {
    'taskgraph/creation/bridge/runtimePlanCreationBridge': 'RuntimeTaskGraphCreationBridge',
    'taskgraph/dispatch/bridge/runtimePlanDispatchBridge': 'RuntimeTaskGraphDispatchBridge',
    'taskgraph/read/status/bridge/runtimePlanStatusBridge': 'RuntimeTaskGraphStatusBridge',
  }
  const shortName = moduleName.split('/').at(-1) ?? moduleName
  const bridgeName = aliases[moduleName] ?? shortName.replace(/^runtime/, 'Runtime')
