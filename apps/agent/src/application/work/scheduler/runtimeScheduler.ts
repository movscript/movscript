...
import type { AgentRun, CreateRunInput, RuntimeContinuation, RuntimeInteraction } from '../../../state/shared/types.js'
import type { AgentStore } from '../../../state/store/core/store.js'
import { isJSONValue } from '../../../shared/json/jsonValue.js'
import { getApprovedToolNames } from '../../../state/run/interaction/runInteractionState.js'
import {
  approveRuntimeInteraction,
  rejectRuntimeInteraction,
  type RuntimeInteractionApprovalResult,
} from '../../run/interactions/records/runtimeInteractions.js'
import type { RuntimeRunControlBridge } from '../../run/control/bridge/runtimeRunControlBridge.js'
import type { RunBackendAuth } from '../../run/auth/runAuth.js'
...
export class RuntimeScheduler {
  ...
  private advanceContinuation(continuation: RuntimeContinuation): AgentRun | undefined {
    ...
    const run = this.input.continueRun({
      ...
      ...(sourceRun?.runtimeLimits ? { runtimeLimits: sourceRun.runtimeLimits } : {}),
      ...(sourceRun?.runtimeLimits?.sandboxMode === true ? { sandboxMode: true } : {}),
