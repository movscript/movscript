...
test('applyRuntimeLocalGenerationCommand forces a generation tool call and completes the run', async () => {
  ...
  assert.deepEqual(traces.map((trace) => trace.kind), ['run', 'tool_call', 'tool_call', 'assistant', 'run'])
  assert.equal((traces[0]?.data as any)?.modelGatewayCalled, false)
  assert.equal((traces[0]?.data as any)?.command.payload, undefined)
  assert.match(String((traces[0]?.data as any)?.command.payloadHash), /^sha256:/)
  assert.equal((traces[0]?.data as any)?.generation.prompt, undefined)
  assert.match(String((traces[0]?.data as any)?.generation.promptHash), /^sha256:/)
  assert.doesNotMatch(JSON.stringify(traces[0]?.data), /雨夜便利店/)
  assert.equal(traces[1]?.title, 'Tool started: core_work_start')
  assert.equal((traces[1]?.data as any)?.args, undefined)
  assert.equal((traces[1]?.data as any)?.argsMode, 'summary')
  assert.doesNotMatch(JSON.stringify(traces[1]?.data), /雨夜便利店/)
  assert.equal(traces[2]?.toolName, 'core_work_start')
  assert.equal((traces[2]?.data as any)?.args, undefined)
  assert.equal((traces[2]?.data as any)?.argsMode, 'summary')
  assert.equal((traces[2]?.data as any)?.result, undefined)
  assert.equal((traces[2]?.data as any)?.resultMode, 'summary')
  assert.equal(typeof (traces[2]?.data as any)?.resultHash, 'string')
  assert.equal(traces[3]?.summary, `Assistant message created (${assistant.content.length} chars).`)
  assert.doesNotMatch(traces[3]?.summary ?? '', /雨夜便利店/)
