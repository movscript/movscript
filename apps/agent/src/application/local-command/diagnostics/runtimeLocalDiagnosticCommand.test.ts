...
test('applyRuntimeLocalDiagnosticCommand completes memory diagnostic run without model gateway side effects', () => {
  ...
  assert.equal(completedStep?.roundIndex, 1)
  assert.equal(completedStep?.roundLabel, 'Runtime command')
  assert.equal(completedStep?.roundSource, 'runtime_rule')
  ...
  assert.equal(traces[0]?.round?.roundIndex, 1)
  assert.equal((traces[0]?.data as any)?.modelGatewayCalled, false)
  assert.equal((traces[0]?.data as any)?.command.name, 'memory')
  assert.equal((traces[0]?.data as any)?.command.payload, undefined)
  assert.equal((traces[0]?.data as any)?.command.systemContract, undefined)
  assert.match(String((traces[0]?.data as any)?.command.payloadHash), /^sha256:/)
  assert.equal((traces[0]?.data as any)?.command.payloadChars, 'lens'.length)
  assert.equal(traces[1]?.kind, 'assistant')
  assert.equal(traces[1]?.round?.roundIndex, 1)
  assert.equal(traces[1]?.summary, `Assistant message created (${assistant.content.length} chars).`)
  assert.doesNotMatch(traces[1]?.summary ?? '', /Use a wide lens/)
  assert.equal(traces[2]?.kind, 'run')
  assert.equal(traces[2]?.round, undefined)
