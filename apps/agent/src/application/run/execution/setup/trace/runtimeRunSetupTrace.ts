...
export function recordRuntimeRunSetupTraces(input: {
  ...
  input.recordTrace(input.run, {
    ...
    round: input.setupRound,
    data: {
      eventType: 'config_file.resolved',
      id: input.layers?.trace.configFileId ?? input.activeManifest.id,
      version: input.layers?.trace.configFileVersion ?? input.activeManifest.version,
      ...(input.layers ? { skillIds: input.layers.trace.skillIds, configFileLayers: input.layers.trace.configFileLayers } : {}),
  ...
  input.recordTrace(input.run, {
    ...
    data: {
      ...
      configFileId: input.layers?.trace.configFileId,
      activeSkillIds: input.skills.map((skill) => skill.id),
