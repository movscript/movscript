...
test('recordRuntimeRunSetupTraces marks fallback context as blocked and includes layered trace metadata', () => {
  ...
  recordRuntimeRunSetupTraces({
    ...
    skillIds: [],
      ...
      skills: [],
      skillDiscovery: { configFileId: 'config_file_1', enabledPackIds: [], availableSkills: [] },
      warnings: ['layer warning'],
      trace: {
        configFileId: 'config_file_1',
        configFileVersion: '1.0.0',
        configFileLayers: [{ source: 'base', id: 'config_file_1', version: '1.0.0' }],
        skillIds: ['runtime_1', 'task_1'],
        intentSignals: [],
        triggerTraces: [],
        skillOmissions: [],
      },
  ...
  assert.equal((traces[1]?.data as any)?.id, 'config_file_1')
  assert.deepEqual((traces[1]?.data as any)?.skillIds, ['runtime_1', 'task_1'])
  assert.equal((traces[4]?.data as any)?.configFileId, 'config_file_1')
