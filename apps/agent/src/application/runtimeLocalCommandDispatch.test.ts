...
test('applyRuntimeLocalCommandDispatch handles generation commands locally', async () => {
  ...
  assert.equal(handled, true)
  assert.equal(toolName, 'core_operation_start')
  assert.equal(run.status, 'completed')
...
test('normalizeRuntimeLocalGenerationToolError preserves backend generation error data', async () => {
  const call = { name: 'core_operation_start' as const, args: { kind: 'generation_job' as JSONValue, request: { prompt: 'hello' as JSONValue } as JSONValue } }
