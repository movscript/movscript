...
function buildLocalResponse(messages: ChatMessage[], context: JSONValue | undefined): string {
  ...
  return [
    ...
    userLine,
    '',
    '当前 agent 对话链路已经打通。要得到真正的模型回复，请在启动 movscript-agent 前设置 MOVSCRIPT_AGENT_OPENAI_API_KEY 或 OPENAI_API_KEY；也可以设置 MOVSCRIPT_AGENT_OPENAI_MODEL 和 MOVSCRIPT_AGENT_OPENAI_BASE_URL 接入 OpenAI-compatible 服务。',
  ].join('\n')
}
