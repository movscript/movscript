...
} from './output/assistantOutput.js'
import { extractRequestedToolCallsFromAssistantContent } from '../tool/toolExchange.js'
import { isMessageRole } from '../thread/threadMessage.js'
...
test('configured assistant messages prefer resolved skill instructions from run metadata', () => {
  ...
  const systemText = messages.filter((message) => message.role === 'system').map((message) => runtimeModelContentText(message.content)).join('\n')
...
test('configured assistant messages ignore non-plain skill metadata records', () => {
  ...
  assert.match(systemText, /core\.rules\.runtime/)
  assert.match(systemText, /Core skill instruction from catalog/)
})
