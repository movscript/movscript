...
7. 对 draft 类输出执行 validation；proposal draft 在最终回复前必须用 `movscript_preview_draft_apply` 做 dry-run。支持后端预览的 kind 必须检查 backend apply-preview；暂不支持的 kind 至少要记录工具返回的本地 validation / skipped 状态。
