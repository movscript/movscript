...
`Project -> Production -> segment/scene moment -> content unit -> keyframe/shot generation -> asset candidate/generation job -> review draft/apply`
...
- 需要改内容结构、镜头职责或表达节拍时，使用 content_unit_proposal。
- 需要真实关键帧、镜头、图片或视频输出时，使用 visual_generation。
- `content_unit_media_proposal` 仅保留为历史草稿审阅兼容，不再作为新任务路由目标。
