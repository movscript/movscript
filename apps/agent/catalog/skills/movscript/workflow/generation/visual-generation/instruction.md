...
- 创建异步生成 operation：{{tool:runtime_operation_start}}，`kind: "generation_job"`，`request` 使用生成参数。
...
9. 只有在需要审批的异步生成 operation 获准运行后，才能提交任务。一个 backend job 只产出一个可提交候选；需要 2 张或更多候选图时，创建多个独立 operation（可用 `output_count`，或显式多次调用 `runtime_operation_start`），不要让一个 operation 承担多个候选输出。创建任务时使用 `runtime_operation_start`，传入 `kind: "generation_job"`。`runtime_operation_start` 不是通用工具包装器；不要用它包装普通同步工具调用。
