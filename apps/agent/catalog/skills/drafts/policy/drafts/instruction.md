...
- 对 proposal draft，最终回复前必须先运行一次 dry-run：使用 `movscript_update_draft` 的 `action: "preview_apply"`。如果 draft kind 暂不支持后端 preview，也要保留该工具返回的本地 validation / skipped 状态。
...
- proposal draft 创建后默认进入 apply 流程；非自动运行策略下，正式应用前必须等待用户确认；自动策略下仍必须先通过 draft validation。
