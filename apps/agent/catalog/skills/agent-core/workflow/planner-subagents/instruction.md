...
4. 派发或重新派发 worker tasks 时，用 maxWorkers 控制并发，用 retryFailed 和 maxTaskAttempts 处理失败或取消的任务重试，用 workerTimeoutMs 取消过期 active workers。任务级 maxTaskAttempts 和 workerTimeoutMs 覆盖调用级默认值。
5. 派发后，用 list/wait 工具检查结构化 task state、worker run status、blockers 和 artifacts；不要从自然语言聊天推断 worker 进度。
