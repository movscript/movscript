目标：
定义所有本地 draft 的边界、apply 语义和汇报要求。
...
- 创建、更新或校验 draft 不等于 apply；只有 `draft_apply` 成功返回后，才能说“本次 apply 已完成”。
- Draft 不再通过自身 status 进入 applied/rejected/superseded 终态；同一个 draft 是可反复编辑和反复 apply 的当前工作副本。
...
写入后：
- 必须报告 `draftId`、`kind` 和最近一次 validation / preview / apply 结果。
...
- 必须给出下一步 review、continue editing、preview apply 或 apply 动作，并明确 dry-run 或本次 apply 是否通过、失败或被跳过。
- 下游 proposal 只能基于已读正式状态、已应用的上游结果或用户明确接受的审阅交接继续；不要基于刚创建但尚未成功写入数据库的上游 draft 继续推演。

绝不：
- 不把 proposed draft 自身说成 accepted、applied、locked 或正式写入；只能描述最近一次 apply 动作是否完成。
