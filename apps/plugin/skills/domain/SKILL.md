...
- Current specialized `content_unit_type` adapters are `production_ref`, `segment_ref`, `asset_ref`, `keyframe_ref`, `storyboard_ref`, `scence_moment_ref` / `scene_moment_ref`, and `expression_unit_ref`. Unknown types are valid generic production slots, but the interpreter does not track their upstream dependencies or stale state.
