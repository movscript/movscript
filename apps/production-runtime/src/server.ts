...
const server = createServer(async (req, res) => {
  ...
  try {
    ...
    if (req.method === 'GET' && url.pathname === '/health') {
      writeJSON(res, 200, {
        ok: true,
        service: 'movscript-production-runtime',
        mode: 'server',
        mcpEndpoint,
        pluginCatalog: {
          skillsDir: pluginCatalog.skillsDir,
...
server.listen(port, '127.0.0.1', () => {
  console.info(`[production-runtime] movscript-production-runtime listening on http://127.0.0.1:${port}`)
  console.info(`[production-runtime] using MovScript MCP endpoint ${mcpEndpoint}`)
