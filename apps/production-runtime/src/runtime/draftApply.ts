...
export function applyDraftAfterApproval(store: AgentDraftStore, input: ApplyDraftInput): ApplyDraftResult {
  ...
  return {
    ...
    review,
    draft: applied,
    message: 'Draft marked applied in the local production runtime lifecycle. Formal backend entity write is not performed by this runtime path yet.',
  }
}
...
export function markDraftApplied(
  ...
  return store.updateDraft(draft.id, {
    ...
    metadata: {
      ...
      appliedBy: 'movscript-production-runtime',
