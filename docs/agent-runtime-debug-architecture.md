...
| Local agent | Run lifecycle, agentic loop, permissions, drafts, memory, sandbox mode. It owns persisted thread/message/run/trace/draft/memory state. | `apps/agent/src/server.ts`, `apps/agent/src/application/agentRuntime.ts` | thread, message, run, policy, steps, approvals, memories, drafts | Overview, Manifest, Skills, Tools, Prompt, Run Timeline, Raw JSON |
| Business context layer | MovScript project entities and production state. | `apps/backend/internal/interfaces/http/router/router.go`, `apps/backend/internal/domain/workflow/entity_schema.go`, `apps/frontend/src/lib/mcpTools.ts` | focus, semantic entities, workflow schemas, resource bindings, canvas tasks | Context JSON, tool result JSON, AI Function Inventory |
| Tool execution layer | MCP/runtime/plugin tool discovery, grants, policy, execution. | `apps/agent/src/tools/toolRegistry.ts`, `apps/agent/src/tools/toolPolicy.ts`, `apps/agent/catalog/tools` | resolved tool catalog, blocked tools, tool calls, tool outcomes | Tools table, approval preview, step timeline args/result/error |
| Model layer | Optional assistant chat reply through backend model gateway. | `apps/agent/src/model/modelConfig.ts`, `apps/agent/src/application/assistantMessage.ts` | compiled prompt, assistant message | Model Connection, Prompt tab, final assistant message |
...
1. Resolve the MCP focus and memories.
