...
- Frontend trace client code now exports its own `AGENT_TRACE_EVENT_KINDS` runtime list, derives `AgentTraceEventKind` from it, preserves top-level `durationMs` on run steps and trace events, and `agentRunUi` consumes that duration for trace detail panels and debug reports. This keeps the visible debugging surface aligned with the backend trace contract and debug-bundle fixtures.
- `scripts/verify-agent-run-debugging.mjs` now checks that frontend `AGENT_TRACE_EVENT_KINDS` stays in lockstep with backend `AGENT_TRACE_EVENT_KINDS`, that the frontend type is derived from the runtime list, and that top-level trace duration remains visible in the debug UI.
