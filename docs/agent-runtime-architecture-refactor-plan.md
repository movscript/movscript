...
- `application/runtimeRunInteraction.ts` now owns approval/input interaction flow, request wrapping, and persistence: requires-action pause state transitions, approve/reject/answer state transitions, thread run-status projection, assistant rejection messages, intentional user-visible answer messages, approval/input trace payload construction, rejection message-step completion, stream snapshot callback ordering, auth-memory callback ordering, execution-restart callback ordering, and timestamp/message-id fan-out from request helpers, while `AgentRuntime` retains the concrete trace sink, step creation, stream sink, auth registry, id/time factories, and execution scheduler.
...
- `application/runtimeTaskEventBridge.ts` now owns request-level task protocol trace plus optional plan task stream event ordering, removing the task protocol bridge method from `AgentRuntime`.
