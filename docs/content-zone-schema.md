...
当前内容区以语义实体为准，不再兼容旧的制作、分场、分镜、镜头生产链表。
...
这些表对应旧版「制作 -> 分场 -> 分镜 -> 镜头 -> 成片」链路。内容区页面改走语义实体表和 `/projects/:id/entities/...` 接口。
...
- 情节：`scene_moments`
- 内容：`content_units`
- 设定资料：`creative_references`、`creative_reference_states`
...
这些概念不是旧版「制作 -> 分场 -> 分镜 -> 镜头」的改名，而是面向 AI 生成、素材准备和画布工作流的语义对象。

### 编排段

编排段对应 `segments`。它是本集内部的情绪、节奏和戏剧功能段，用来回答“这一段在本集情绪推进、节奏变化和戏剧任务上承担什么”。编排段不等同于剧本文本段落、场次或情节复述；它可以引用来源文本，但核心语义是编排功能。

用户理解：

- 编排段描述本集内部的情绪阶段、节奏变化、戏剧功能和观众预期管理。
- 编排段可以保留来源范围、补充说明、顺序、类型和确认状态。
- 编排段下面可以继续拆出多个情景，也可以关联多个内容单元。
- 编排段偏“创作编排判断”，不应该被具体素材生成结果随意覆盖。
...
- 情景从编排段中拆出，描述一个可用于生成画面、声音、字幕或素材需求的具体时刻。
...
设定资料对应 `creative_references` 和 `creative_reference_states`。它是项目级的“设定资料库”或“设定圣经”，统一承载人物、地点、道具、产品、品牌、风格、世界规则、时代背景和限制条件。

用户理解：

- 设定资料是跨片段、跨情节复用的稳定设定。
- `creative_references` 表示设定资料本体，例如角色、场景、道具、品牌、风格。
- `creative_reference_states` 表示设定资料在某个范围内的临时状态，例如某个角色在某个情节里的服装、情绪、道具、视觉备注。
- `creative_reference_usages` 记录某个片段、情节、内容单元或关键帧使用了哪份设定资料或哪份设定状态，保证连续性。
- `creative_relationships` 记录设定资料之间的关系，例如人物关系、品牌归属、道具归属、风格约束。
...
- 素材槽可以绑定设定资料、设定资料状态、片段、情节、内容单元或关键帧。
...
画布应定位为生产阶段工具，主要负责生产图片、视频、音频等可交付素材。片段、情节、设定资料和内容单元可以作为生产上下文被读取，但不应由画布直接改写；画布写入应集中在素材槽、候选素材、资源文件和资源绑定上，并留下审计。结构性内容的改动应回到内容区、设定资料区或专门的审核/候选流程。

| 对象 | 画布应可读 | 画布应可写 | 建议原因 |
| --- | --- | --- | --- |
| 编排段 `segment` | 标题、类型、顺序、情绪/节奏/戏剧功能说明、来源范围、状态、下游情景/内容计数 | 不可写 | 编排段是本集内部的情绪、节奏和戏剧功能结构，属于内容区事实。画布只能把它作为提示词上下文，不能在生产素材时改写结构。 |
| 情景/情节 `scene_moment` | 时间、地点、条件、动作、情绪、描述、关联设定资料、素材需求缺口 | 不可写 | 情景是生成上下文，画布应读取它来生产图片、视频、音频；情景内容本身的修改应回到内容区。 |
| 设定资料 `creative_reference` | 名称、类型、别名、描述、内容、重要性、状态、状态列表、关系、素材槽 | 不可写 | 设定资料是连续性来源和设定事实。画布可读取人物、地点、道具、品牌、风格等设定资料，但不应在生产运行中修改资料本体。 |
| 设定资料状态 `creative_reference_state` | 范围、状态名、视觉备注、情绪、服装、道具、标签 | 不可写 | 状态可作为局部生成约束读取；状态变更会影响连续性，应由设定资料区或审核流程确认。 |
...
| 素材定义 `asset_slot` | 名称、类型、状态、优先级、描述、槽位、提示词、所属设定资料、候选集、锁定素材 | 可写候选图片、候选视频、候选音频、结果资源、资源绑定；必要时更新候选状态 | 素材槽就是生产缺口和候选容器，是画布写回的主要目标。画布写入的是“产出的素材”，不是上游设定。 |
...
- 前端画布左侧语义货架已经展示编排段、情景、设定资料、素材槽、内容单元，并可拖成 `entity_card`。
...
- workflow entity schema 已移除旧 `setting` 实体，后续应统一通过 `creative_reference`、`creative_reference_state` 和使用关系读取设定资料上下文。
- `EntityIOService` 当前的写入能力偏通用，后续需要明确区分只读上下文端口和生产写回端口，避免画布直接写入片段、情景、设定资料等上游事实。
- `EntityCardNode` 目前只为 `asset_slot`、`content_unit` 拉取 `/entities/:kind/:id/semantic-values`，编排段、情景、设定资料主要依赖拖拽时的摘要文本，卡片预览不足。
...
3. 在画布实体卡片中区分“读取上下文端口”和“生产写回端口”，编排段、情景、设定资料只读，素材槽开放候选/资源写入，内容单元只允许挂接生产结果。
4. 将画布写回目标限制为图片、视频、音频等资源、`resource_bindings`、`asset_slot_candidates` 和素材槽锁定/候选状态。
5. 在 UI 中补齐素材候选采纳/拒绝、锁定、回滚入口，让画布生成结果进入可审核的生产素材流程。
