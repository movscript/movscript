...
- Set `ACTUAL_FFMPEG_RELEASE_URL` to the real upstream release page or artifact URL before staging.
...
- `pnpm run package:desktop` or the matching explicit target command passes, such as `package:desktop:mac:x64`, `package:desktop:mac:arm64`, `package:desktop:linux:x64`, `package:desktop:linux:arm64`, `package:desktop:win`, or `package:desktop:win:arm64`, including `pnpm run release:verify-desktop`.
