...
- Media Pipeline 第一百零二批补充：`packages/editing` 已新增 `MEDIA_PIPELINE_TASK_CREATE_ENDPOINT`、`MEDIA_PIPELINE_TASK_ACTION_ENDPOINT`、`MediaPipelineTaskCreateRequest/Response`、`MediaPipelineTaskActionRequest/Response` 和 `MediaPipelineServiceClient.createTask/taskAction`；`services/media-pipeline` 的 HTTP runtime 已可注入同一套 `EditingRuntimePort`，并通过 `POST /v1/media-pipeline/task/create`、`POST /v1/media-pipeline/task/action` 托管 `createTask`、`getTask`、`cancelTask`、`getTaskLogs`。未注入 runtimePort 时，task execution endpoint 必须返回 `media_pipeline_runtime_unavailable`，不能在 Desktop、Plugin 或 MCP 里各自做隐式 fallback。后续 Desktop full runtime 和 Plugin full-local fallback 只负责在应用启动时把自己的 media pipeline adapter 注入/启动为 `movscript.media.pipeline` 服务，并把 endpoint 写入 MovScript Home。
- Media Pipeline 第一百零三批补充：Desktop Electron 的真实 media pipeline adapter 已抽出为 `createDesktopMediaPipelineRuntimePort`，Electron IPC 和 Desktop ApplicationRunner 共用同一套 adapter。`apps/desktop/runtime/desktopApplicationRuntime.ts` 现在通过 `@movscript/media-pipeline` 的 typed package API 启动 in-process `movscript.media.pipeline` HTTP server，并注入 `EditingRuntimePort`；`desktop-bootstrap` policy 已声明 `movscript.media.pipeline` 可选 desktop profile，启动后会把该 endpoint 写入 MovScript Home runtime records。`services/media-pipeline` 同步补齐 `exports` / `types` 和 `@movscript/runtime-contracts` 显式依赖，Desktop 不再通过相对路径偷引服务实现。Desktop renderer 同步改为通过 `packages/editing/src/browser.ts` 消费 browser-safe pure editing entry，避免把 `@movscript/runtime-contracts` 的 Node fs/path discovery 带入浏览器 bundle。这样在 Desktop 已启动时，Agent/Plugin/MCP host 可优先发现 Desktop-owned `movscript.media.pipeline` endpoint；Plugin full-local fallback 后续只在未发现 Desktop full runtime 时启动自己的同构 media pipeline service。
- Media Pipeline 第一百零四批补充：Plugin App 已具备发现 `movscript.media.pipeline` endpoint 的 runtime-status 语义，但 `plugin-basic` 不再启动任何 media pipeline sidecar，只保留 launcher + stdio MCP host。Desktop-owned 模式只外连 Home 中已有的 Desktop-owned `movscript.media.pipeline` endpoint；Plugin 侧 media pipeline 只允许作为 `plugin-full-local` 的 required local service 启动。Plugin full-local 启动的 media-pipeline CLI 默认装配 headless runtime adapter：可 probe `ffmpeg`、维护 task state/logs/cancel，并承接 `media_transcode`、`media_reframe`、最小本地媒体 clip 的 `timeline_render` / `timeline_hls` ffmpeg task lifecycle；缺少 ffmpeg 时 task endpoint 返回稳定 failed task，而不是 503 runtime unavailable。Agent/MCP host 可通过 `movscript_runtime_status.mediaPipeline` 看到 media-pipeline endpoint 是否存在。
...
- Agent Plugin App 不唤起 Desktop full runtime；需要本地执行时直接 ensure/attach `movscript.local-node`。
...
Electron main process
  ...
  -> starts/supervises desktop-owned local services
...
`movscript.desktop.shell` 内部可以有 Electron main、renderer、preload bridge、service clients、local service supervisor、preview/player adapter 等模块，但这些都是 Desktop App 的内部实现，不进入全局 service graph。
...
- 页面文案应说明 Desktop 是本机优先 runtime owner；Plugin 在 Desktop 不存在或不可用时可以独立工作。
...
103. 已完成第一百零三批：Desktop full runtime 开始把 Electron mediaPipeline adapter 托管为 `movscript.media.pipeline` service。`apps/desktop/electron/services/mediaPipelineRuntimePort.ts` 抽出 `createDesktopMediaPipelineRuntimePort`，Electron IPC 和 Desktop ApplicationRunner 共用同一套 `EditingRuntimePort` adapter；`apps/desktop/runtime/desktopApplicationRuntime.ts` 通过 `@movscript/media-pipeline` 启动 in-process HTTP server、注入该 adapter，并由 ApplicationRunner 写入 MovScript Home service/endpoint records。`desktop-bootstrap` policy 已声明 optional desktop profile 的 `movscript.media.pipeline`，Desktop runtime 测试验证 Home discovery 可解析该 endpoint 且 probe 返回 `EditingRuntimePort` envelope。`services/media-pipeline` 补齐 typed package export 和 runtime-contracts 显式依赖；`packages/editing/src/browser.ts` 补齐 browser-safe pure editing entry，Desktop renderer alias 指向该入口，避免 Node runtime discovery 进入浏览器 bundle。Desktop typecheck、Desktop build、Media Pipeline package 测试、service boundary / runtime manifest 测试均覆盖该分层。
104. 已完成第一百零四批：Plugin App 接入 media-pipeline endpoint discovery，但 `plugin-basic` 语义已收敛为只启动 launcher + stdio MCP host，不再启动 Plugin-owned media pipeline。Desktop runtime ready 时继续选择 `plugin-desktop-owned` 场景并只外连 Desktop/Home 中已有的 media-pipeline endpoint；Plugin 自己启动 `movscript.media.pipeline` 只属于 `plugin-full-local` required service。MCP Host `movscript_runtime_status` 新增 `mediaPipeline` 状态并把 Desktop probe 中的 `mediaPipeline/mediaPipelineEndpoint` 改为来自 Home discovery。Plugin launcher smoke、MCP host 和 Media Pipeline 边界测试覆盖该路径。
