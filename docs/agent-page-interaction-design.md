...
前端负责收集业务上下文、组织页面任务、展示 Agent 结果、维护草稿/会话状态，并在用户确认后发起 apply。它不是单纯展示层，而是人和 Agent 之间的业务编排层。

### 3. Agent

Agent 负责推演、生成、修改和校验草稿或提案，管理 run/thread/memory/tool 过程。它产出的 draft/proposal 是审阅协议对象，不是后端正式实体。
