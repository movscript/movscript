...
编排段是本集内部的情绪、节奏和戏剧功能容器。编排工作台在这一层只确认编排段与情景；内容单元属于下游制作颗粒，由制作工作台基于已确认情景再生成或编辑。
...
- 内容单元负责“最终要生成或剪辑的一段画面、字幕、旁白、转场、音乐点等”，但它不是编排工作台的默认产出。
- 一个编排段可以有多个情景；内容单元可以在制作工作台中基于编排段或情景派生。
...
内容单元是更细的执行颗粒，它通常来源于某个情节，同时也会使用设定资料和素材槽。内容单元、运镜、台词定稿、关键帧和 prompt 的细化入口应在制作工作台，而不是编排工作台。
