...
│   默认 agent 实例的运行时绑定:persona + candidate workflows/policies/│
│   tool grants + limits + model. 不由 UI mode 选择。                  │
└──────────────────────────────────────────────────────────────────────┘
                              ▲ binds
┌──────────────────────────────────────────────────────────────────────┐
...
│  ── Mode 已删除;UI 只发送 context/intent,不选择 runtime profile。     │
└──────────────────────────────────────────────────────────────────────┘
...
| Pack | 哪些资源同发布、是否完整 | id 列表 + version 约束；linter 用它检查 skill/tool/schema 是否齐全 | 何时启用、启用顺序 |
| Profile | 现在启用了什么 | persona 引用、workflow/policy 列表、tool 授权、model 绑定 | skill body、schema 内容、tool description |

Workflow 还必须显式声明执行边界。规划/提案类 workflow 只产出可审阅的 draft 或候选方向；生成执行类 workflow 才能提交异步生成任务；审阅类 workflow 只比较和评价结果；正式写入必须走审批工具或 UI apply。不要把素材候选规划和图片/视频生成 job 提交放在同一个 workflow 里。
...
// Note: 'mode' 已删除。UI 通过 route/selection/labels/message 触发 workflow。
```
...
| 旧字段 | 新位置 |
|---|---|
| `MODE_CONFIGS['plan'].soul` | `catalog/skills/agent-core/persona/mode-personas.persona.json` 的 `instructionTemplate` |
| `DEFAULT_AGENT_MANIFEST.soul` | `catalog/skills/agent-core/persona/movscript-default.persona.json` 的 `instructionTemplate` |
...
- `scope: { workflow: ['movscript.workflow.project-proposal'] }`:仅当该 workflow 当轮被激活时生效
...
├── core.pack.json                  // agent 自身能力(memory + subagent + input + catalog reload)
├── draft.pack.json                 // 通用 draft schema 与编辑工具
└── movscript.pack.json             // workspace + proposal + visual-generation 能力清单
```
...
export interface CapabilityPack {
  id: string                       // 'movscript.pack.movscript'
  ...
  /** 与其他 pack 的依赖 / 互斥 */
  requires?: {
    packs?: Record<string, string>     // { 'movscript.pack.agent-core': '>=1.0.0' }
...
Pack 是 catalog 完整性的检查清单。Linter 必须验证:

- `schemas / tools / skills` 中列出的资源在 registry 中存在。
- pack 中包含的 skill 若声明 `toolRefs / schemaRefs`,这些依赖必须由当前 pack 或 `requires.packs` 的闭包覆盖。
- 因此一个 pack 是否完整,不是看文件名或目录层级,而是看它声明的资源清单及其依赖闭包是否齐全。

### 5.5 与 Profile 的关系
Profile 通过 `enabledPacks: string[]` 引入 pack。引入 pack 等价于把它包含的所有 skill/tool/schema **登记为运行时可用**。Loader 会从 enabled pack 闭包自动推导候选 workflows、policies 和 tool grants。
...
Profile 的 `enabledWorkflows / enabledPolicies / toolGrants` 只保留为兼容收窄字段:非空时只能从 enabled packs 已覆盖的资源中筛选或收紧,不能把 pack 未注册的资源打开。
...
| `modeRegistry.MODES` | 删除。UI 不再选择 runtime profile。 |
...
export interface AgentProfile {
  ...
  id: string                          // 'movscript.profile.default'
  version: string
  name: string
  description?: string
  ...
  /** Persona 显式选择；workflow/policy 默认由 enabled packs 推导 */
  persona: string | null              // 至多 1 个 persona skill id
  enabledWorkflows: string[]          // 兼容收窄字段；空数组表示使用 enabled packs 中全部 workflow
  enabledPolicies: string[]           // 兼容收窄字段；空数组表示使用 enabled packs 中全部 policy

  /** Tool 授权兼容覆盖；只能收窄/收紧 enabled packs 中的 tool defaults */
...
├── default.profile.json                       // 唯一内置 runtime profile
├── org/
...
```json
{
  ...
  "version": "1.0.0",
  "name": "MovScript Default Profile",
  "description": "默认 draft-first agent profile。"
  "enabledPacks": [
    "movscript.pack.agent-core",
    "movscript.pack.drafts",
    "movscript.pack.movscript"
  ],
  "persona": "movscript.persona.default",
  "enabledWorkflows": [],
  "enabledPolicies": [],
  "toolGrants": [],
  "model": {
...
- 内置 runtime 只有一个 default profile。UI 不得通过 mode 切换 profile。
- default profile 的 `enabledWorkflows / enabledPolicies / toolGrants` 可为空;loader 会按 `enabledPacks` 覆盖的 skill/tool defaults 自动补齐候选项和允许工具。
- 如果一个 workflow 没被任何 enabled pack 注册,它就不会进入 prompt(即使 trigger 命中)。如果 profile 显式填写 `enabledWorkflows`,该列表只会从 enabled pack 覆盖的 workflow 中收窄。
- `permissions` 字段不能手写,由 toolGrants 派生

### 6.7 Profile 合并语义(Org / User)
...
```
effective = merge(default.profile, org.profile?, user.profile?)
                                                ↑ 同层时按文件顺序
```
...
| `enabledPacks` | default profile 声明可用 pack；org/user override 只能收窄,不能新增 |
| `persona` | 取最后一层非 null 值(后覆盖前) |
| `enabledWorkflows` | 兼容收窄字段；最终值必须来自 enabled pack 覆盖的 workflow |
| `enabledPolicies` | 兼容收窄字段；最终值必须来自 enabled pack 覆盖的 policy |
| `toolGrants` | 兼容收窄字段；最终值必须来自 enabled pack 覆盖的 tool,且只能 deny 或收紧 approval |
...
| 层 | 可写字段 |
|---|---|
| `org` | `enabledPacks`(只能减不能加,值需为 default profile 中的子集)、`enabledWorkflows`(只能减)、`enabledPolicies`(只能加,合规红线)、`toolGrants`(只能收紧)、`limits`(只能调小)、`metadata` |
...
```
[1] HTTP 请求到达 (message + uiContext + intents/labels)
       │
       ▼
[2] 选 Profile:
    profile = resolveProfile(orgId, userId)             // 见 §6.7
       │
       ▼
[3] 解析 Pack → 可用资源集
    available = ∑ packs(profile.enabledPacks) → {schemas, tools, skills}
       │
       ▼
[4] 从 enabled packs 派生 Profile 有效资源:
    persona  = lookup(profile.persona)                  // 至多 1
    policies = packPolicies ∩ optional(profile.enabledPolicies)
                .filter(scopeMatches(profile, ctx))     // 应用 PolicyScope
    candidates = packWorkflows ∩ optional(profile.enabledWorkflows)
    toolGrants = packTools defaults, optionally narrowed/tightened by profile.toolGrants
       │
       ▼
[5] Trigger 评估:
...
| 现状文件 / 字段 | 目标位置 |
|---|---|
| `catalog/skills/modes.json::movscript.intent.mode.plan` | `catalog/profiles/modes/plan.profile.json`(soul → `catalog/skills/agent-core/persona/mode-personas.persona.json`;workflow 移除) |
| `catalog/skills/modes.json::movscript.intent.project-proposal` | `catalog/skills/movscript/workflow/proposal/project/project-proposal/skill.workflow.json`(schema 抽出到 `packages/draft-schemas/project-proposal/prompt-summary.md`) |
| `catalog/skills/modes.json::movscript.intent.production-proposal` | `catalog/skills/movscript/workflow/proposal/production/production-proposal/skill.workflow.json`(同上) |
| `catalog/skills/modes.json::movscript.intent.dual-orchestration` | `catalog/skills/movscript/workflow/proposal/production/dual-orchestration/skill.workflow.json` |
| `catalog/skills/drafts.json::movscript.drafts.safe-drafts` | `catalog/skills/drafts/policy/drafts/skill.policy.json` |
| `catalog/skills/drafts.json::approval boundary policy text` | `catalog/skills/agent-core/policy/agent-core/skill.policy.json` |
| `catalog/skills/platform.json::movscript.platform.concepts` | `catalog/skills/movscript/policy/movscript/skill.policy.json` |
...
| **I11** | Mode 不存在于 runtime 架构。UI 只提供 context/intent;workflow trigger 决定行为。 |
...
| Policy | `<vendor>.policy.<pack-or-layer>` | `movscript.policy.drafts` |
| Pack | `<vendor>.pack.<name>` | `movscript.pack.movscript` |
...
Profile 是默认运行时绑定;组织/用户覆盖只能收紧,不作为功能入口发布。
...
export interface PluginPackManifest {
  ...
  contributes: {
    ...
    profiles?: AgentProfile[]               // 可选,plugin 可贡献收紧型 org/user profile
  }
...
1. **默认 profile + 代表性 workflow 场景** 跑 `composePrompt` 和 trigger 测试
...
`apps/agent/src/__tests__/catalog/` 覆盖 default profile prompt composition、workflow trigger、tool scope。快照不是按 mode 维度维护。
...
15. **Prompt/trigger 测试**:default profile 与代表性 workflow 场景必须覆盖。
