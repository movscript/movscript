...
Project proposal 内部也应该按 snapshot 的语义分两层组织，但不需要拆成两个独立 draft：
...
Draft 的权威状态应该面向 snapshot，而不是面向 operation log。用户和下一轮 Agent 都应该看到“当前方案长什么样”；operation / patch 更适合做修改历史、审计记录和最终 apply 时的系统产物。
...
- 最终落库 operation 由系统根据最新数据库和当前 snapshot 生成，不直接信任历史 patch
