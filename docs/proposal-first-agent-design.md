...
用户不需要先知道 `project_proposal`、`production_proposal`、`workflow`、`MCP` 或 `manifest`。产品默认展示的是：
...
- `kind`：提案类型，例如 `project_proposal`、`production_proposal`、`script_split_proposal`、`asset_proposal`、`delivery_proposal`。
...
`project_proposal` 和 `production_proposal` 仍然保留，但它们不再代表“必须先进入某个固定工作流”的两条入口，而是 proposal-first 体系里的两个核心 proposal kind。
...
默认产品入口应围绕 proposal，而不是围绕实体表和工作流列表。
...
- Project proposal、production proposal、script split、asset proposal 统一进入 review shell。
