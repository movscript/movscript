...
- 生产对象：项目、剧本、素材、制作、场景、分镜、镜头、画布、生成任务等核心实体已经存在。
- 本地 Agent：agent、MCP-shaped 工具、planner、draft、memory、debug surface 已经搭起实验框架。
...
2. **前端负责体验，backend 负责关键能力**
   ...
   - agent 可保留本地直连模型能力，但默认应优先走 Movscript model gateway。
...
- 明确 AI 调用边界：
  ...
  - agent 默认可通过 backend model gateway 调模型。
...
2. 打磨本地单人 MVP。
3. 让模型配置和运行状态对普通用户更顺。
4. 将 agent 默认模型路径向 backend model gateway 靠拢。
