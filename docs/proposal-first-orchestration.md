...
preparation into the production step. Instead, it should create a reviewable
project proposal first, wait for the user to approve or reject it, then resume
the production proposal using the accepted project state.
...
production orchestration request
  -> detect missing project-level prerequisites
    -> create project_proposal draft
      -> user reviews/applies project proposal
...
`project_proposal` owns project-level setup:
...
Switch to `project_proposal` first when the missing item changes reusable
project knowledge:
...
4. Compare against approved or reviewable project state.
   Existing project data and already approved/applied project proposals count as
   available. Drafts that are not approved may be referenced only as pending
   prerequisites.

5. Choose the next proposal.
   If project-level prerequisites are missing, create or update a
   `project_proposal` draft. If all project prerequisites are available, create
   or update a `production_proposal` draft.

6. Pause at review boundaries.
   After creating a project proposal, the Agent reports the draft ID, review
...
7. Resume after approval.
   Once the user applies the project proposal, the Agent re-reads focus and
...
Recommended metadata for the project proposal:
...
```json
{
  ...
  "resumeAfter": {
    "event": "project_proposal_applied",
    "draftId": "draft_abc"
  }
}
```
...
- Which production request caused the project proposal.
- Which project draft blocks the production draft.
...
User asks for production orchestration
  ...
  -> proposal_first decision redirects to project_proposal
  -> project_proposal draft is created or updated
...
- Which project proposal draft was created or updated.
...
6. Add tests.
   ...
   draft creation before production draft completion, and resume after project
   proposal apply.
...
- Missing characters, locations, or reusable references produce a
  `project_proposal` draft first.
- The Agent pauses after creating the project proposal and does not claim the
  production proposal is complete.
- After the project proposal is applied, the Agent can continue the production
