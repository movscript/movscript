...
} from '../dist/shot-library/index.js'

test('core shot library analyzes local video references into reusable semantics', () => {
...
test('core shot library normalizes runtime source URLs for API v1 access', () => {
...
test('core shot library maps API entries to source-scoped runtime entries', () => {
  const [source] = normalizeShotLibraryRuntimeSources([
...
test('core shot library resolves resource URLs and merges source-scoped references', () => {
...
test('core shot library builds stable search index data from semantic entries', () => {
  const entry = shotLibraryEntryFromApi({
    ID: 7,
    resource_id: 8,
...
test('core shot library builds vector documents with caller-provided localization', () => {
  const entry = analyzeShotReference(
    { ID: 42, name: 'slow_push_reveal.mp4', url: '/api/v1/resources/42/file', size: 4096, mime_type: 'video/mp4' },
...
test('core shot library scores translated keyword search and facet filters', () => {
