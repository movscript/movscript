...
export function domainTools(): MCPTool[] {
  return [
    ...
    },
    {
      ...
      description: 'Convert the production preview timeline plus selected scene_moment video candidates into an OpenCut-compatible production timeline document. Returns timeline_document, compose_inputs, clips, and blockers for missing scene_moment output selections.',
      inputSchema: projectSchema({
        ...workspaceLocator,
        productionId: { type: ['string', 'number'] },
        production_id: { type: ['string', 'number'] },
    ...
    },
    {
      ...
      description: 'Convert a scene_moment edit plan into an OpenCut-compatible timeline document for MVP timeline UI or agent editing. Returns timeline_document plus compose_inputs.',
      inputSchema: projectSchema({
        ...workspaceLocator,
        sceneMomentId: { type: ['string', 'number'] },
        scene_moment_id: { type: ['string', 'number'] },
    ...
    {
      name: 'domain_apply_scene_moment_timeline_commands',
      description: 'Apply OpenCut-style split/trim/move/delete/insert commands to a scene_moment timeline document. If no timeline_document is passed, the tool derives one from sceneMomentId. Returns the edited timeline_document and compose_inputs for composition.',
      inputSchema: projectSchema({
        ...workspaceLocator,
        sceneMomentId: { type: ['string', 'number'], description: 'Required when timeline_document is omitted.' },
        scene_moment_id: { type: ['string', 'number'], description: 'Alias for sceneMomentId.' },
        timelineDocument: { type: 'object', additionalProperties: true },
        timeline_document: { type: 'object', additionalProperties: true },
    ...
    {
      name: 'domain_apply_production_timeline_commands',
      description: 'Apply OpenCut-style split/trim/move/delete/insert commands to a production timeline document. If no timeline_document is passed, the tool derives one from productionId. Returns the edited timeline_document and compose_inputs for final production composition.',
      inputSchema: projectSchema({
        ...workspaceLocator,
        productionId: { type: ['string', 'number'], description: 'Required when timeline_document is omitted.' },
    ...
    {
      name: 'domain_compose_production_from_timeline',
      description: 'Compose the final production video candidate from an edited OpenCut-compatible production timeline_document, or from the current production timeline when omitted. The output video is written as a candidate on the target production content unit.',
      inputSchema: projectSchema({
        ...workspaceLocator,
        productionId: { type: ['string', 'number'], description: 'Production whose scene_moment outputs are composed.' },
        production_id: { type: ['string', 'number'], description: 'Alias for productionId.' },
        contentUnitId: { type: ['string', 'number'], description: 'Production output content unit that receives the composed candidate.' },
        content_unit_id: { type: ['string', 'number'], description: 'Alias for contentUnitId.' },
        candidateId: { type: ['string', 'number'], description: 'Optional candidate id. Defaults to a generated production_comp_* id.' },
        ...
        timelineDocument: { type: 'object', additionalProperties: true, description: 'Optional edited OpenCut-compatible production timeline document returned by domain_read_production_timeline or domain_apply_production_timeline_commands.' },
        timeline_document: { type: 'object', additionalProperties: true, description: 'Alias for timelineDocument.' },
        document: { type: 'object', additionalProperties: true, description: 'Alias for timelineDocument.' },
        ...
        adopt: { type: 'boolean', description: 'When true, immediately adopt/select the composed production candidate after creation.' },
    ...
    },
    {
      ...
      description: 'Compose a scene_moment final video candidate from its derived edit plan or an edited OpenCut-compatible timeline_document. This domain-level tool composes the video track into a new RawResource and writes that output as a candidate on the target scene_moment content unit. Voice/subtitle/audio tracks are preserved in candidate metadata until multi-track mixing is available.',
      inputSchema: projectSchema({
        ...workspaceLocator,
        ...
        folder_id: { type: 'string', description: 'Optional resource library folder id for the composed video.' },
        folderId: { type: 'string', description: 'Camel-case alias for folder_id.' },
        timelineDocument: { type: 'object', additionalProperties: true, description: 'Optional edited OpenCut-compatible timeline document returned by domain_read_scene_moment_timeline or domain_apply_scene_moment_timeline_commands.' },
        timeline_document: { type: 'object', additionalProperties: true, description: 'Alias for timelineDocument.' },
        document: { type: 'object', additionalProperties: true, description: 'Alias for timelineDocument.' },
        max_video_bytes: { type: 'number', description: 'Maximum per-source video bytes for compose.' },
        maxVideoBytes: { type: 'number', description: 'Camel-case alias for max_video_bytes.' },
