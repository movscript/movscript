...
export type APIErrorResponseIntent =
  | { type: 'translation'; key: string; detail?: string; defaultRaw?: string; useFallbackDefault?: boolean }
  | { type: 'raw'; raw: string }
  | { type: 'fallback' }
...
const EXACT_KEYS: Record<string, string> = {
  ...
  '手记不存在': 'apiErrors.scriptNotFound',
  '手记内容不能为空': 'apiErrors.scriptContentEmpty',
...
export function resolveAPIErrorResponseIntent(input: unknown): APIErrorResponseIntent {
  const body = input as APIErrorBodyLike | undefined
  ...
  if (typeof body?.code === 'string' && CODE_KEYS[body.code]) {
    return { type: 'translation', key: CODE_KEYS[body.code], useFallbackDefault: true, ...(raw ? { defaultRaw: raw } : {}) }
  }

  if (!raw) return { type: 'fallback' }

  const exactKey = EXACT_KEYS[raw]
  if (exactKey) return { type: 'translation', key: exactKey }

  const prefix = PREFIX_KEYS.find(([value]) => raw.startsWith(value))
  if (prefix) {
    const detail = raw.slice(prefix[0].length).trim()
    return { type: 'translation', key: prefix[1], detail }
  }

  return { type: 'raw', raw }
}

export function stringErrorValue(value: unknown): string | undefined {
...
export function isGenerationContextDebug(value: unknown): value is { code: string } {
