...
const assetSlotOwnerTypeSchema = {
  enum: ['setting', 'setting_state', 'segment', 'scene_moment', 'content_unit', 'keyframe'],
}
const projectLayerWorkspaceSettingsSchema = {
  type: 'array',
...
export const settingWorkspaceSchema = {
  ...
  jsonSchema: objectSchema(['schema', 'scope', 'workspace'], {
    ...
    mode: workspaceModeSchema,
    workspace: objectSchema([], {
      settings: projectLayerWorkspaceSettingsSchema,
    }),
  ...
  promptSummary: [
    ...
    '',
    'Content shape:',
    '{ schema: "movscript.setting_workspace.v1", scope: "setting_workspace", mode: "snapshot", workspace: { settings?: Array<{ id?, client_id?, name, kind?, description?, status?, merge_candidates? }> }, impact_notes?: string, summary? }',
    '',
    'Rules:',
    '- Setting workspaces only create, update, merge, or retire settings.',
    '- Do not include material requirements, candidate image plans, prompts, generation jobs, or generated resources.',
    '- Workspace content is an editable backend snapshot in workspace.settings. Existing rows keep backend id; new rows use client_id only until apply returns a canonical snapshot with backend ids.',
    '- workspace_create hydrates current settings into workspace.settings when creating the workspace. After creation, edit workspace.settings directly; existing backend ids omitted from the workspace snapshot are intentional delete/retire candidates.',
  ].join('\n'),
  examples: [{
    name: 'basic',
    content: {
...
export const productionWorkspaceSchema = {
  ...
  promptSummary: [
    ...
    '- Existing segments, scene_moments, setting usages, or writing_expressions omitted from the snapshot are treated as removals by backend apply.',
    ...
    '- Each scene_moment should include at least one settings reuse node unless the gap is intentionally explained in impact_notes.',
    '- Use settings with existing project-level ids; do not create project-level settings here.',
...
export const assetWorkspaceSchema = {
  ...
  promptSummary: [
    ...
    '- Put ownership in owner, for example { type: "scene_moment", id: 7 } or { type: "setting", id: 1 }. owner.client_id is only valid inside a same-request bundle where that local reference is also created.',
    '- Asset workspaces must not create isolated assets. For character, location, prop, world-rule, or style-reference material, cite an existing setting backend id through owner, candidate plan references, or context. If the needed setting does not exist, create/update a setting_workspace first and make the asset workspace depend on that setting.',
    ...
    '- When asset_workspace receives scene/location/space settings from setting_workspace or backend settings, extract a required top-down floor-plan asset slot for each scene setting. The slot should be kind="image", owner.type="setting", owner.id=<scene reference id>, and use a stable slot_key such as "top_down_floor_plan".',
    ...
    '- Do not include setting edits, generation jobs, or generated resource bindings.',
  ].join('\n'),
  examples: [{
