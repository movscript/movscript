...
function rateLimitWindowDetail(label: string, value: unknown): string {
  const rateLimitWindow = recordValue(value)
  if (!rateLimitWindow) return ''
  return [
    `${label}:`,
    typeof rateLimitWindow.usedPercent === 'number' ? `${rateLimitWindow.usedPercent}% used` : '',
    typeof rateLimitWindow.windowDurationMins === 'number' ? `${rateLimitWindow.windowDurationMins} min window` : '',
    typeof rateLimitWindow.resetsAt === 'number' ? `resets at ${rateLimitWindow.resetsAt}` : '',
  ].filter(Boolean).join(' ')
}

function rateLimitWindowReached(value: unknown): boolean {
  const rateLimitWindow = recordValue(value)
  return typeof rateLimitWindow?.usedPercent === 'number' && rateLimitWindow.usedPercent >= 100
}
