...
export function publicAgentBackendModelId(model: Pick<AgentBackendPublicModel, 'id' | 'model_id' | 'logical_model_id'>): string {
  return model.model_id?.trim() || model.logical_model_id?.trim() || `model_config:${model.id}`
}

export function publicAgentBackendModelLabel(
  model: Pick<AgentBackendPublicModel, 'display_name' | 'short_name'>,
