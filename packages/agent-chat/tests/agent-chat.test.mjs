...
import * as agentChat from '../dist/index.js'
import * as attachmentInputs from '../dist/attachmentInputs.js'

const agentChatSourceDir = new URL('../src/chat/', import.meta.url)
...
test('agent chat entrypoint exports the provider-neutral runtime contract', () => {
  for (const name of [
...
test('agent chat package metadata publishes the provider-neutral entrypoint', () => {
  const packageSource = readFileSync(new URL('../package.json', import.meta.url), 'utf8')

  assert.match(packageSource, /"name": "@movscript\/agent-chat"/)
  assert.match(packageSource, /"\.": \{/)
  assert.match(packageSource, /"\.\/dist\/index\.js"/)
  assert.match(packageSource, /"\.\/dist\/index\.d\.ts"/)
})

test('agent chat classifies only model-reachable remote image URLs as native image inputs', () => {
  assert.equal(attachmentInputs.isModelReachableRemoteUrl('https://cdn.example.com/frame.png'), true)
  assert.equal(attachmentInputs.isModelReachableRemoteUrl('http://localhost:8765/api/v1/resources/9/file'), false)
  assert.equal(attachmentInputs.isModelReachableRemoteUrl('blob:http://localhost:5173/asset'), false)
  assert.equal(attachmentInputs.isModelReachableRemoteUrl('/api/v1/resources/9/file'), false)
...
test('agent chat prepares provider-session image attachments through source-aware resolvers', async () => {
  const warnings = []
  const refs = await attachmentInputs.prepareProviderSessionAttachmentRefs([
    {
      id: 'att_1',
      name: 'resource.png',
      type: 'image',
...
test('agent chat keeps unresolved image attachments metadata-only and reports resolver warnings', async () => {
  const warnings = []
  const refs = await attachmentInputs.prepareProviderSessionAttachmentRefs([
    {
      id: 'att_1',
