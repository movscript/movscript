...
export type AgentChatToolSurfaceView = {
  key: string
  title: string
  url: string
  intent?: string
...
export function agentChatToolCallView(item: Extract<AgentChatThreadItem, { type: 'mcpToolCall' | 'dynamicToolCall' }>): AgentChatToolCallView {
  return {
    ...
    surfaces: agentChatToolSurfaces(item),
...
function agentChatToolSurfaces(item: Extract<AgentChatThreadItem, { type: 'mcpToolCall' | 'dynamicToolCall' }>): AgentChatToolSurfaceView[] {
  const candidates = item.type === 'mcpToolCall'
    ? agentChatSurfaceCandidates(item.result)
    : agentChatSurfaceCandidates(item.result)
  const seen = new Set<string>()
  return candidates.flatMap((candidate, index) => {
    const surface = agentChatToolSurfaceView(candidate, index)
    if (!surface) return []
    const key = `${surface.url}|${surface.intent ?? ''}|${surface.title}`
...
function agentChatSurfaceCandidates(value: unknown): unknown[] {
  if (!isRecord(value)) return []
  const structuredContent = isRecord(value.structuredContent) ? value.structuredContent : undefined
  return [
    value.surface,
    ...(Array.isArray(value.secondary_surfaces) ? value.secondary_surfaces : []),
    ...(Array.isArray(value.secondarySurfaces) ? value.secondarySurfaces : []),
    structuredContent?.surface,
    ...(Array.isArray(structuredContent?.secondary_surfaces) ? structuredContent.secondary_surfaces : []),
    ...(Array.isArray(structuredContent?.secondarySurfaces) ? structuredContent.secondarySurfaces : []),
  ]
}

function agentChatToolSurfaceView(value: unknown, index: number): AgentChatToolSurfaceView | null {
  if (!isRecord(value)) return null
  const url = stringValue(value.url)
  if (!url) return null
  const title = stringValue(value.title)
    ?? stringValue(value.label)
    ?? agentChatSurfaceIntentLabel(stringValue(value.intent))
    ?? `Surface ${index + 1}`
  return {
    key: `${index}:${url}`,
    title,
    url,
    ...(stringValue(value.intent) ? { intent: stringValue(value.intent) } : {}),
    ...(stringValue(value.route) ? { route: stringValue(value.route) } : {}),
    ...(stringValue(value.description) ? { description: stringValue(value.description) } : {}),
    ...(stringValue(value.usage) ? { usage: stringValue(value.usage) } : {}),
  }
}

function agentChatSurfaceIntentLabel(intent: string | undefined): string | undefined {
  switch (intent) {
    case 'open_resource_library':
    ...
    case 'resource_detail':
      return '查看资源'
    case 'edit_prompt':
    case 'prompt_workbench':
      return '打开提示词工作台'
    case 'monitor_generation':
    case 'generation_job':
      return '查看生成任务'
    case 'review_candidates':
    case 'content_candidates':
      return '查看候选'
