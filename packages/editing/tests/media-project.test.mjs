...
test('creates a production MediaEditingProject from preview timeline clips', () => {
  ...
  assert.equal(project.source.targetKind, 'production')
  assert.equal(project.source.targetRef, 'pilot')
  ...
  assert.equal(project.timeline.metadata.targetKind, 'production')
  assert.equal(project.timeline.metadata.targetRef, 'pilot')
  ...
  assert.equal(project.timeline.tracks[0].clips[0].metadata.movscript.targetRef, 'pilot')
  assert.equal(project.provenance.targetKind, 'production')
  assert.equal(project.provenance.targetRef, 'pilot')
...
test('creates a MediaEditingProject from edit decisions and asset manifest', () => {
  ...
  const music = project.timeline.tracks.find((track) => track.id === 'track_music')
