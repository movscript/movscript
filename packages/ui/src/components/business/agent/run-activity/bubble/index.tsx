...
export const AgentRunActivityBubble = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => {
    return <div ref={ref} className={cn("ms-type-label ms-agent-run-activity-bubble", className)} {...props} />;
  }
);
...
export const AgentRunActivityDetailButton = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ className, size = "xs", variant = "ghost", ...props }, ref) => {
    return <Button ref={ref} size={size} variant={variant} className={cn("ms-type-tiny ms-agent-run-activity-detail-button", className)} {...props} />;
  }
);
