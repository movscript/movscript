...
import type { SettingCardKind, SettingCardStatus, SettingIcon } from "./types";

export const settingKindMeta: Record<SettingCardKind, { label: string; icon: SettingIcon; tone: AccentTone }> = {
...
export const settingStatusMeta: Record<SettingCardStatus, { label: string; intent: StatusIntent }> = {
...
export function normalizeSettingKind(kind?: string): SettingCardKind {
...
export function normalizeSettingStatus(status?: string): SettingCardStatus {
  const normalized = String(status ?? "").toLowerCase();
  if (normalized in settingStatusMeta) return normalized as SettingCardStatus;
  return "workspace";
}

export function accentForSettingKind(kind: SettingCardKind) {
  return accentGradientClass(settingKindMeta[kind].tone);
}
