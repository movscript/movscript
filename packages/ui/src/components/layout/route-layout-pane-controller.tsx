...
interface RouteLayoutPaneHostStateApi {
  getDesktopState?: (input: { key: string }) => Promise<{ value?: unknown }>;
  setDesktopState?: (input: { key: string; value: string }) => Promise<unknown>;
}
...
function readStoredPaneValue(storageKey: string): string | null {
  ...
  const api = readRouteLayoutPaneHostStateApi();
  if (!api?.getDesktopState) return readBrowserStorageItem(storageKey);
  ...
  const legacy = readBrowserStorageItem(storageKey);
...
function writeStoredPaneValue(storageKey: string, value: string): void {
  ...
  void api.setDesktopState({ key: routeLayoutPaneDesktopKey(storageKey), value })
    .then(() => removeBrowserStorageItem(storageKey))
    .catch(() => writeBrowserStorageItem(storageKey, value));
}
...
function readRouteLayoutPaneHostStateApi(): RouteLayoutPaneHostStateApi | undefined {
  if (typeof window === "undefined") return undefined;
  return (window as Window & { api?: RouteLayoutPaneHostStateApi }).api;
}
...
function listenToWindowEvent(
  ...
  addEventListener.call(window, type, listener as EventListener, options);
  return () => {
    const removeEventListener = window.removeEventListener;
    if (typeof removeEventListener !== "function") return;
    removeEventListener.call(window, type, listener as EventListener, options);
  };
}

function publishWindowEvent(event: Event): boolean {
