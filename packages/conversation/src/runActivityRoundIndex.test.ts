...
import { buildAgentRunActivityRoundIndex } from './index'
import type { AgentTimelineActivity } from '@movscript/protocol'

test('buildAgentRunActivityRoundIndex orders rounds, decisions, tools, approvals, then results structurally', () => {
  ...
  assert.deepEqual(roundIndex.rounds.map((round) => round.index), [1])
  assert.deepEqual(roundIndex.rounds[0]?.decisions[0]?.toolCalls.map((call) => call.name), [
    'movscript_focus_get',
    'candidate_asset_slot_attach',
  ])
  assert.deepEqual(roundIndex.rounds[0]?.toolExecutions.map((tool) => tool.id), [
    'step-step_read',
    'step-step_write',
  ])
  assert.deepEqual(roundIndex.rounds[0]?.toolExecutions.map((tool) => tool.decisionOrder), [0, 1])
  const write = roundIndex.rounds[0]?.toolExecutions[1]
  assert.equal(write?.approvals[0]?.id, 'approval_write')
  assert.equal(write?.step?.id, 'step_write')
})

test('buildAgentRunActivityRoundIndex falls back to recorded step order when timestamps are unstable', () => {
  const roundIndex = buildAgentRunActivityRoundIndex(activity({
    steps: [{
      id: 'step_models',
      type: 'tool_call',
  ...
  assert.deepEqual(roundIndex.rounds[0]?.toolExecutions.map((tool) => tool.toolName), [
    'generation_model_list',
    'core_work_start',
  ])
})

test('buildAgentRunActivityRoundIndex keeps approval-only tools as executions before results exist', () => {
  const roundIndex = buildAgentRunActivityRoundIndex(activity({
    approvals: [{
      id: 'approval_write',
      toolName: 'candidate_asset_slot_attach',
  ...
  assert.equal(roundIndex.rounds.length, 1)
  assert.deepEqual(roundIndex.rounds[0]?.toolExecutions.map((tool) => ({
    id: tool.id,
...
test('buildAgentRunActivityRoundIndex keeps late approval-only tools in their decision round', () => {
  const roundIndex = buildAgentRunActivityRoundIndex(activity({
    events: [{
  ...
  assert.deepEqual(roundIndex.rounds.map((round) => round.index), [1, 2])
  assert.deepEqual(roundIndex.rounds[0]?.toolExecutions.map((tool) => ({
    id: tool.id,
  ...
  assert.deepEqual(roundIndex.rounds[1]?.toolExecutions, [])
})

test('buildAgentRunActivityRoundIndex treats blocked input traces as waiting instead of failures', () => {
  const roundIndex = buildAgentRunActivityRoundIndex(activity({
    events: [{
  ...
  assert.equal(roundIndex.rounds[0]?.failed, false)
  assert.deepEqual(roundIndex.rounds[0]?.inputs.map((input) => input.id), ['input_1'])
})

test('buildAgentRunActivityRoundIndex preserves final round metadata', () => {
  const roundIndex = buildAgentRunActivityRoundIndex(activity({
    events: [{
      id: 'event_final',
      kind: 'assistant',
  ...
  assert.equal(roundIndex.rounds[0]?.index, 999)
  assert.equal(roundIndex.rounds[0]?.label, 'Final response')
