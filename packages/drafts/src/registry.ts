...
export const contentUnitProposalSchema = {
  ...
  promptSummary: [
    ...
    '- For kind="shot", use the nested shot object only for camera/framing parameters; do not model Shot as a separate proposal entity.',
...
export const assetProposalSchema = {
  ...
  promptSummary: [
    ...
    '- proposal.asset_slots is the complete desired snapshot. draft_create hydrates current asset slots into proposal.asset_slots when creating the draft; after creation, edit proposal.asset_slots directly.',
    '- Existing backend asset slot ids omitted from proposal.asset_slots are intentional delete/retire candidates.',
    '- Do not generate, renumber, replace, or reorder existing asset slot ids. Only brand-new asset slots that were not present in the draft/base snapshot may omit id or receive a new id according to system rules.',
    ...
    '- When asset_proposal receives scene/location/space creative_references from setting_proposal or backend settings, extract a required top-down floor-plan asset slot for each scene setting. The slot should be kind="image", owner.type="creative_reference", owner.id=<scene reference id>, and use a stable slot_key such as "top_down_floor_plan".',
