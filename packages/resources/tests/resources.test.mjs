...
test('resources package drag payload writes both resource id and resource JSON keys', () => {
...
test('resources package drag payload reads full resources and id fallbacks', () => {
  const dataTransfer = new FakeDataTransfer()
  writeResourceDragPayload(dataTransfer, resource)
...
test('resources package drag payload rejects missing or invalid resource ids', () => {
  const dataTransfer = new FakeDataTransfer()
...
test('resources package video clip rules validate ranges and source size', () => {
...
test('resources package video clip rules validate and derive output file names', () => {
...
test('resources package video clip rules parse clip timecodes', () => {
...
test('resources package video timeline rules normalize local export decisions', () => {
...
test('resources package video timeline rules preserve explicit timeline ordering', () => {
  const clips = normalizeTimelineVideoClips([
...
test('resources package URL rules prefer provider URLs and resolve backend paths', () => {
...
test('resources package upload accept rules cover supported media families', () => {
...
test('resources package script document rules classify supported script files', () => {
...
test('resources package external search snapshot rules normalize persisted filters', () => {
  const result = {
    total: 1,
...
test('resources package external search snapshot rules restore only exact active searches', () => {
  const result = {
    total: 1,
    items: [{ provider_key: 'pexels', media_type: 'image', external_id: '1' }],
    page: 2,
...
test('resources package publishes drag protocol rules without frontend dependencies', () => {
  const packageSource = readFileSync(new URL('../package.json', import.meta.url), 'utf8')
  const tsupSource = readFileSync(new URL('../tsup.config.ts', import.meta.url), 'utf8')
  const source = readFileSync(new URL('../src/dragPayload.ts', import.meta.url), 'utf8')
  const externalResourceSearchSnapshotSource = readFileSync(new URL('../src/externalResourceSearchSnapshot.ts', import.meta.url), 'utf8')
  const mediaCacheSource = readFileSync(new URL('../src/mediaCache.ts', import.meta.url), 'utf8')
  const mediaTypesSource = readFileSync(new URL('../src/mediaTypes.ts', import.meta.url), 'utf8')
  const resourceUrlSource = readFileSync(new URL('../src/resourceUrl.ts', import.meta.url), 'utf8')
  const scriptDocumentsSource = readFileSync(new URL('../src/scriptDocuments.ts', import.meta.url), 'utf8')
  const videoClipSource = readFileSync(new URL('../src/videoClip.ts', import.meta.url), 'utf8')
  const videoTimelineSource = readFileSync(new URL('../src/videoTimeline.ts', import.meta.url), 'utf8')
  const forbiddenFrontendPattern =
    /from ['"]@\/|from ['"]react['"]|@movscript\/ui|\bwindow\.|(^|[^\w])document\.|localStorage|sessionStorage/

  assert.match(packageSource, /"@movscript\/resources"/)
  assert.match(tsupSource, /'src\/index\.ts'/)
