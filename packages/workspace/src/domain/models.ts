...
export const MOVSCRIPT_DOMAIN_WORKSPACE_MODELS: Record<MovScriptDomainWorkspaceKind, MovScriptDomainWorkspaceModel> = {
  ...
  content_unit_workspace: {
    ...
    instructions: ['Content units are project-level stable production units. They declare content_unit_type, output_kind, flat business refs, edit_prompt, generation_references, reference_assets, and model_intent. Specialized content_unit_type adapters perform dependency and regeneration checks; unknown types are valid but untracked for regeneration. Use expression_unit_ref for shot expression units, keyframe_ref/storyboard_ref for visual anchors, and audio_cue_ref for sound cues. Use production editing workspaces for namespace-scope playback assembly. generation_references/reference_assets are the independent model input reference pool. In edit_prompt, use semantic refs such as {{asset::id}}, {{storyboard::id}}, {{keyframe::id}}, {{audio_cue::id}}, {{candidate::id}}, or {{resource::123}} only when that source already exists in the reference pool; prompt text can describe or mention references but must not create model inputs by itself.'],
  },
  keyframe_workspace: {
