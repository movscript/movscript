...
test('builds typed resource mentions and reference assets from semantic ref metadata', async () => {
  const index = indexFromDocuments([
    ...
    document('content_units/cu_arrival_video/content_unit.json', {
      ...
      edit_prompt: {
        text: 'Animate from {{asset::hero_portrait role=first_frame media=image}}.',
      },
    }),
  ])
  ...
  assert.equal(result.ok, true)
  assert.equal(result.prompt.text, 'Animate from @[resource:image:first_frame:321].')
