...
test('resources and pre-production inspector use package menu and empty primitives', () => {
  ...
  assert.match(preProductionSource, /ResourcePrepEmptyInspectorState/)
  assert.match(preProductionSource, /ResourcePrepInspectorTabs/)
  assert.match(preProductionSource, /ResourcePrepInspectorHeader/)
  assert.match(preProductionSource, /ResourcePrepHeaderActionButton/)
  assert.match(preProductionSource, /\bResourcePrepHiddenFileInput\b/)
  assert.match(preProductionSource, /function EmptyInspectorState[\s\S]*?ResourcePrepEmptyInspectorState/)
  ...
  assert.match(preProductionSource, /icon=\{mode === 'asset' \? <PackageCheck size=\{16\} \/> : <Sparkles size=\{16\} \/>/)
...
test('agent full-page surfaces use package page shell layout', () => {
  ...
  assert.match(agentModeCanvasListSource, /\bCanvasListItemIcon\b/)
  assert.match(agentModeCanvasListSource, /\bCanvasListItemMeta\b/)
  assert.match(canvasListPackageSource, /function CanvasListItemMeta[\s\S]*?canvas-list-item__meta/)
  assert.match(agentModeCanvasListSource, /\bCanvasListError\b/, 'agent mode canvas list errors must use package canvas list semantic error surface')
  assert.match(canvasListPackageSource, /function CanvasListError[\s\S]*?toneTextClass\("danger"\)/)
  assert.doesNotMatch(agentModeCanvasListSource, /\btoneTextClass\b/)
  assert.match(agentModeCanvasListSource, /function CanvasRow[\s\S]*?<CanvasListItemIcon[\s\S]*?<Icon size=\{17\}/)
  assert.doesNotMatch(agentModeCanvasListSource, /\b(?:AppEmptyState|AppIconFrame|Badge|Button|Input)\b/)
  assert.doesNotMatch(joinedSources, /text-destructive/)
  assert.doesNotMatch(agentModeCanvasListSource, /flex h-9 w-9 shrink-0 items-center justify-center rounded-md border border-border bg-muted\/30/)
