...
test('production query keys are delegated beyond feature components', () => {
  ...
  assert.match(agentQueryKeysSource, /controlAppServerStatus/)
  ...
  assert.match(agentControlCenterSource, /agentConsoleKeys\.providerModelConfig/)
  assert.match(agentControlCenterSource, /agentConsoleKeys\.controlAppServerStatus/)
  assert.match(agentControlCenterSource, /agentConsoleKeys\.controlCapabilityHealth/)
  assert.match(providerSessionQueryKeysSource, /health: \(baseURL: string, sessionId: string \| null\) => \['provider-session-health', baseURL, sessionId\] as const/)
  assert.match(providerSessionContextSource, /providerSessionKeys\.health\(providerSessionHealthClient\.baseURL, sessionId\?\.trim\(\) \|\| null\)/)
