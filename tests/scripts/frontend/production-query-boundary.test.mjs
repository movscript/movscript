...
test('production query keys are delegated beyond feature components', () => {
  ...
  assert.match(providerSessionQueryKeysSource, /health: \(sessionId: string \| null\) => \['provider-session-health', sessionId\] as const/)
  assert.match(providerSessionContextSource, /providerSessionKeys\.health\(trimmedSessionId\)/)
  assert.match(providerSessionContextSource, /ensureAgentProviderSessionHealth\(\{ sessionId: trimmedSessionId \?\? undefined \}\)/)
  assert.doesNotMatch(providerSessionContextSource, /providerSessionClient|ProviderSessionClient|baseURL/)
  assert.match(providerSessionHealthServiceSource, /providerSessionClient/)
