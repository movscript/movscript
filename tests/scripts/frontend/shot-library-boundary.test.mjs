...
const importClipPlayerSource = readSource('apps/frontend/src/features/shot-library/components/ShotLibraryImportClipPlayer.tsx')
const importWorkspaceBrowserSource = readSource('apps/frontend/src/features/shot-library/components/ShotLibraryImportWorkspaceBrowser.tsx')
...
const importControllerSource = readSource('apps/frontend/src/features/shot-library/components/useShotLibraryImportController.ts')
...
const shotLibraryManualWorkspaceModelSource = readSource('apps/frontend/src/features/shot-library/domain/shotLibraryManualWorkspaceModel.ts')
const shotLibraryPageCss = readSource('apps/frontend/src/features/shot-library/components/ShotLibraryPage.css')
const shotLibraryLayoutCss = readSource('apps/frontend/src/features/shot-library/components/ShotLibraryPage.layout.css')
const shotLibraryDetailCss = readSource('apps/frontend/src/features/shot-library/components/ShotLibraryPage.detail.css')

test('shot library page delegates shot cut Electron API access', () => {
  assert.doesNotMatch(pageSource, /from '@\/features\/shot-library\/components\/shotLibraryImportPreparation'/)
  ...
  assert.match(importControllerSource, /from '@\/features\/shot-library\/components\/shotLibraryImportPreparation'/)
  assert.match(importPreparationSource, /from '@\/features\/shot-library\/application\/shotCutElectron'/)
  assert.match(importPreparationSource, /await analyzeMediaPipelineShotCuts\(/)
  assert.doesNotMatch(importPreparationSource, /window\.api/)
  assert.match(shotCutElectronSource, /export async function analyzeMediaPipelineShotCuts/)
...
test('shot library page delegates workspace conversion model', () => {
  ...
  assert.match(shotLibraryWorkspaceModelSource, /from '@\/features\/shot-library\/domain\/shotLibraryManualWorkspaceModel'/)
  assert.doesNotMatch(shotLibraryWorkspaceModelSource, /export function detailWorkspaceFromEntry/)
  assert.doesNotMatch(shotLibraryWorkspaceModelSource, /export function executionDetailsFromWorkspace/)
  ...
  assert.match(shotLibraryManualWorkspaceModelSource, /export function detailWorkspaceFromEntry/)
  assert.match(shotLibraryManualWorkspaceModelSource, /export function executionDetailsFromWorkspace/)
  assert.match(shotLibraryManualWorkspaceModelSource, /export function manualWorkspaceToManualUpdate/)
  ...
  assert.doesNotMatch(shotLibraryManualWorkspaceModelSource, /window\.api/)
  assert.doesNotMatch(shotLibraryManualWorkspaceModelSource, /window\.addEventListener/)
})
...
test('shot library page delegates reference cards and video preview helpers', () => {
  ...
  assert.match(referenceDetailSource, /from '@\/features\/shot-library\/components\/ShotLibraryImportClipPlayer'/)
...
test('shot library page delegates import dialog components', () => {
  ...
  assert.match(pageSource, /from '@\/features\/shot-library\/components\/useShotLibraryImportController'/)
  ...
  assert.doesNotMatch(pageSource, /async function revealImportWorkspaces/)
  assert.doesNotMatch(pageSource, /async function startImportFromResource/)
  assert.doesNotMatch(pageSource, /loadVideoMetadataFromBlob/)
  ...
  assert.doesNotMatch(importDialogSource, /export function ShotWorkspaceClipPlayer/)
  assert.doesNotMatch(importDialogSource, /subscribeShotLibraryMeasuredBox/)
  assert.doesNotMatch(importDialogSource, /calculateShotWorkspaceGridMetrics/)
  ...
  assert.match(importClipPlayerSource, /export function ShotWorkspaceClipPlayer/)
  assert.match(importClipPlayerSource, /import \{ ResourceVideo \}/)
  assert.match(importWorkspaceBrowserSource, /export function ShotImportWorkspaceBrowser/)
  assert.match(importWorkspaceBrowserSource, /from '@\/features\/shot-library\/components\/ShotLibraryImportClipPlayer'/)
  assert.match(importWorkspaceBrowserSource, /subscribeShotLibraryMeasuredBox/)
  assert.match(importWorkspaceBrowserSource, /calculateShotWorkspaceGridMetrics/)
  ...
  assert.match(importControllerSource, /export function useShotLibraryImportController/)
  assert.match(importControllerSource, /async \(file: File \| undefined\)/)
  assert.match(importControllerSource, /async \(resource: RawResource\)/)
  assert.match(importControllerSource, /const revealImportWorkspaces = useCallback/)
})
...
test('shot library page splits browser layout and detail card styles', () => {
  assert.match(shotLibraryPageCss, /@import '\.\/ShotLibraryPage\.layout\.css'/)
  assert.match(shotLibraryPageCss, /@import '\.\/ShotLibraryPage\.detail\.css'/)
  assert.match(shotLibraryLayoutCss, /\.shot-library-page__body\s*\{/)
  assert.match(shotLibraryLayoutCss, /\.shot-library-page__grid\s*\{/)
  assert.doesNotMatch(shotLibraryLayoutCss, /\.shot-reference-card\s*\{/)
  assert.doesNotMatch(shotLibraryLayoutCss, /\.shot-library-manual-form\s*\{/)
  assert.doesNotMatch(shotLibraryLayoutCss, /\.shot-library-page__match-panel\s*\{/)
  assert.match(shotLibraryDetailCss, /\.shot-reference-card\s*\{/)
  assert.match(shotLibraryDetailCss, /\.shot-library-page__detail-card\s*\{/)
  assert.match(shotLibraryDetailCss, /\.shot-library-manual-form\s*\{/)
  assert.match(shotLibraryDetailCss, /\.shot-library-page__match-panel\s*\{/)
})
