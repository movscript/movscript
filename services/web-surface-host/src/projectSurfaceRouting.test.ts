...
test('web route context preserves non-production timeline focus over stale production query', () => {
  ...
  assert.equal(context.domainFocus.target?.targetRef, 'timeline_assembly:episode:episode_01')
})

test('web route context derives legacy production from timeline assembly ref alias only for production scope', () => {
  const production = projectRouteContext(
    '/studio/7/preview',
    new URLSearchParams('projectId=7&timeline_assembly_ref=timeline_assembly:production:pilot'),
  )
  assert.equal(production.productionId, 'pilot')
  assert.equal(production.domainFocus.target?.targetRef, 'timeline_assembly:production:pilot')

  const episode = projectRouteContext(
    '/studio/7/preview',
    new URLSearchParams('projectId=7&productionId=pilot&timeline_assembly_ref=timeline_assembly:episode:episode_01'),
  )
...
test('web project surface href normalizes focus query while preserving production compatibility', () => {
  const episodeHref = webProjectSurfaceHref({
    ...
    projectId: '7',
    search: new URLSearchParams('productionId=pilot&timeline_assembly_ref=timeline_assembly:episode:episode_01'),
  })
  const episodeUrl = new URL(episodeHref, 'http://localhost')
  assert.equal(episodeUrl.searchParams.get('productionId'), null)
  assert.equal(episodeUrl.searchParams.get('timeline_assembly_ref'), 'timeline_assembly:episode:episode_01')

  const productionQuery = new URLSearchParams('timeline_assembly_ref=timeline_assembly:production:pilot')
  normalizeTimelineFocusQuery(productionQuery)
  assert.equal(productionQuery.get('productionId'), 'pilot')
  assert.equal(productionQuery.get('timeline_assembly_ref'), 'timeline_assembly:production:pilot')
})
