...
func TestShotReferenceCreateFromResourceReturnsForbiddenForInvisibleResource(t *testing.T) {
	...
	owner := newHandlerExternalUser("owner")
	requester := newHandlerExternalUser("requester")
	...
	router.Use(func(c *gin.Context) {
		setTestAuthContextUser(c, handlerUserProfile(requester))
		c.Next()
	})
...
func TestShotReferencePatchAcceptsProfessionalAnnotationFields(t *testing.T) {
	...
	user := newHandlerExternalUser("owner")
	resource := model.RawResource{
		OwnerID:        user.ID,
		Type:           "video",
	...
	router := gin.New()
	router.Use(func(c *gin.Context) {
		setTestAuthContextUser(c, domainidentity.UserProfile{ID: user.ID, Username: user.Username, Status: domainidentity.UserStatusActive})
		c.Next()
	})
