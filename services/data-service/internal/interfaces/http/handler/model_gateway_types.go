...
type modelGatewayIdentity interface {
	authidentity.Reader
	authidentity.OrgDirectory
}

func NewModelGatewayHandler(db *gorm.DB, svc *ai.AIService, identity ...modelGatewayIdentity) *ModelGatewayHandler {
	var reader modelGatewayIdentity
	if len(identity) > 0 {
		reader = identity[0]
	}
	return &ModelGatewayHandler{db: db, service: modelgatewayapp.NewServiceWithIdentity(db, reader, svc)}
}
