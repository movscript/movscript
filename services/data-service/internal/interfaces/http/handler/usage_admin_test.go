...
func TestUsageLogListFiltersByOrgProjectAndSince(t *testing.T) {
	...
	db := testutil.OpenSQLite(t, "handler-usage-list.db", &persistencemodel.UsageLog{})
	user := newHandlerExternalUser("usage-list-user")
...
func TestUsageLogListPaginates(t *testing.T) {
	gin.SetMode(gin.TestMode)
	db := testutil.OpenSQLite(t, "handler-usage-pagination.db", &persistencemodel.UsageLog{})
	user := newHandlerExternalUser("usage-pagination-user")
	runtimeModelID := uint(1)
