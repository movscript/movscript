...
func TestResourceAdminServeFileReadsAnyResource(t *testing.T) {
	...
	user := newHandlerExternalUser("preview-owner")
	resource := persistencemodel.RawResource{
		OwnerID:        user.ID,
		Type:           "image",
...
func TestResourceServeFileReturnsImmutableCacheHeadersAndNotModified(t *testing.T) {
	gin.SetMode(gin.TestMode)
	db := testutil.OpenSQLite(t, "handler-resource-serve-file.db", &persistencemodel.RawResource{})
...
func newTestResourceAdminRouter(t *testing.T) (*gin.Engine, *gorm.DB, *handlerFakeStorage) {
	t.Helper()
	db := testutil.OpenSQLite(t, "handler-resource-admin.db", &persistencemodel.ResourceBlob{}, &persistencemodel.RawResource{}, &persistencemodel.CanvasTask{}, &persistencemodel.AuditLog{})
