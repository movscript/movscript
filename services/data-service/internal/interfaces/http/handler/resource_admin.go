...
func NewResourceAdminHandler(db *gorm.DB, store storage.Storage, identity ...authidentity.Reader) *ResourceAdminHandler {
	return &ResourceAdminHandler{service: adminresource.NewService(db, identity...), store: store, db: db}
}
