...
func TestProjectAdminForceSetOwnerWritesAuditWithPreviousOwner(t *testing.T) {
	...
	router, db, identity := newTestProjectAdminRouterWithIdentity(t)

	owner := newHandlerExternalUser("old-owner")
	newOwner := newHandlerExternalUser("new-owner")
	identity.users[owner.ID] = handlerUserProfile(owner)
	identity.users[newOwner.ID] = handlerUserProfile(newOwner)
	...
	disabledOwner := newHandlerExternalUserWithStatus("disabled-owner", "disabled")
	identity.users[disabledOwner.ID] = domainidentity.UserProfile{ID: disabledOwner.ID, Username: disabledOwner.Username, SystemRole: domainidentity.SystemRoleUser, Status: "disabled"}
...
func TestProjectAdminCreateWritesAudit(t *testing.T) {
	...
	owner := newHandlerExternalUserWithStatus("admin-create-owner", "active")
	identity.users[owner.ID] = domainidentity.UserProfile{ID: owner.ID, Username: owner.Username, SystemRole: domainidentity.SystemRoleUser, Status: domainidentity.UserStatusActive}
	org := persistencemodel.Organization{Name: "Admin Create Org", Slug: "admin-create-org", Plan: "team", Status: "active", CreatedBy: owner.ID}
	if err := db.Create(&org).Error; err != nil {
		t.Fatalf("create org: %v", err)
	}
	identity.orgs[org.ID] = authidentity.Organization{
		ID:        org.ID,
		Name:      org.Name,
		Slug:      org.Slug,
		Plan:      org.Plan,
	...
	identity.members[org.ID] = map[uint]authidentity.OrganizationMember{
		owner.ID: {OrgID: org.ID, UserID: owner.ID, Role: domainorg.RoleOwner},
	}
	...
	disabledOwner := newHandlerExternalUserWithStatus("admin-create-disabled-owner", "disabled")
	identity.users[disabledOwner.ID] = domainidentity.UserProfile{ID: disabledOwner.ID, Username: disabledOwner.Username, SystemRole: domainidentity.SystemRoleUser, Status: "disabled"}
	disabledReq := httptest.NewRequest(
		http.MethodPost,
		"/admin/projects",
...
func TestProjectAdminListFiltersByProjectID(t *testing.T) {
	...
	owner := newHandlerExternalUserWithStatus("admin-list-owner", "active")
...
func TestProjectAdminUpdateWritesAuditAndValidatesName(t *testing.T) {
	...
	owner := newHandlerExternalUserWithStatus("admin-update-owner", "active")
...
func TestProjectAdminDetailReturnsOperationalSummary(t *testing.T) {
	...
	owner := newHandlerExternalUserWithStatus("detail-owner", "active")
...
func TestProjectAdminDeleteWritesAuditAndRejectsMissing(t *testing.T) {
	...
	owner := newHandlerExternalUserWithStatus("delete-owner", "active")
...
func TestProjectAdminMemberActionsWriteAuditAndRejectFailures(t *testing.T) {
	...
	owner := newHandlerExternalUser("member-owner")
	memberUser := newHandlerExternalUser("project-member")
	disabledUser := newHandlerExternalUserWithStatus("disabled-project-member", "disabled")
	identity.users[memberUser.ID] = handlerUserProfile(memberUser)
	identity.users[disabledUser.ID] = handlerUserProfile(disabledUser)
...
func TestProjectMemberActionsWriteOrgAudit(t *testing.T) {
	...
	owner := newHandlerExternalUser("workspace-owner")
	memberUser := newHandlerExternalUser("workspace-member")
	...
	identity.members[org.ID] = map[uint]authidentity.OrganizationMember{
		memberUser.ID: {OrgID: org.ID, UserID: memberUser.ID, Role: "member", User: &domainidentity.UserProfile{ID: memberUser.ID, Username: memberUser.Username, Status: domainidentity.UserStatusActive}},
	}
...
func TestProjectGitProxyUsesLocalGitHTTPBackend(t *testing.T) {
	...
	owner := newHandlerExternalUserWithStatus("local-git-owner", "active")
	...
	identity := projectAdminIdentityFor(owner, org)
	h := NewProjectHandlerWithConfigEncryptionTokensAndIdentity(db.Session(&gorm.Session{SkipHooks: true}), &config.Config{
		WorkspaceStorageBackend: "http",
...
func TestProjectGitProxyAllowsSmartHTTPAndInjectsServerToken(t *testing.T) {
	...
	owner := newHandlerExternalUserWithStatus("git-proxy-owner", "active")
	...
	identity := projectAdminIdentityFor(owner, org)
	h := NewProjectHandlerWithConfigEncryptionTokensAndIdentity(db.Session(&gorm.Session{SkipHooks: true}), &config.Config{}, nil, nil, identity)
...
func TestProjectGitProxySupportsEnterpriseGitProviders(t *testing.T) {
	for _, tt := range []struct {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			identity := projectAdminIdentityFor(owner, org)
			h := NewProjectHandlerWithConfigEncryptionTokensAndIdentity(db.Session(&gorm.Session{SkipHooks: true}), &config.Config{}, nil, nil, identity)
			h.repositories = projectrepoapp.NewServiceWithIdentity(db.Session(&gorm.Session{SkipHooks: true}), projectrepoapp.Config{Provider: tt.provider, RepoPrefix: "project-"}, nil, identity)
			h.httpClient = &http.Client{Transport: upstream}
...
func projectAdminIdentityFor(owner testutil.ExternalUser, org persistencemodel.Organization) *fakeAuthIdentityManager {
	identity := newFakeAuthIdentityManager()
	identity.users[owner.ID] = handlerUserProfile(owner)
	identity.orgs[org.ID] = authidentity.Organization{
		ID:         org.ID,
...
func newTestProjectAdminRouter(t *testing.T) (*gin.Engine, *gorm.DB) {
	router, db, _ := newTestProjectAdminRouterWithIdentity(t)
	return router, db
}

func newTestProjectAdminRouterWithIdentity(t *testing.T) (*gin.Engine, *gorm.DB, *fakeAuthIdentityManager) {
	t.Helper()
