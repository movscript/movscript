package middleware

import (
	"errors"
	"net/http"
	"strconv"
	"strings"

	"github.com/gin-gonic/gin"
	"github.com/movscript/auth-service/pkg/authidentity"
	domainidentity "github.com/movscript/auth-service/pkg/authidentity/identity"
	projectapp "github.com/movscript/movscript/internal/app/project"
	domainorg "github.com/movscript/movscript/internal/domain/org"
	"github.com/movscript/movscript/internal/interfaces/http/api"
	"gorm.io/gorm"
)

const ContextOrgMemberKey = "currentOrgMember"

var (
	errMembershipForbidden = errors.New("membership forbidden")
	errMembershipSuspended = errors.New("membership suspended")
)

// ResolveOrgMember selects the current workspace membership for the authenticated user.
// It prefers X-Org-ID when provided, otherwise falls back to the personal org,
// then any other org membership if no personal org exists.
func ResolveOrgMember(identity authidentity.Reader) gin.HandlerFunc {
	return func(c *gin.Context) {
		user, ok := CurrentUserFromContext(c)
		if !ok {
			c.Next()
			return
		}
		if user.SystemRole == domainidentity.SystemRoleSuperAdmin && isAdminAPIPath(c.Request.URL.Path) {
			c.Next()
			return
		}

		var preferredOrgID *uint
		if raw := firstNonEmptyString(c.GetHeader("X-Org-ID"), c.Query("org_id")); raw != "" {
			parsed, err := strconv.ParseUint(raw, 10, 64)
			if err != nil || parsed == 0 {
				c.AbortWithStatusJSON(http.StatusBadRequest, api.InvalidInput("无效的组织 ID"))
				return
			}
			orgID := uint(parsed)
			preferredOrgID = &orgID
		} else if raw, ok := c.Get(ContextPreferredOrgIDKey); ok {
			switch id := raw.(type) {
			case uint:
				if id != 0 {
					preferredOrgID = &id
				}
			case *uint:
				if id != nil && *id != 0 {
					preferredOrgID = id
				}
			}
		}

		member, found, err := resolveCurrentMember(c, identity, user.ID, preferredOrgID)
		if err == errMembershipForbidden {
			c.AbortWithStatusJSON(http.StatusForbidden, api.Forbidden("你没有权限访问该工作区"))
			return
		}
		if errors.Is(err, errMembershipSuspended) {
			c.AbortWithStatusJSON(http.StatusForbidden, api.Forbidden("工作区已暂停"))
			return
		}
		if err != nil || !found {
			c.Next()
			return
		}
		c.Set(ContextOrgMemberKey, member)
		c.Next()
	}
}

func resolveCurrentMember(c *gin.Context, identity authidentity.Reader, userID uint, preferredOrgID *uint) (domainorg.OrganizationMember, bool, error) {
	if identity == nil {
		return domainorg.OrganizationMember{}, false, nil
	}
	memberships, err := identity.OrgMemberships(c.Request.Context(), userID)
	if err != nil {
		return domainorg.OrganizationMember{}, false, err
	}
	if preferredOrgID != nil {
		for _, membership := range memberships {
			if membership.OrgID != *preferredOrgID {
				continue
			}
			if membership.Status == domainorg.StatusSuspended {
				return domainorg.OrganizationMember{}, false, errMembershipSuspended
			}
			return orgMemberFromIdentityMembership(userID, membership), true, nil
		}
		return domainorg.OrganizationMember{}, false, errMembershipForbidden
	}
	for _, membership := range memberships {
		if membership.IsPersonal && membership.Status != domainorg.StatusSuspended {
			return orgMemberFromIdentityMembership(userID, membership), true, nil
		}
	}
	seenSuspended := false
	for _, membership := range memberships {
		if membership.Status == domainorg.StatusSuspended {
			seenSuspended = true
			continue
		}
		return orgMemberFromIdentityMembership(userID, membership), true, nil
	}
	if seenSuspended {
		return domainorg.OrganizationMember{}, false, errMembershipSuspended
	}
	return domainorg.OrganizationMember{}, false, nil
}

func orgMemberFromIdentityMembership(userID uint, membership authidentity.OrgMembership) domainorg.OrganizationMember {
	return domainorg.OrganizationMember{
		OrgID:  membership.OrgID,
		UserID: userID,
		Role:   membership.Role,
	}
}

func firstNonEmptyString(values ...string) string {
	for _, value := range values {
		if trimmed := strings.TrimSpace(value); trimmed != "" {
			return trimmed
		}
	}
	return ""
}

func isAdminAPIPath(path string) bool {
	return path == "/api/v1/admin" || strings.HasPrefix(path, "/api/v1/admin/")
}

// RequireProjectInCurrentOrg aborts when :id does not belong to the current workspace.
// It is intended for /projects/:id and nested project routes.
func RequireProjectInCurrentOrg(db *gorm.DB) gin.HandlerFunc {
	projectService := projectapp.NewService(db)
	return func(c *gin.Context) {
		member, ok := CurrentOrgMemberFromContext(c)
		if !ok {
			c.AbortWithStatusJSON(http.StatusForbidden, api.Forbidden("无工作区信息"))
			return
		}

		projectID, err := strconv.ParseUint(c.Param("id"), 10, 64)
		if err != nil || projectID == 0 {
			c.AbortWithStatusJSON(http.StatusBadRequest, api.InvalidInput("无效的项目 ID"))
			return
		}

		belongs, err := projectService.BelongsToOrg(c.Request.Context(), uint(projectID), member.OrgID)
		if err == projectapp.ErrProjectNotFound {
			c.AbortWithStatusJSON(http.StatusNotFound, api.NotFound("项目不存在"))
			return
		}
		if err != nil {
			c.AbortWithStatusJSON(http.StatusInternalServerError, api.Internal("项目校验失败"))
			return
		}
		if !belongs {
			c.AbortWithStatusJSON(http.StatusForbidden, api.ForbiddenProject("项目不属于当前工作区"))
			return
		}
		c.Next()
	}
}

// RequireProjectRole aborts with 403 unless the current user has one of the
// allowed project roles. Super admins resolve through the project service as a
// project-level super_admin role, but the project must still exist.
func RequireProjectRole(db *gorm.DB, roles ...string) gin.HandlerFunc {
	projectService := projectapp.NewService(db)
	allowed := make(map[string]struct{}, len(roles))
	for _, role := range roles {
		allowed[role] = struct{}{}
	}
	return func(c *gin.Context) {
		user, ok := CurrentUserFromContext(c)
		if !ok {
			c.AbortWithStatusJSON(http.StatusUnauthorized, api.AuthRequired())
			return
		}
		projectID, err := strconv.ParseUint(c.Param("id"), 10, 64)
		if err != nil || projectID == 0 {
			c.AbortWithStatusJSON(http.StatusBadRequest, api.InvalidInput("无效的项目 ID"))
			return
		}
		role, err := projectService.ResolveRole(c.Request.Context(), uint(projectID), user.ID, user.SystemRole)
		if errors.Is(err, projectapp.ErrProjectNotFound) {
			c.AbortWithStatusJSON(http.StatusNotFound, api.NotFound("项目不存在"))
			return
		}
		if err != nil {
			c.AbortWithStatusJSON(http.StatusForbidden, api.Forbidden("权限不足"))
			return
		}
		if _, ok := allowed[role.Role]; ok {
			c.Next()
			return
		}
		c.AbortWithStatusJSON(http.StatusForbidden, api.Forbidden("权限不足"))
	}
}

// InjectOrgMember loads the OrganizationMember for the current user + :orgId path param.
// Sets ContextOrgMemberKey in gin context. Aborts with 403 if user is not a member.
func InjectOrgMember(identity authidentity.Reader) gin.HandlerFunc {
	return func(c *gin.Context) {
		user, ok := CurrentUserFromContext(c)
		if !ok {
			c.AbortWithStatusJSON(http.StatusUnauthorized, api.AuthRequired())
			return
		}

		orgIDStr := c.Param("orgId")
		orgID, err := strconv.ParseUint(orgIDStr, 10, 64)
		if err != nil || orgID == 0 {
			c.AbortWithStatusJSON(http.StatusBadRequest, api.InvalidInput("无效的组织 ID"))
			return
		}

		member, err := memberForUserOrg(c, identity, user.ID, uint(orgID))
		if err != nil {
			if errors.Is(err, errMembershipSuspended) {
				c.AbortWithStatusJSON(http.StatusForbidden, api.Forbidden("工作区已暂停"))
				return
			}
			c.AbortWithStatusJSON(http.StatusForbidden, api.Forbidden("你不是该组织的成员"))
			return
		}

		c.Set(ContextOrgMemberKey, member)
		c.Next()
	}
}

func memberForUserOrg(c *gin.Context, identity authidentity.Reader, userID uint, orgID uint) (domainorg.OrganizationMember, error) {
	if identity == nil {
		return domainorg.OrganizationMember{}, errMembershipForbidden
	}
	memberships, err := identity.OrgMemberships(c.Request.Context(), userID)
	if err != nil {
		return domainorg.OrganizationMember{}, err
	}
	for _, membership := range memberships {
		if membership.OrgID != orgID {
			continue
		}
		if membership.Status == domainorg.StatusSuspended {
			return domainorg.OrganizationMember{}, errMembershipSuspended
		}
		return orgMemberFromIdentityMembership(userID, membership), nil
	}
	return domainorg.OrganizationMember{}, errMembershipForbidden
}

// RequireOrgRole aborts with 403 if the injected org member doesn't have one of the given roles.
// Must be used after InjectOrgMember.
func RequireOrgRole(roles ...string) gin.HandlerFunc {
	return func(c *gin.Context) {
		member, ok := CurrentOrgMemberFromContext(c)
		if !ok {
			c.AbortWithStatusJSON(http.StatusForbidden, api.Forbidden("无组织成员信息"))
			return
		}
		for _, r := range roles {
			if member.Role == r {
				c.Next()
				return
			}
		}
		c.AbortWithStatusJSON(http.StatusForbidden, api.Forbidden("权限不足"))
	}
}
