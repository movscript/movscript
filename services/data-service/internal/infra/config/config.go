...
type Config struct {
	...
	GitProxyTokenSecret   string
	GitProxyTokenTTLHours int
	...
	// Provider asset certification gateways.
	ProviderAssetPublicBaseURL string
	ProviderAssetSigningSecret string
...
func Load() *Config {
	...
	gitProxyTokenSecret := getEnv("GIT_PROXY_TOKEN_SECRET", "")
	...
	cfg := &Config{
		...
		AuthMode:              normalizeAuthMode(getEnv("MOVSCRIPT_AUTH_MODE", "")),
		AuthBaseURL:           strings.TrimRight(getEnv("MOVSCRIPT_AUTH_BASE_URL", ""), "/"),
		AuthManagementToken:   getEnv("MOVSCRIPT_AUTH_SERVICE_MANAGEMENT_TOKEN", ""),
		GitProxyTokenSecret:   gitProxyTokenSecret,
		GitProxyTokenTTLHours: getEnvInt("GIT_PROXY_TOKEN_TTL_HOURS", 24),
		...
		ProviderAssetPublicBaseURL: strings.TrimRight(getEnv("MOVSCRIPT_PROVIDER_ASSET_PUBLIC_BASE_URL", getEnv("PUBLIC_RESOURCE_BASE_URL", "")), "/"),
		ProviderAssetSigningSecret: getEnv("MOVSCRIPT_PROVIDER_ASSET_SIGNING_SECRET", gitProxyTokenSecret),
...
func (c *Config) ValidateStartup() error {
	...
	if c.GitProxyTokenTTLHours <= 0 {
		problems = append(problems, "GIT_PROXY_TOKEN_TTL_HOURS must be greater than 0")
	}
	authMode := effectiveAuthMode(c)
	switch authMode {
	case "", "opaque-key", "local-owner", "no-auth", "test":
	default:
		problems = append(problems, "MOVSCRIPT_AUTH_MODE must be one of: opaque-key, local-owner, no-auth, test")
	}
	if authMode == "opaque-key" && strings.TrimSpace(c.AuthBaseURL) == "" {
		problems = append(problems, "MOVSCRIPT_AUTH_BASE_URL must be set when MOVSCRIPT_AUTH_MODE=opaque-key")
	}
...
func (c *Config) SafeSummary() map[string]any {
	summary := map[string]any{
		...
		"git_proxy_token_ttl_hours":         c.GitProxyTokenTTLHours,
		"auth_mode":                         effectiveAuthMode(c),
		"auth_base_url":                     c.AuthBaseURL,
		"auth_management_token_set":         strings.TrimSpace(c.AuthManagementToken) != "",
		...
		"git_proxy_token_secret_set":        c.GitProxyTokenSecret != "",
		...
		"provider_asset_public_base_url":    c.ProviderAssetPublicBaseURL,
		"provider_asset_signing_secret_set": strings.TrimSpace(c.ProviderAssetSigningSecret) != "",
...
func effectiveAuthMode(c *Config) string {
	if c == nil {
		return ""
	}
	if mode := normalizeAuthMode(c.AuthMode); mode != "" {
		return mode
	}
	if strings.TrimSpace(c.AuthBaseURL) != "" {
		return "opaque-key"
	}
	if strings.TrimSpace(c.AppMode) == "local" || normalizeDependencyProfile(c.DependencyProfile) == "local" {
		return "local-owner"
	}
	return "opaque-key"
}
