...
func TestLoadAuthProviderConfig(t *testing.T) {
	t.Setenv("MOVSCRIPT_AUTH_MODE", "opaque-key")
	t.Setenv("MOVSCRIPT_AUTH_BASE_URL", "http://auth.example/")
	t.Setenv("MOVSCRIPT_AUTH_SERVICE_MANAGEMENT_TOKEN", "sk-admin")

	cfg := Load()

	if cfg.AuthMode != "opaque-key" {
		t.Fatalf("AuthMode = %q, want opaque-key", cfg.AuthMode)
	}
	if cfg.AuthBaseURL != "http://auth.example" {
		t.Fatalf("AuthBaseURL = %q, want trimmed base url", cfg.AuthBaseURL)
	}
	if cfg.AuthManagementToken != "sk-admin" {
		t.Fatalf("AuthManagementToken = %q, want management token", cfg.AuthManagementToken)
	}
	if cfg.EffectiveAuthMode() != "opaque-key" {
		t.Fatalf("EffectiveAuthMode() = %q, want opaque-key", cfg.EffectiveAuthMode())
	}
}

func TestEffectiveAuthModeDefaultsLocalProfileToLocalOwner(t *testing.T) {
	cfg := &Config{AppMode: "local", DependencyProfile: "local"}

	if cfg.EffectiveAuthMode() != "local-owner" {
		t.Fatalf("EffectiveAuthMode() = %q, want local-owner", cfg.EffectiveAuthMode())
	}
}

func TestEffectiveAuthModeUsesOpaqueKeyWhenBaseURLIsConfigured(t *testing.T) {
	cfg := &Config{AppMode: "cloud", AuthBaseURL: "http://auth.example"}
...
func TestValidateStartupRequiresAuthBaseURLForOpaqueKeyMode(t *testing.T) {
	cfg := &Config{
		EncryptionKey:           "0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef",
		GitProxyTokenSecret:     "test-git-proxy-token-secret",
		GitProxyTokenTTLHours:   24,
		AuthMode:                "opaque-key",
		DBDriver:                "sqlite",
		DBPath:                  ":memory:",
		...
		GitBinary:               "git",
		AIGatewayProvider:       "local",
		VectorIndexProvider:     "local-index",
		MediaProcessingProvider: "desktop-managed",
		AgentRuntimeProvider:    "desktop-managed",
	}

	if err := cfg.ValidateStartup(); err == nil {
		t.Fatal("ValidateStartup returned nil for opaque-key mode without base URL")
	}
}
