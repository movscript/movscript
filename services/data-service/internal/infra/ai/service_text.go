...
func (s *AIService) callCatalogTextRuntime(ctx context.Context, userID uint, route ModelRoute, runtime catalogRouteRuntime, capability string, req TextRequest, usage UsageContext) (TextResponse, error) {
	...
	if usage.ReservationID == nil {
		estimate := estimateUsage(runtime.model.usageProfile(), runtime.def, "text", estimateTextInputTokens(attemptReq), maxPositive(attemptReq.MaxTokens, 1024), 0, 1)
	...
	estimate := estimateUsageDetails(runtime.model.usageProfile(), runtime.def, "text", resp.Usage, 0, 1)
...
func (s *AIService) callCatalogResponsesStreamRuntime(ctx context.Context, userID uint, route ModelRoute, runtime catalogRouteRuntime, req ResponsesRequest, usage UsageContext) (<-chan ResponsesStreamEvent, error) {
	...
	if usage.ReservationID == nil {
		estimate := estimateUsage(runtime.model.usageProfile(), runtime.def, "text", estimateTextInputTokens(attemptReq.Text), maxPositive(attemptReq.Text.MaxTokens, 1024), 0, 1)
		reservation, err := s.ReserveUsage(ctx, userID, route.RuntimeModelID, estimate, usage)
		if err != nil {
			return nil, err
		}
	...
	return s.wrapResponsesStream(ctx, userID, route.RuntimeModelID, route.CredentialID, runtime.model.usageProfile(), runtime.def, runtime.adapterType, attemptReq, usage, start, finishAttempt, upstream), nil
}

func (s *AIService) wrapResponsesStream(ctx context.Context, userID uint, billRuntimeModelID uint, credentialID uint, profile modelUsageProfile, def *ModelDef, provider string, req ResponsesRequest, usage UsageContext, start time.Time, finishAttempt func(error), upstream <-chan ResponsesStreamEvent) <-chan ResponsesStreamEvent {
	...
	go func() {
		...
		estimate := estimateUsageDetails(profile, def, "text", tokenUsage, 0, 1)
...
func (s *AIService) callCatalogTextStreamRuntime(ctx context.Context, userID uint, route ModelRoute, runtime catalogRouteRuntime, req TextRequest, usage UsageContext) (<-chan TextStreamEvent, error) {
	...
	go func() {
		...
		estimate := estimateUsageDetails(runtime.model.usageProfile(), runtime.def, "text", tokenUsage, 0, 1)
