...
func (s *AIService) EstimateTextRouteCost(ctx context.Context, userID uint, route ModelRoute, req TextRequest) (UsageEstimate, error) {
	...
	for _, capability := range textRuntimeCapabilities() {
		...
		if handled {
			...
			return estimateUsage(definition.model.usageProfile(), definition.def, "text", inputTokens, outputTokens, 0, 1), nil
		}
	}
...
func (s *AIService) EstimateCapabilityPerCallRouteCost(ctx context.Context, userID uint, route ModelRoute, capability string) (UsageEstimate, error) {
	...
	if handled {
		return estimateUsage(definition.model.usageProfile(), definition.def, capability, 0, 0, 0, 1), nil
	}
...
func (s *AIService) EstimateAudioGenerateRouteCost(ctx context.Context, userID uint, route ModelRoute, capability string, durationSec int) (UsageEstimate, error) {
	...
	if handled {
		return estimateUsage(definition.model.usageProfile(), definition.def, capability, 0, 0, positiveAudioDuration(durationSec, definition.def), 1), nil
	}
...
func estimateUsage(profile modelUsageProfile, def *ModelDef, opType string, inputTokens, outputTokens, durationSec, imageCount int) UsageEstimate {
	return estimateUsageDetails(profile, def, opType, TokenUsage{InputTokens: inputTokens, OutputTokens: outputTokens}, durationSec, imageCount)
}

func estimateUsageDetails(profile modelUsageProfile, def *ModelDef, opType string, usage TokenUsage, durationSec, imageCount int) UsageEstimate {
	if imageCount <= 0 {
		imageCount = 1
	}
	return UsageEstimate{
		...
		Cost:              calcUsageCost(profile, def, usage.InputTokens, usage.OutputTokens, durationSec, imageCount),
	}
}
