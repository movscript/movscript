...
func TestBuiltinWorkflowTemplatesExposeReusablePorts(t *testing.T) {
	...
	var data canvasdomain.NodeData
...
func TestTemplateCanvasAppliesWorkflowDefaults(t *testing.T) {
	...
	cv := TemplateCanvas(7, tpl, "My Flow", "generation")
	...
	if cv.Stage != "generation" || cv.WorkflowTags != `["image"]` {
		t.Fatalf("unexpected canvas stage/tags: %+v", cv)
	}
	modelCanvas := cv.ToModel()
	modelCanvas.ID = 31
	roundTrip := canvasdomain.CanvasFromModel(modelCanvas)
