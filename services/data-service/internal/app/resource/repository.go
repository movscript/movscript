...
func (r *gormRepository) Transaction(ctx context.Context, fn func(repository) error) error {
	return r.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		return fn(&gormRepository{db: tx, identity: r.identity})
	})
}
...
type UsageSummary struct {
	...
	Jobs        []UsageJob        `json:"jobs"`
	Derivatives []UsageDerivative `json:"derivatives"`
...
type UsageDerivative struct {
	ID               uint      `json:"id"`
	Role             string    `json:"role"`
	OutputResourceID uint      `json:"output_resource_id"`
...
func (r *gormRepository) ResourceUsages(ctx context.Context, input UsageInput) (UsageSummary, error) {
	summary := UsageSummary{ResourceID: input.ResourceID}
	jobs, err := r.resourceUsageJobs(ctx, input)
	if err != nil {
		return summary, err
	}
	derivatives, err := r.resourceUsageDerivatives(ctx, input.ResourceID)
	if err != nil {
		return summary, err
	}
	decisions, err := r.resourceUsageDecisions(ctx, input)
	if err != nil {
		return summary, err
	}
	summary.Jobs = jobs
	...
	summary.Counts = UsageCounts{
		Jobs:        len(jobs),
		Derivatives: len(derivatives),
		Decisions:   len(decisions),
		Total:       len(jobs) + len(derivatives) + len(decisions),
	}
	return summary, nil
}

func (r *gormRepository) resourceUsageJobs(ctx context.Context, input UsageInput) ([]UsageJob, error) {
	if !r.db.Migrator().HasTable(&persistencemodel.Job{}) {
		return []UsageJob{}, nil
	}
	rows := make([]persistencemodel.Job, 0)
	q := r.db.WithContext(ctx).Model(&persistencemodel.Job{}).Where("user_id = ?", input.UserID)
	q = r.applyJobOrgScope(ctx, q, input.OrgID, input.UserID)
	q = q.Where("input_resource_id = ? OR output_resource_id = ? OR input_resource_ids = ? OR input_resource_ids LIKE ? OR input_resource_ids LIKE ? OR input_resource_ids LIKE ?", jobResourceReferenceArgs(input.ResourceID)...)
	if err := q.Order("id desc").Limit(50).Find(&rows).Error; err != nil {
		return nil, err
	}
	out := make([]UsageJob, 0, len(rows))
	for _, row := range rows {
		out = append(out, UsageJob{
			ID:        row.ID,
			Role:      jobUsageRole(row, input.ResourceID),
			Status:    row.Status,
			JobType:   row.JobType,
			Title:     row.Title,
...
func (r *gormRepository) resourceUsageDerivatives(ctx context.Context, resourceID uint) ([]UsageDerivative, error) {
	if !r.db.Migrator().HasTable(&persistencemodel.ResourceDerivative{}) {
		return []UsageDerivative{}, nil
	}
	rows := make([]persistencemodel.ResourceDerivative, 0)
	if err := r.db.WithContext(ctx).
		Model(&persistencemodel.ResourceDerivative{}).
		Where("output_resource_id = ? OR input_resource_ids = ? OR input_resource_ids LIKE ? OR input_resource_ids LIKE ? OR input_resource_ids LIKE ?", resourceDerivativeReferenceArgs(resourceID)...).
		Order("id desc").
		Limit(50).
		Find(&rows).Error; err != nil {
		return nil, err
	}
	out := make([]UsageDerivative, 0, len(rows))
	for _, row := range rows {
		role := "input"
		if row.OutputResourceID == resourceID {
			role = "output"
		}
		out = append(out, UsageDerivative{
			ID:               row.ID,
			Role:             role,
...
func (r *gormRepository) resourceUsageDecisions(ctx context.Context, input UsageInput) ([]UsageDecision, error) {
	if !r.db.Migrator().HasTable(&persistencemodel.DecisionContext{}) {
		return []UsageDecision{}, nil
	}
	rows := make([]persistencemodel.DecisionContext, 0)
	projectIDs, err := r.visibleProjectIDs(ctx, input.UserID, input.OrgID)
	if err != nil {
		return nil, err
	}
	if len(projectIDs) == 0 {
	...
	pattern := `%"resource_id":` + strconv.FormatUint(uint64(input.ResourceID), 10) + `%`
	if err := r.db.WithContext(ctx).
		Model(&persistencemodel.DecisionContext{}).
		Where("project_id IN ?", projectIDs).
		Where("candidates_json LIKE ? OR selection_json LIKE ?", pattern, pattern).
		Order("updated_at desc").
		Limit(100).
		Find(&rows).Error; err != nil {
		return nil, err
	}
	out := make([]UsageDecision, 0, len(rows))
	for _, row := range rows {
		out = append(out, decisionUsagesForResource(row, input.ResourceID)...)
	}
	return out, nil
}

func (r *gormRepository) visibleProjectIDs(ctx context.Context, userID uint, orgID *uint) ([]uint, error) {
	if !r.db.Migrator().HasTable(&persistencemodel.Project{}) {
		return []uint{}, nil
	}
	ids := make([]uint, 0)
	q := r.db.WithContext(ctx).Model(&persistencemodel.Project{}).Select("id")
	if orgID != nil {
		q = q.Where("org_id = ? OR owner_id = ?", *orgID, userID)
	} else {
...
func (r *gormRepository) applyJobOrgScope(ctx context.Context, q *gorm.DB, orgID *uint, userID uint) *gorm.DB {
	if orgID == nil {
...
func jobUsageRole(row persistencemodel.Job, resourceID uint) string {
	if row.OutputResourceID != nil && *row.OutputResourceID == resourceID {
		return "output"
	}
	return "input"
}

func decisionUsagesForResource(row persistencemodel.DecisionContext, resourceID uint) []UsageDecision {
	out := make([]UsageDecision, 0)
	candidates := make([]map[string]any, 0)
	_ = json.Unmarshal([]byte(row.CandidatesJSON), &candidates)
	for _, candidate := range candidates {
		if candidateUsesResource(candidate, resourceID) {
			out = append(out, UsageDecision{
				ID:          row.ID,
				ProjectID:   row.ProjectID,
				TargetKind:  row.TargetKind,
				TargetRef:   row.TargetRef,
				CandidateID: stringFromJSON(candidate["id"]),
	...
	_ = json.Unmarshal([]byte(row.SelectionJSON), &selection)
	if uintFromJSON(selection["resource_id"]) == resourceID {
		out = append(out, UsageDecision{
			ID:          row.ID,
			ProjectID:   row.ProjectID,
			TargetKind:  row.TargetKind,
			TargetRef:   row.TargetRef,
			CandidateID: stringFromJSON(selection["candidate_id"]),
			Role:        "selection_resource",
			Status:      row.Status,
			CreatedAt:   row.CreatedAt,
			UpdatedAt:   row.UpdatedAt,
		})
...
func candidateUsesResource(candidate map[string]any, resourceID uint) bool {
	if uintFromJSON(candidate["resource_id"]) == resourceID {
		return true
	}
	outputs, ok := candidate["outputs"].([]any)
	if !ok {
		return false
	}
	for _, output := range outputs {
		...
		if uintFromJSON(record["resource_id"]) == resourceID {
			return true
		}
	}
	return false
...
func uintFromJSON(value any) uint {
	switch typed := value.(type) {
	case float64:
		if typed > 0 {
			return uint(typed)
...
func resourceUpdateColumns(spec domainresource.UpdateSpec) map[string]any {
	...
	if spec.ProviderAssetCertifications != nil {
		raw, err := json.Marshal(spec.ProviderAssetCertifications)
		if err == nil {
			updates["provider_asset_certifications"] = string(raw)
		}
	}
	if spec.ProviderGeneratedArtifact != nil {
		raw, err := json.Marshal(spec.ProviderGeneratedArtifact)
		if err == nil {
			updates["provider_generated_artifact"] = string(raw)
		}
	}
	return updates
}
