...
type projectRepoIdentity interface {
	authidentity.Reader
...
func NewService(db *gorm.DB, cfg Config, adapter GitRepositoryAdapter) *Service {
	return NewServiceWithIdentity(db, cfg, adapter, nil)
}

func NewServiceWithIdentity(db *gorm.DB, cfg Config, adapter GitRepositoryAdapter, identity projectRepoIdentity) *Service {
	return NewServiceWithRepositoryAndIdentity(&gormRepository{db: db}, cfg, adapter, identity)
}

func NewServiceWithRepository(repo repository, cfg Config, adapter GitRepositoryAdapter) *Service {
	return NewServiceWithRepositoryAndIdentity(repo, cfg, adapter, nil)
}

func NewServiceWithRepositoryAndIdentity(repo repository, cfg Config, adapter GitRepositoryAdapter, identity projectRepoIdentity) *Service {
	return &Service{repo: repo, config: normalizeConfig(cfg), adapter: adapter, identity: identity}
}
...
func (s *Service) GitProxyTarget(ctx context.Context, projectID uint, orgID *uint) (GitProxyTarget, error) {
	...
	if s.adapter == nil {
		if spec, err := s.bindingSpec(ctx, project); err == nil && strings.TrimSpace(spec.Owner) != "" {
...
func (s *Service) provisionRepository(ctx context.Context, project projectRecord, binding repositoryBinding) repositoryBinding {
	...
	ownerName, err := s.ownerName(ctx, project)
	if err != nil {
		updated, updateErr := s.repo.UpdateProvisioning(ctx, binding.ID, StatusError, binding.ProviderRepoID, binding.HeadCommit, err.Error())
		if updateErr == nil {
			return updated
		}
	...
	result, err := s.adapter.EnsureRepository(ctx, EnsureRepositoryInput{
		...
		Private:       true,
		OwnerType:     s.ownerType(ctx, project),
		OwnerName:     ownerName,
	})
	if err != nil {
...
func (s *Service) projectOwner(ctx context.Context, project projectRecord) (string, error) {
	...
	if org == nil || org.IsPersonal {
		profile, err := s.ownerProfile(ctx, project.OwnerID)
		if err != nil {
			return "", err
		}
	...
	if slug == "" {
		return "", fmt.Errorf("%w: project organization slug is required", ErrInvalidRepositoryConfig)
	}
...
func (s *Service) ownerName(ctx context.Context, project projectRecord) (string, error) {
	...
	if name == "" {
		return "", fmt.Errorf("%w: project organization name is required", ErrInvalidRepositoryConfig)
	}
	return name, nil
}
...
func (s *Service) projectOrg(ctx context.Context, project projectRecord) (*authidentity.Organization, error) {
	...
	if s.identity == nil {
		return nil, fmt.Errorf("%w: project organization identity is required", ErrInvalidRepositoryConfig)
	}
	page, err := s.identity.ListOrgs(ctx, authidentity.ListOrgsFilter{OrgID: project.OrgID, Page: 1, PageSize: 1})
	if err != nil || len(page.Items) == 0 {
		return nil, fmt.Errorf("%w: project organization identity unavailable", ErrInvalidRepositoryConfig)
	}
	return &page.Items[0], nil
}

func (s *Service) ownerProfile(ctx context.Context, ownerID uint) (domainidentity.UserProfile, error) {
	if ownerID == 0 || s.identity == nil {
		return domainidentity.UserProfile{}, fmt.Errorf("%w: project owner identity is required", ErrInvalidRepositoryConfig)
	}
	profile, err := s.identity.UserProfile(ctx, ownerID)
	if err != nil {
		return domainidentity.UserProfile{}, fmt.Errorf("%w: project owner identity unavailable", ErrInvalidRepositoryConfig)
	}
	if strings.TrimSpace(profile.Username) == "" {
		return domainidentity.UserProfile{}, fmt.Errorf("%w: project owner username is required", ErrInvalidRepositoryConfig)
	}
	return profile, nil
}
