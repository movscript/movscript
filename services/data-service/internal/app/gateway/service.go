...
func NewServiceWithIdentity(db *gorm.DB, identity gatewayIdentity, aiService ...*ai.AIService) *Service {
	var svc *ai.AIService
	if len(aiService) > 0 {
		svc = aiService[0]
	}
	repo := &gormRepository{db: db}
	service := &Service{repo: repo, ai: svc, policy: &PolicyService{repo: repo, identity: identity}, identity: identity}
	if svc != nil {
		service.catalog = svc
		service.routing = svc
	}
...
func (s *Service) PrincipalForAPIKey(ctx context.Context, rawKey string) (Principal, bool, error) {
	...
	ownerActive, err := s.gatewayKeyOwnerActive(ctx, key.OwnerUserID)
	if err != nil {
		return Principal{}, false, err
	}
	if !ownerActive {
...
func (s *Service) gatewayKeyOwnerActive(ctx context.Context, userID uint) (bool, error) {
	if s.identity == nil {
		return false, nil
	}
	...
	return profile.Status == domainidentity.UserStatusActive, nil
}
