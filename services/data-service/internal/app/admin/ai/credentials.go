...
func (s *Service) CreateCredential(ctx context.Context, input CreateCredentialInput) (domainai.Credential, error) {
	return s.createCredential(ctx, input, true)
}

func (s *Service) createCredential(ctx context.Context, input CreateCredentialInput, syncProviderMirror bool) (domainai.Credential, error) {
	...
	if syncProviderMirror {
		if err := s.syncProviderFromLegacyCredential(ctx, cred.ToModel()); err != nil {
			return cred, err
		}
	}
	return cred, nil
...
func (s *Service) DeleteCredential(ctx context.Context, id string) (domainai.Credential, error) {
	...
	if err := s.disableLegacyProviderCredential(ctx, credentialID); err != nil {
		return cred, err
	}
	return cred, nil
}
