...
func TestListResourcesFiltersAndIncludesOwner(t *testing.T) {
	...
	orgID := uint(22)
	otherOrgID := uint(23)
	createResourceWithOrg(t, db, ownerID, &orgID, "poster.png", "image", "local", 120)
	createResourceWithOrg(t, db, ownerID, &otherOrgID, "poster-copy.png", "image", "local", 180)
	createResource(t, db, ownerID, "clip.mp4", "video", "s3", 240)

	service := NewService(db, fakeResourceIdentity{
		profiles: map[uint]domainidentity.UserProfile{
			ownerID: {ID: ownerID, Username: "alice", SystemRole: domainidentity.SystemRoleUser, Status: domainidentity.UserStatusActive},
		},
	})
...
func TestStorageStatsIncludesAuthIdentityUsername(t *testing.T) {
	db := newTestDB(t)
	ownerID := uint(9)
	createResource(t, db, ownerID, "poster.png", "image", "local", 120)

	service := NewService(db, fakeResourceIdentity{
		profiles: map[uint]domainidentity.UserProfile{
			ownerID: {ID: ownerID, Username: "resource-owner", SystemRole: domainidentity.SystemRoleUser, Status: domainidentity.UserStatusActive},
		},
	})
	stats, err := service.StorageStats(context.Background())
	if err != nil {
		t.Fatalf("StorageStats returned error: %v", err)
	}
	if len(stats) != 1 || stats[0].Username != "resource-owner" {
		t.Fatalf("stats = %+v, want AuthIdentity username", stats)
	}
}
...
func newTestDB(t *testing.T) *gorm.DB {
	t.Helper()
	return testutil.OpenSQLite(t, "adminresource.db", &persistencemodel.ResourceBlob{}, &persistencemodel.RawResource{}, &persistencemodel.CanvasTask{})
}

func createUser(t *testing.T, _ *gorm.DB, username string) testutil.ExternalUser {
	t.Helper()
	nextAdminResourceUserID++
	return testutil.NewExternalUser(nextAdminResourceUserID, username)
}

var nextAdminResourceUserID uint = 100
...
type fakeResourceIdentity struct {
	profiles map[uint]domainidentity.UserProfile
}

func (f fakeResourceIdentity) UserProfile(_ context.Context, userID uint) (domainidentity.UserProfile, error) {
...
func (f fakeResourceIdentity) OrgMemberships(_ context.Context, _ uint) ([]authidentity.OrgMembership, error) {
	return nil, nil
}
