...
func TestServiceUsesGatewayUsageReporterContract(t *testing.T) {
	...
	reporter := &fakeUsageReporter{
		...
		summary: providercontract.AIGatewayUsageSummary{
			...
			TopUsers:    []providercontract.AIGatewayUsageUserSummary{{UserID: 7, AIGatewayUsageTotals: providercontract.AIGatewayUsageTotals{Records: 1, Cost: 0.5}}},
			GeneratedAt: now,
		},
	}
	service := NewServiceWithReporter(reporter, fakeUsageIdentity{
		profiles: map[uint]domainidentity.UserProfile{
			7: {ID: 7, Username: "identity-user", SystemRole: domainidentity.SystemRoleUser, Status: domainidentity.UserStatusActive},
		},
	})
	...
	if page.Items[0].User == nil || page.Items[0].User.Username != "identity-user" {
		t.Fatalf("page item user = %#v, want AuthIdentity user ref", page.Items[0].User)
	}
	...
	if exported[0].User == nil || exported[0].User.Username != "identity-user" {
		t.Fatalf("export user = %#v, want AuthIdentity user ref", exported[0].User)
	}
	...
	if len(summary.TopUsers) != 1 || summary.TopUsers[0].User == nil || summary.TopUsers[0].User.Username != "identity-user" {
		t.Fatalf("summary top users = %#v, want AuthIdentity user refs", summary.TopUsers)
	}
...
func TestSummaryAggregatesFilteredUsage(t *testing.T) {
	db := testutil.OpenSQLite(t, "adminusage.db", &persistencemodel.AICredential{}, &persistencemodel.AIModelCatalogEntry{}, &persistencemodel.AIModelRouteBinding{}, &persistencemodel.UsageLog{})
	...
	service := NewService(db, fakeUsageIdentity{
		profiles: map[uint]domainidentity.UserProfile{
			userA.ID: {ID: userA.ID, Username: "alice", SystemRole: domainidentity.SystemRoleUser, Status: domainidentity.UserStatusActive},
			userB.ID: {ID: userB.ID, Username: "bob", SystemRole: domainidentity.SystemRoleUser, Status: domainidentity.UserStatusActive},
		},
	})
...
type fakeUsageIdentity struct {
	profiles map[uint]domainidentity.UserProfile
}

func (f fakeUsageIdentity) UserProfile(_ context.Context, userID uint) (domainidentity.UserProfile, error) {
...
func (f fakeUsageIdentity) OrgMemberships(context.Context, uint) ([]authidentity.OrgMembership, error) {
	return nil, nil
}

func createUsageUser(t *testing.T, _ *gorm.DB, username string) testutil.ExternalUser {
	t.Helper()
	nextUsageUserID++
	return testutil.NewExternalUser(nextUsageUserID, username)
}

var nextUsageUserID uint = 100
