...
func New() (*App, error) {
	...
	if strings.TrimSpace(cfg.GitProxyTokenSecret) != "" {
		var err error
		tokens, err = scopedtoken.NewManager(cfg.GitProxyTokenSecret, time.Duration(cfg.GitProxyTokenTTLHours)*time.Hour)
		if err != nil {
	...
	authIdentity, err := buildAuthIdentityManager(cfg)
	if err != nil {
		return nil, fmt.Errorf("initialize auth identity manager: %w", err)
	}
	...
	entitlements := entitlementapp.NewServiceWithIdentity(database, cfg, authIdentity)
